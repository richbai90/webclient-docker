...
class CSwKnowldgeBaseAccess extends CSwLocalDbConnection
{
	...
	function KbQuery($Query, $QueryType, $Catalog = 0, $MaxResults = 100, $KeywordFlags = 0xFFFF0000, $analyst = false, $boolRecordset=false)
	{
		$arrCols = array("docref","date","title");
		if(!isset($boolRecordset))$boolRecordset=false;
		if(!isset($boolMakeRS))$boolMakeRS=null;
		$Columns = "*";
		$strOperation = "";
		$strQueryType = "";
		...
		{
			//var_dump(htmlentities($xmlmc->paramsxml));
	...
	function GetDocumentURL($docref, &$docurl, $strServerIP)
	{
		$xmlmc = new XmlMethodCall();
		$strServer = $this->_server;
		...
		if($xmlmc->Invoke("knowledgebase","documentGetUrl",$strServer))
		{
			...
			$docurl = str_replace("&[app.server]:&[app.httpport]", $strUseIP . ":" . $_SERVER['SERVER_PORT'], $docurl);
	...
	// -- Get URL to open the document
	// -- Use $_SERVER['SERVER_NAME'] to determine the document path
	function GetExternalDocumentURL($docref, &$docurl, $strServerName)
	{
		$xmlmc = new XmlMethodCall();
		$strServer = $this->_server;
		$xmlmc->SetParam("docRef",$docref);
		if($xmlmc->Invoke("knowledgebase","documentGetUrl",$strServer)){
			...
			//-- We currently hard-code the instance ID at the moment '/sw'
			//-- nwj 15.05.2008 bug 68781 - ensure we use port setting of the server
			$docurl = str_replace("&[app.webroot]", "http://" . $strServerName . ":" . $_SERVER['SERVER_PORT'] ."/sw", $docurl);

			$docurl = str_replace("&[app.server]:&[app.httpport]", $strServerName . ":" . $_SERVER['SERVER_PORT'], $docurl);
			//-- CB 2010.05.20 - new documentation typically has this url: http://&; [app.server]/documentation/ ...
			// so we want to expand this variable as well.
			$docurl = str_replace("&[app.server]", $strServerName . ":" . $_SERVER['SERVER_PORT'], $docurl);
					...
					for ($i=0;$i<$dTotal;$i++)
					{
						...
						if($colNode->node_name()!="#text" && $colNode->node_name()!="#comment")
						{
							...
							if($strColName == "docVisibleToCustomers")
								$GLOBALS[$docref]["docVisibleToCustomers"] = $colNode->get_content();
						}
					}
				}
			}
