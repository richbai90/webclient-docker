...
class PHPSQLParser
{
		...
		#returns their location.  This might be faster as a regex
		 function count_paren($token,$chars=array('(',')')) {
			$len = strlen($token);
			$open=array();
			$close=array();
			for($i=0;$i<$len;++$i){
...
;
		...
		*/
		 function process_sql(&$tokens,$start_at = 0, $stop_at = false) {
			$prev_category = "";
		...
		*/
		 function process_set_list($tokens) {
			$column="";
			$expression="";
		...
		*/
		 function process_select(&$tokens) {
			$expression = "";
			$expr = array();
			foreach($tokens as $token) {
				if($token == ',') {
		...
		*/
		 ...
		 function trimSubquery($sq)
		{
		 ...
		 function process_group(&$tokens, &$select) {
		...
		*/
		 function process_expr_list($tokens) {
			$expr = "";
			$type = "";
			$prev_token = "";
			$skip_next = false;
...
//					}
	...
	*/
		 ...
		 function process_update($tokens) {

		}

		 function process_delete($tokens) {
			$tables = array();
			$del = $tokens['DELETE'];
