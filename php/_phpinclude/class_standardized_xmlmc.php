...
class XmlMethodCall
{
    ...
    public function generatexmlcall($service, $method)
    {
        ...
        foreach ($this->params as $paramName => $paramValue) {
            ...
            $xml .= "<" . $paramName . ">" . $this->prepareForXml(utf8_encode($paramValue)) . "</" . $paramName . ">";
        }
        if ($bParams) {
            $xml .= "</params>";
        }
    ...
    private function prepareForXml($strValue)
	{
		$xmlchars = array("&", "<", ">",'"',"'");
		$escapechars = array("&amp;", "&lt;", "&gt;","&quot;","&apos;");
    ...
    public function GetParam($strName)
    {
        if ($this->_parseAsJson) {
            return $this->_getJsonParam($strName);
        }

        return $this->_getXmlParam($strName);
...
class swphpXmlMethodCall extends XmlMethodCall {}