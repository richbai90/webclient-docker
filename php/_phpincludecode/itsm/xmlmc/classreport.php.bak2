...
class classReport
{
	...
	function load_report($reportFileName)
	{
		...
		// DTH_ 20140922 moving the location of the XML files to the same folder as the PHP file which runs the report
		// If there is no path so we need to get the path. The first time this class is created there will only be a file name and no path.
		// The second time it is created (after any user input has been entered and the criteria is submitted), the path will be different 
		if (strpos($this->xmlFileName, "/") == false) 
		{
			$reportPath = $GLOBALS['script_path']; // the folder location of the PHP file which launches the report and also the XML definition file			
			$this->xmlFileName = substr($reportPath, 0, strrpos($reportPath, "/")) . "/" . $reportFileName.".xml"; // replace PHP file name with XML filename
		}
		$this->xmlfile = file_get_contents($this->xmlFileName);
		//-- create dom instance of the xml file
		$xmlDoc = domxml_open_mem($this->xmlfile);
	...
	function construct_select($arrCriteria,$strOrderByColumn = "", $strOrderByDir = "ASC")
	{
		...
		//-- TK MSSQL FIX
		if($GLOBALS['HTTP_SESSION_VARS']['wc_mssqlinuse'] == 1)
		{
			$arrCriteria['db_type'] = "mssql";
		}
		if($GLOBALS['HTTP_SESSION_VARS']['wc_oracleinuse'] == 1)
		{
			$arrCriteria['db_type'] = "oracle";
		}
	...
	function construct_select_by_last_level($arrCriteria, $strOrderByColumn = "", $strOrderByDir = "ASC", &$setConversion, &$setDBName, &$namesToDisplay, $isCSV){
		...
		if($this->xmlRoot){
			...
			//All the columns from the xml are in $xmlColumns
			$xmlColumns = $this->xmlRoot->get_elements_by_tagname("column");
			//All the freetext from the xml
			$xmlfreetext = $this->xmlRoot->get_elements_by_tagname("freetext");
			//
			$xmlTables = $this->xmlRoot->get_elements_by_tagname("table");

			//Getting all the level from the xml
			foreach($xmlColumns as $colKey => $aCol){
				$strLevel = getAttribute("level", $aCol->attributes());
				if (!in_array($strLevel,$array_level))
					array_unshift($array_level, $strLevel);
			}

			//Getting the query by levels - > COLUMNS
			foreach($array_level as $unique_level) {
				  
				  $level_table = "";
				  //FIRST GETTING COLUMNS
				  foreach($xmlColumns as $colKey => $aCol)
				  {
				  	//-- create select cols
					$strLevel = getAttribute("level", $aCol->attributes());

					if ($strLevel<>$unique_level)
						continue;
					...
					$bColAlias = getAttribute("dbtablealias", $aCol->attributes());
					$displayName = $aCol->get_content();
					

					//array_push($setDBName, $strColName);
					//array_push($setConversion, $strConversion);
					$setDBName[]=$strColName;
					$setConversion[]=$strConversion;
					$namesToDisplay[]=$displayName;
					...
					if($strCast!=""){
						...
						if ($strSelect != "")
							$strSelect .= " , ";
						if ($arrCriteria['db_type']=='oracle'){
							//Before add, being sure it was not added before
							if (strpos($strSelect, $selCol .' "'.$strColName.'"')== false)
							   $strSelect .= $selCol .' "'.$strColName.'"';
						}
						else{
							$strColNameStriped = str_replace("(","",$strColName);				
							...
							if (strpos($strSelect, $selCol ." as '".$strColNameStriped."'")== false)			
								$strSelect .= $selCol ." as '".$strColNameStriped."'";
						}
						if($strGroupBy != "")
							$strGroupBy .= " , ";
						...
						if (strpos($strSelect, $selCol)== false){
							if($strSelect != "")
								$strSelect .= " , ";
							$strSelect .= $selCol;
						}
						
						if (strpos($strGroupBy, $selCol)== false){
							if($strGroupBy != "")$strGroupBy .= " , ";
							$strGroupBy .= $selCol;	
						}
					}

				  $level_table = $strColTable;	
				  }
			}
		...
		$strSelectStatement = $strSelect . " from " . $strTables;
		if ($strJoins != "") 
			$strSelectStatement .= " where " . $strJoins . " and ";
		$strWhereClause = $this->construct_criteria($arrCriteria, $isCSV);
	...
	function construct_criteria($arrCriteria, $isCSV=false)
	{
		$strUserCriteria = "";
		if($this->xmlRoot)
		{
		...
		if($strStaticCriteria!="")
		{
			...
			//Check if the field static has the instruction order by
			//In case we want to download, it is necessary to quit the order
			if ($isCSV){

				$position_order_by = strpos($strStaticCriteria, 'order by');
				if ($position_order_by !== false){
					$new_Static_Criteria = substr($strStaticCriteria, 0, $position_order_by-1);
					$strStaticCriteria = $new_Static_Criteria;
				}
			}
	...
	function perform_search($arrCriteria,$level = 1, $strOrderByColumn = "", $strOrderByDir = "ASC", $isCSV=false, $dateRange = "")
	{
		foreach($arrCriteria as $key=>$val){
			$arrCriteria[$key] = html_entity_decode($val);
		}
		...
		if ($isCSV){
			...
			$strSelect = $this->construct_select_by_last_level($arrCriteria, $strOrderByColumn,$strOrderByDir,$array_Conversion, $array_Name, $names_Display,$isCSV);
		}
		else
		...
		else
		{
			...
			//Yes CSV -> XMLC result as text
			if ($isCSV)
				$strResults = create_content_CSV($recSet,$strOrderByColumn, $strOrderByDir,$array_Conversion, $array_Name, $names_Display, $dateRange);
			//No CSV -> XMLC result as html
			else{
				if(!$recSet){
							//	return "<center>Could not run SQL query on DSN [".$strDSN."]. Please contact your Supportworks administrator<br><br>" . $strSQL . "</center>";
					...
					$strResults = $this->output_results($recSet,$strOrderByColumn, $strOrderByDir, $isCSV);
			}
			unset($recset);
		}
		//$reportODBC->Close();
	...
	function perform_trend_search($arrCriteria,$thisdummy, $strOrderByColumn = "", $strOrderByDir = "ASC", $isCSV=false)
	{
			...
			while($currentStart<$finalEnd)
			{
				...
				$strOutput .= "<div style='width:95%;overflow:hidden;'>";
				$block_content .= $this->perform_search($arrCriteria,1,$strOrderByColumn, $strOrderByDir, true,date($dt_fmt,$currentStart).' to '.date($dt_fmt,$currentEnd+1)); 
...
function create_report_daterange($binding,$op,$prompt,$xmlInput)
{
	...
	$strHTML = "<p>".$prompt."<br/><input readonly style='width:133px;' class='input-date' maxlength=10 type='text' id='".$binding."' intype='daterange' op='>='> and <input readonly class='input-date' maxlength=10 style='width:133px;' type='text' id='".$binding."__1' intype='daterange' op='<='></p>";
	return $strHTML;
}
...
{
	$binding = str_replace('.','_',$binding);
	$strHTML = "<p>".$prompt."<br/><input readonly style='width:133px;' class='input-date' maxlength=10 type='text' id='".$binding."' intype='date' op='".$op."'></p>";
	return $strHTML;
}
...
function create_report_dbpicklist($binding,$op,$prompt,$xmlInput)
{
	...
	else
	{
		...
		if(!$recSet)
		{
			//return "<center>Could not run SQL query on DSN [".$tableODBC->DSN."]. Please contact your Supportworks administrator<br><br>" . $strSQL . "</center>";
		...
		else
		{
			...
			while (!$recSet->eof) 
			{
				...
				$keycol = $recSet->f("keycol");
				$txtcol = $recSet->f("txtcol");
				$strHTML .= "<option value='".urlencode($keycol)."'>".$txtcol."</option>";
				$recSet->movenext();
			}
			$strHTML .= "</select></p>";
		}
					...
					else
					{
						...
						if($strConversion=="callref")
						{
							if ( ($_SESSION['portalmode'] == "FATCLIENT") && (gv('phpprintmode')!="1") )
							{
								...
								if ($port==80)
								   $strDoc = "http://" . $machine . "/sw/clisupp/details/" . gv('dd') . "/";
								else
								  $strDoc = "http://" . $machine . ":" . $port ."/sw/clisupp/details/" . gv('dd') . "/";
								
								if ($_SERVER['HTTPS'] =="on") $strDoc=str_replace("http://","https://", $strDoc);
								$DisplayfieldValue = "<a href='".$strDoc."itsm_swcall.php?callref=".$fieldValue."'>".swcallref_str($fieldValue)."</a>";
							}
							else
							{
...
function create_content_csv($oRS,$xmlCols, $xmlReport, $array_Conversion, $array_Name, $names_Display, $dateRange=""){
	$rawHeader="";
	$rawData="";

	$flag_seen=1;
	$jump_header = 0;
	$size_conversion = count($array_Conversion);
	$size_name = count($array_Name);
	$size_display = count($names_Display);
	...
	while (!$oRS->eof) {
		...
		for ($i=0;$i<$size_display;$i++){
			...
			if ($flag_seen){
				...
				if ($i<$size_display-1)
					$rawHeader.= strtolower($names_Display[$i])." ,";
				else
					$rawHeader.= strtolower($names_Display[$i]);
			}

			//Data
			$data = $oRS->xf(strtolower($array_Name[$i]));
			$aux = strtolower($data);
			$data = $aux;


			//Check if the data has a comma -> If it has, replace for blank spaces 
			/*$position_first_comma = strpos($data, ',');
			if ($position_first_comma !== false){
				//$auxiliar = str_replace(","," ",$data);
				$aux = chr(34).$data.chr(34);
				$data = $aux;
			}*/


			if (strcmp($array_Conversion[$i],"callref")==0){
				if ($i<$size_display-1)
					$rawData.=swcallref_str($data). " ,";
				else
					$rawData.=swcallref_str($data);
			}
			else{
				if ($i<$size_display-1)
					$rawData.=common_convert_field_value($array_Conversion[$i],$data,$array_Name[$i]) . " ,";
				else
					$rawData.=common_convert_field_value($array_Conversion[$i],$data,$array_Name[$i]);
			}


			//Before next jump check if it is neccesary fill up the headers again
			if ($i==$size_display-1){
				$flag_seen=0;
				$jump_header++;
			}
...
function output_summary_datalist(&$oRS, $arrXmlCols, &$xmlReport,$strOrderByColumn = "", $strOrderByDir = "ASC")
{
	...
	if($xmlReport->criteria['wssm']==true)
	{
		$style = "width: 100%;  overflow:hidden;display:inline;float:left; ";
	}else
	{
		$style = "clear:left;width: 100%;  overflow:hidden;display:inline;float:left; ";
	}
	...
	$strHTML .= "<div class='header' style=\"width:100%;display:inline;float:left; \">";
	$strHTML .=			$strHeader;
	$strHTML .= "</div>";
	...
	$strHTML .= "<div style=\"width: 100%;  overflow:hidden;display:inline;float:right; \">";
	$strHTML .= 		$strDataRows;
	$strHTML .= "</div>";
	$strHTML .= "</div>";
	return $strHTML;
...
function create_summary_table_header($xmlCols,$xmlReport,$strOrderByColumn = "", $strOrderByDir = "ASC")
{
	...
	{
		$strHTML .= "&nbsp;&nbsp;&nbsp;".end($arrLabels). "";
		$strHTML .= "</div><div class='header' style=\"clear:right;width:100%;display:inline;float:left; \">";
	}

	$strHTML .= "<div class='report' style='width:100%;display:inline;align:right;'>";

		$strHTML .="<div class='li-report-tbl-img' style=\"width:2%;display:inline;clear:left;\">";
		...
		if($aCol->has_attributes())
		{
			if(!getAttribute("hidden",$aCol->attributes())){
				...
				if($strDBname!="")
				{
					...
					if($xmlReport->levelOfSelect==1)
					{
						$strHTML .=	"<div onclick='report_sort(this);' class='li-report-tbl-header' style=\"padding-left:0px;width: ".$widthPerColumn.";\" tablename='".$colNum."' dbname='' align='left'>".$strHeader.$strImg."</div>";
					}
					else
					{
						...
						$strHTML .=	"<div onclick='report_sub_sort(this,\"".$xmlReport->levelOfSelect."\");' class='li-report-tbl-header' style=\"padding-left:0px;width: ".$widthPerColumn.";\" reload=\"".$parentDiv."\" tablename='".$colNum."' dbname='' align='left'>".$strHeader.$strImg."</div>";
					}
...
function create_summary_table_datalist($oRS,$xmlCols, &$xmlReport)
{
	...
	while (!$oRS->eof) 
	{
		...
		$strHTML .= "<div class='report' style='width=100%;display:inline;clear:left;'>";
		$strHTML .="<div class='li-report-tbl-img' style=\"width:2%;display:inline;clear:left;\">";
...
function output_trend_datalist(&$oRS, $arrXmlCols, $xmlReport,$strOrderByColumn = "", $strOrderByDir = "ASC")
{
	...
	$strHTML = "";
	$strHTML .= "<div id='testing' style=\"clear:right;\">";
...
function create_graph_data($strData,$strExistingKeys,$strValue,$strLabels,$strExistingValues){
	...
	{
		if (!isset($strInputHTML)) $strInputHTML="";
