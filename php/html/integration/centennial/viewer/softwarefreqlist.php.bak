...
if (!$query) $query = "SELECT DISTINCT defUsage.defFrequency, defUsage.ID FROM SoftwareRecognisedPrimary JOIN defUsage ON SoftwareRecognisedPrimary.Frequency = defUsage.ID WHERE SoftwareRecognisedPrimary.Client='".$compid."' ORDER BY defUsage.defFrequency";
