...
if (!$query) $query = "SELECT DISTINCT ProductType.Name, ProductType.ProductType FROM SoftwareRecognisedSecondary JOIN ProductType ON SoftwareRecognisedSecondary.ProductType = ProductType.ProductType WHERE SoftwareRecognisedSecondary.Client='".$compid."' ORDER BY ProductType.Name";
