...
// if the page is the Collected Files page, do a join on the Collected Date, otherwise don't.
if ($strPageTitle == "Collected Files")
{
	$query = "SELECT * FROM SoftwareInventory JOIN SoftwareFilePath ON SoftwareInventory.FilePathId = SoftwareFilePath.FilePathId JOIN SoftwareFile ON SoftwareFile.FileId = SoftwareInventory.FileId JOIN SoftwareInventoryStatus ON SoftwareInventory.ClientId = SoftwareInventoryStatus.ClientId WHERE SoftwareInventory.ClientId = ".$strCompID." AND SoftwareInventory.ModifiedDate = SoftwareInventoryStatus.LastUpdateDate";
}//end if
else
{
	$query = "SELECT * FROM SoftwareInventory JOIN SoftwareFilePath ON SoftwareInventory.FilePathId = SoftwareFilePath.FilePathId JOIN SoftwareFile ON SoftwareFile.FileId = SoftwareInventory.FileId WHERE SoftwareInventory.ClientId = ".$strCompID;
}
