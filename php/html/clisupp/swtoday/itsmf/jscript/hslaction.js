...
function _run_mycallstatus(strCommaStatus, strPfsAid, strDD, intLimit)
{
	...
	var strSQL = "select h_formattedcallref, callclass, status, priority, itsm_impact_level, itsm_urgency_level,cust_name, site, fixbyx, callref from opencall ";
    strSQL += "where callclass IN ('Incident','Problem','Known Error','Change Request','Service Request','Release Request') ";
	strSQL += "and status in("+ strCommaStatus + ") and owner='" + strPfsAid + "' and appcode='"+strDD+"' order by callref desc";
...
function _run_callstatus(strCommaStatus, strDD, intLimit)
{
    ...
    strSQL += "where callclass IN ('Incident','Problem','Known Error','Change Request','Service Request','Release Request') ";
	strSQL += "and status in("+ strCommaStatus + ") and appcode='"+strDD+"' order by callref desc";
...
function _run_mycallclass(strPfsAid, strDD, intLimit)
{
	...
	strSQL += "and status not in (18,17,16,15,6) and appcode='"+strDD+"' order by callref desc";
...
function _run_mycallslastatus(timecriteria, strDD, intLimit)
{
    ...
    strSQL += "where callclass IN ('Incident','Problem','Known Error','Change Request','Service Request','Release Request') ";
	strSQL += "and status not in (18,16,15,17,6) and appcode='"+strDD+"' and " + timecriteria + " order by callref asc";
...
function _run_mycallolastatus(timecriteria, strDD, intLimit)
{
	...
	strSQL += "and status not in (18,16,15,17) and appcode='"+strDD+"' and " + timecriteria + " order by callref asc";
...
function _run_mycallaged(timecriteria, strDD, intLimit)
{
	...
	var strSQL = "select callref, callclass, status, priority, itsm_impact_level, itsm_urgency_level,cust_name, site, respondbyx, fixbyx from opencall ";
    strSQL += "where callclass IN ('Incident','Problem','Known Error','Change Request','Service Request','Release Request') ";
	strSQL += "and status not in (18,16,15,17,6) and appcode='"+strDD+"' and " + timecriteria + " order by callref desc";
