<?php
	...
	class fc_my_requests_status
	{
		...
		function prepareXML()
		{
			$strCallClasses = "('Incident','Problem','Known Error','Change Request','Service Request','Release Request')";
			...
			$strSelectCustCalls = "select STATUS, count(status) as count from OPENCALL where callclass in ('Incident','Problem','Known Error','Change Request','Release Request','Service Request') and owner = '".$GLOBALS['analystid']."' and appcode IN(".gv("datasetfilterlist").")  group by status";
			$rsCustCalls = $swData->query($strSelectCustCalls,true);

			while(!$rsCustCalls->eof && $rsCustCalls)
			{
				//-- output
			...
			$strTemp = "";
			$strTemp .= "<graph caption='' BgColor='#FFFFFF'  xAxisName='' yAxisName='' showNames='1' numdivlines='0' decimalPrecision='0' showColumnShadow='0' bgAlpha='100' canvasBorderThickness='1' canvasBorderColor='AFD8F8' canvasBgAlpha='100' showLimits='0' formatNumberScale='0' showBorder='0'>";
		...
		function add_status_graph_xml_entry($strName, $intValue, $intStatusValue, $strColor, $strCallClasses)
		{
			...
			if(gv('_webclient')=="1")
			{
				if($intValue>0)$strXML="<set name='".$strName."' value='".$intValue."' color='".$strColor."' link='javascript:_run_fsc_action(&apos;mycallstatus&apos;,&apos;".$intStatusValue."&apos;,&apos;"._pfx($GLOBALS['analystid'])."&apos;,&apos;".removeQuotes(gv("datasetfilterlist"))."&apos;,100)'/>";
			}
			else
			{
				if($intValue>0)$strXML="<set name='".$strName."' value='".$intValue."' color='".$strColor."' link='hsl:sqlsearch?query=select h_formattedcallref, callref, callclass, status, priority, itsm_impact_level, itsm_urgency_level,cust_name, site, fixbyx from opencall where callclass IN ".$strCallClassesXML." and status=".pfs($intStatusValue)." and owner=&apos;".$GLOBALS['analystid']."&apos; and appcode IN(".gv("encoded_datasetfilterlist").") order by h_formattedcallref desc&amp;limit=100'/>";
			}		
