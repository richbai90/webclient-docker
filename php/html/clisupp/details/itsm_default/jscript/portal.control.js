...
function load_chart(strDivID, strType, strDataUrl, strPageXML)
{
	if(top.bWebClient) 
	{
		//-- if a parameter is already set on the URL then encode all params before passing
		//-- as only the first param is received otherwise to prevent injections
		var intQpos = strDataUrl.indexOf("?");
		if(intQpos>-1)
		{
			strUrl = strDataUrl.substring(0,intQpos);
			//-- encode the params - common.php deals with decoding them
			var strParams = strDataUrl.substring(intQpos+1,strDataUrl.length);
			strParams+="&_webclient=1";
			var strEncodedParams = "?ied=" + B64.encode(strParams);
			strUrl += strEncodedParams
			strDataUrl=strUrl;
		}
	...
	if(oDiv!=null)
	{
		if(FusionCharts.items[strUID]==undefined)
		{
			...
			myChart.setDataURL(pfu(strDataUrl));
		...
		else
		{
			...
			var myChart1 = FusionCharts(strUID);//.dispose();
			myChart1.dispose();
			...
			myChart1.setDataURL(pfu(strDataUrl));
...
//-- load a fusion chart into a given div
function load_chart_url(strDivID, strType, strDataUrl, strPageXML)
{
	if(top.bWebClient) 
	{
		//-- if a parameter is already set on the URL then encode all params before passing
	...
	//-- get body with and height
	if(strPageXML==undefined)strPageXML = "np1_";
	var strUID = strPageXML+"_"+strDivID;
	//-- get body with and height
	var oDiv = document.getElementById(strDivID);
	if(oDiv!=null)
	{
		var myChart = FusionCharts(strUID);
		if(!myChart)
		{
			myChart = new FusionCharts("../../fce/"+strType+".swf", strUID, oDiv.offsetWidth, oDiv.offsetHeight);
		}else
		{
			//-- Else the object is set to set the new width that will be beign called by resize_charts()
			myChart.width = oDiv.clientWidth;
		}
		myChart.autoInstallRedirect = false; //-- switch off alert box
		var strChartXML = run_php(strDataUrl,true);
