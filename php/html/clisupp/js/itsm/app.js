...
function OnAssignCall(strCallRefs, strGroup, strAnalyst)
{
	...
	p.crs = strCallRefs;
	p.cdb=1;
	var oRS = g.get_sqrecordset("helpdesk/get.callinfo.basic",p);
	while(oRS.Fetch())
	{
		strCallclass = g.get_field(oRS,"callclass");
		strOrigSupportGroup = g.get_field(oRS,"suppgroup");
		strOwner = g.get_field(oRS,"owner");
		if(strCallclass==="OLA Task")
		{
			if(strOrigSupportGroup!==strGroup)
			{
				alert('Cannot reassign OLA Task to another Support Group.');
				return false;
			}
		}
	}
...
{
	if(bThirdParty && !g.boolStandard)
	{
		var strParams = "priority="+pfu(nSlaId);
		var oRS = g.get_sqrecordset("form/tfcontract/get_contract",strParams);
		if(oRS.Fetch())
		{
			var strPK = g.get_field(oRS,"pk_contract_id");
...
function kb_search()
{
	if(!g.use_sw_knowledgebase())
	{
		g.open_third_party_kb_search();
		return false;
	}else{
		g.search_for("swkb", false, "searchmode=0");
		return "";
	}
	return true;
}
...
function kb_newdoc()
{
	if(!g.use_sw_knowledgebase())
	{
		g.open_third_party_kb_add_form();
		return false;
	}
...
function kb_extdoc()
{
	if(!g.use_sw_knowledgebase())
	{
		alert("This option is not available with your Knowledgebase.");
		return false;
	}
...
function kb_managecats()
{
	...
	if(!session.HaveRight(ANALYST_RIGHT_D_GROUP, ANALYST_RIGHT_D_CANMANAGEKBCATALOGS,true)) return false;
...
function OnOpenKBArticleProperties(strDoc)
{
	...
	var p = {};
	p.docref=strDoc
	var oRS = g.get_tablerecordset_bycol("swkb_articles",p,false,"flg_indexed");
	...
	else
	{
		...
		intIndexed = oKB.copy_syskbdoc_to_swdata(strDoc);
...
function OnDeleteAnalyst(szAnalystID, bIsThirdPartySupplier) 
{
	...
	{
		return g.sqlexecute_delete("SUPPLIER",szAnalystID);
	}
	return true;
}
...
function OnDeleteSLA(iSlaId, bIsThirdPartySLA)
{
	if(bIsThirdPartySLA)
	{
		return g.delete_tablebycol("CONTRACT","FK_PRIORITY",iSlaId);
	}
	return true;
}
...
function OnNewCallLogged(nCallRef, strCallClass)
{
	...
	if(g.bDoNotShowLogCallConfirmation)
	{
		if(g.bPromptToOpenCallDetails)
		{
			if(confirm("Would you like to open the detail record for this " + strCallClass))
			{
				global.OpenCallDetailsView(nCallRef);
			}
		}
		g.bDoNotShowLogCallConfirmation = false;
		g.bPromptToOpenCallDetails = true;
		return false;
	}
...
function PCMGetOldSlaSqlStatement(strColumn1, iITSMSlaId)
{
	...
	var oRS = g.get_tablerecordset_bycol("itsmsp_slad_priority",p,false,"fk_priority");
	while(oRS.Fetch())
	{
		strPriority = g.get_field(oRS,"fk_priority");
		if(strNames!=="")
			strNames+=",";
		strNames+="'"+g.pfs(strPriority)+"'";
...
function WebClientOverride_OpenFormForEdit(strFormName,varRecordKey,strParams,boolModal,openfromWin,fCallback)
{
    if(strFormName==="cmdb_popup_ci")
    {
        ...
        {
			//-- open for insert
            return cmdb.create_configitem("",true);
        }
        else
		{
	        //-- opened for edit
            return cmdb.popup_configitem(varRecordKey,true,"",true);
        }
	}
...
function kb_addarticletocall(strParams)
{
	...
	if(xmlmc.Invoke("data", "addRecord"))
	{
		...
		if(!objRes.success)
		{
			if(!g.rakb_record_in_sw(arrSplitParams.id))
			{
				alert("Failed to process record creation for Knowledgebase Article (" + objRes.message + "). Please contact your Supportworks Supervisor.");
			}
		}	
		else
		{
	...
	else
	{
		if(!g.rakb_record_in_sw(arrSplitParams.id))
		{
			alert("Failed to process record creation for Knowledgebase Article. " + xmlmc.GetLastError() + ". Please contact your Supportworks Supervisor.");
		}
	}		
	
	if(arrSplitParams.callref>0)
	{
		g.relate_call_to_rakb(arrSplitParams.id,arrSplitParams.callref);
	
		//-- populate existing articles refs against call into array filter
		var strExistingDocs = g.sl_get_valuesdel(app.array_openkbdocs[arrSplitParams.callkbidentifier].extform.sl_rakb,0,",","'");
	...
	app.array_openkbdocs[arrSplitParams.callkbidentifier].extform.sl_rakb.filter = "docref IN (" +this.array_strRAKbDocFilter[arrSplitParams.callkbidentifier]+ ")";
}
...
function reset_worfklow_ui()
{
	...
	{
		var strURL = "https://" + session.server + ":" + session.httpPort+"/sw/clisupp/_bpmui/"+ session.dataDictionary +"/index-left.php?sessid="+session.sessionId;
		OpenEmbeddedURL(strURL,true,true);    

		var strURL = "https://" + session.server + ":" + session.httpPort+"/sw/clisupp/_bpmui/"+ session.dataDictionary +"/splash.php";
		OpenEmbeddedURL(strURL,true,false);    
	}else
	{
		var strURL = "http://" + session.server + ":" + session.httpPort+"/sw/clisupp/_bpmui/"+ session.dataDictionary +"/index-left.php?sessid="+session.sessionId;
		OpenEmbeddedURL(strURL,true,true);    

		var strURL = "http://" + session.server + ":" + session.httpPort+"/sw/clisupp/_bpmui/"+ session.dataDictionary +"/splash.php";
		OpenEmbeddedURL(strURL,true,false);   
	}
...
function load_workflow_ui(strParams)
{
	...
	if(session.httpPort === "443")
	{
		var strURL = "https://" + session.server + ":" + session.httpPort+"/sw/clisupp/_bpmui/"+ session.dataDictionary +"/index.php?sessid="+session.sessionId+"&wid=" + encodeURIComponent(arrRecParams.wid)+"&callclass=" + encodeURIComponent(arrRecParams.callclass)+"&selectcell=" + encodeURIComponent(arrRecParams.selectcell);

	}else
	{
		var strURL = "http://" + session.server + ":" + session.httpPort+"/sw/clisupp/_bpmui/"+ session.dataDictionary +"/index.php?sessid="+session.sessionId+"&wid=" + encodeURIComponent(arrRecParams.wid)+"&callclass=" + encodeURIComponent(arrRecParams.callclass)+"&selectcell=" + encodeURIComponent(arrRecParams.selectcell);
	}
	OpenEmbeddedURL(strURL,true,false);    
}
...
function bpmui_edit_workflow(strParams)
{
	...
	{
		if(g.SavedWorkflowRecord)
		{
			g.SavedWorkflowRecord=false;
...
{
	var colourSchema = g.get_CurrentSkinColorSchema();
	var result = OpenHtmlWindow("bpm_util", "frame", "&[app.webroot]/clisupp/_bpmutil/"+ session.dataDictionary +"/index.php?sessid="+session.sessionId+"&colourscheme="+colourSchema,"BPM Utility",true,1024,768);
}
...
{		
	if(session.httpPort === "443")
	{
		var strURL = "https://" + session.server + ":" + session.httpPort+"/sw/clisupp/fsc/index.php?sessid="+session.sessionId;

	}
	else
	{
		var strURL = "http://" + session.server + ":" + session.httpPort+"/sw/clisupp/fsc/index.php?sessid="+session.sessionId;
	}
	OpenEmbeddedURL(strURL,true,false);    
}
