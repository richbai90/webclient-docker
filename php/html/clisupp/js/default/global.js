...
function cfg_process_call_contract_check(strAssetID, boolLogging,oDoc)
{
	...
	var strSLAS = "'" + g.pfs(oDoc.opencall.priority)+ "'";
	var strSelect = "select FK_PRIORITY, FK_MANAGER, PK_CONTRACT_ID, FLG_VALID, FLG_ALWAYSSUPPORT, ASSET_TITLE from CONTRACT, EQUIPMNT where FK_CONTRACT_ID = PK_CONTRACT_ID and EQUIPID = '" + g.pfs(strAssetID) + "'";
	
	var oRS = g.get_recordset(strSelect,"swdata");
	if(oRS.Fetch())
	{
	
		var strAssetDesc = g.get_field(oRS,"ASSET_TITLE");
		var strValid = g.get_field(oRS,"FLG_VALID");
		var strAlwaysOn = g.get_field(oRS,"FLG_ALWAYSSUPPORT");
		var boolSupport = ((strValid=="1") || (strAlwaysOn=="1"));
		
		//-- store sla & contract
		var currSLA = g.get_field(oRS,"FK_PRIORITY");
		array_slacontracts[currSLA] = g.get_field(oRS,"PK_CONTRACT_ID");
	...
	if(strInvalidItems!="")
	{
		var oAnalyst = g.get_analyst_detail(g.get_field(oRS,"FK_MANAGER"));
		var strAnalystMsg = (oAnalyst)? "(" + oAnalyst.id + ") " + oAnalyst.name:g.get_field(oRS,"FK_MANAGER")
		alert(strInvalidItems + " does not have a valid support contract (" + g.get_field(oRS,"PK_CONTRACT_ID")+ ") please process as required. The contract is managed by "+ strAnalystMsg);
	}
	...
	if((strBestSLA!=oDoc.opencall.priority)&&(strBestSLA!=""))
	{
		if(confirm("This request currently has a longer overall response and fix time than the best contracted SLA. Would you like to change the SLA?"))
		{
			...
			{
				if(g.new_dairyevent(oDoc.opencall.callref,"The priority on this request was changed due to a contracted asset change.","Asset Change","Priority Update (contract)",strBestSLA))
				{
					oDoc.opencall.fk_contract_id = array_slacontracts[strBestSLA];
					oDoc.Save(0);
...
function itsm_addAttachmentsToCallXmlmc(fl_attachslist, xmlmc)
{
	...
	for (var i=0; i<fl_attachslist.rowCount; i++)
	{
		...
		if (serverFile < 0)
		{
			...
			var strData = global.LoadFileInBase64(fl_attachslist.GetItemText(i,3));
...
{
	var strSLA = "";
	var strSelect = "select fk_sla from itsm_slamatrix where fk_impact = '" + g.pfs(strImpact) + "' and fk_urgency = '" + g.pfs(strUrgency) + "'";
	var oRS = g.get_recordset(strSelect,"swdata");
	if(oRS.Fetch())
	{
...
{
	return 	session.HaveAppRight(PG_RFC, RFC_AUTHCANCEL,session.dataDictionary);;
}
...
function pt_resolve_unbound_record(strTable,strResolveCol,strResolveValue,strFieldPrefix, aDoc, boolAutoResolve, boolMSG)
{
	...
	var strWhere = strResolveCol + strOp + g.encapsulate(strTable,strResolveCol,strResolveValue);
	
	//-- get record count
	var intCount = g.get_rowcount(strTable,strWhere);
	if(intCount==0)
	{
		//-- no matches
		if(boolMSG)alert("No matching record was found in table (" + strTable +") where (" + strWhere + ")");
	}
	else if(intCount==1)
	{
		//-- resolve
		oRec = g.get_recordwhere(strTable, strWhere);
		if(oRec)
		{
	...
	else if(intCount>1)
	{
		...
		if(boolMSG) alert("More than one matching record was found. This indicates that there is a data issue in table (" + strTable +") where (" + strWhere + ")");
...
function fglobal_get_shortest_sla(strSLAS)
{
	...
	var oRS = g.get_recordset(strSelect,"syscache");
	while(oRS.Fetch())
	{
...
function fglobal_search_for(strSearchType, boolMulti,strAppendURL)
{
	...
	var aForm = g.popup("search_" + strSearchType,strURL);	
...
{
	var strSelect = "select fullname from userdb where keysearch = '" + g.pfs(strMgrKeysearch) + "'";
	var oRS = g.get_recordset(strSelect,"swdata");
	if(oRS.Fetch())
	{
...
function fglobal_relate_calls(intCallrefM,intCallrefS,strCode)
{
	...
	return g.submitsql(strSQL,true);
}
...
function fglobal_sl_load_call_slaves(oSqlList, intCallrefM , strCode)
{
	...
	oSqlList.filter = "callref in(" + strFilter + ")";
	//Sleep(150);
	//oSqlList.Refresh();
...
function fglobal_submit_php(strURL,boolSWsession)
{
	...
	SW_WEBSERVER = g.get_webserver()+"/sw/clisupp/";
    ...
    for (var x = 0; x<oForm.elements.length;x++)
    {
        ...
        if ((element.dataRef != "") && (element.type != FE_SEARCHFIELD))
        {
			...
			if(strTestTable.toLowerCase() == strTable.toLowerCase())
			{
				...
				//alert(aRec[strColName.toLowerCase()])
...
function fglobal_sqllist_appendfilter(oList,strAppendFilter,intKeyCol)
{
	...
	oList.filter = UC(strColName) + " in (" + strFilter + ")";
	//alert(oList.filter)
	//Sleep(10);
...
{
	var result	= SqlExecute("swdata",strSQL);
    if (result==-1)
    {
        //-- an error occured
...
{
	return global.PrepareForSQL(inValue);
}
...
{
	var oCall = g.get_record("OPENCALL", strValue);
...
function fglobal_callref_unpad(strValue)
{
	...
	var oCall = g.get_recordwhere("OPENCALL","h_formattedcallref = '"+strValue+"'");
...
function fglobal_pick_analystorgroup(strTitle,funcCallback)
{
	...
	if(dlg.Open(strTitle,function()
	{
		...
		var oResult = g.get_analystorgroup(strGroup,strAnalystID);
...
function fglobal_get_analystorgroup(strGroup,strAnalystID)
{
	...
	if(strAnalystID!="")
	{
		...
		tmpobject.oGroup =g.get_group_detail(strGroup);
 	}
	else if(strGroup!="")
	{
		tmpobject.oAnalyst =g.get_blank_analyst();
		tmpobject.oGroup =g.get_group_detail(strGroup);
	}
...
function fglobal_pick_analyst(strTitle,funcCallback)
{
	...
	if(dlg.Open(strTitle,function()
	{
		...
		strGroup = dlg.groupid;
		strAnalystID = dlg.analystid;
		var oResult = g.get_analyst_detail(strAnalystID);
		funcCallback(oResult);
	}));
}
...
function fglobal_get_analyst_detail(strAnalystID)
{
	...
	var result	= false;
	var strSQL  	= "select analystid,name,supportgroup,contactb,contacte,priveligelevel from swanalysts where analystid = '" + g.pfs(strAnalystID) + "'";
	var strDB	= "syscache";
	var oRS		= this.get_recordset(strSQL,strDB);
	while (oRS.Fetch())
	{	
...
function fglobal_get_group_analystids(strGroupID)
{
	...
	//-- call generic php to get analyst info (we use php because we have no way of connecting to systemdb)
	var strSQL  	= "select analystid from swanalysts where class not in (0,2) and supportgroup = '" + g.pfs(strGroupID) + "'";
	var strDB	= "syscache";
	var oRS		= this.get_recordset(strSQL,strDB);
	while (oRS.Fetch())
	{	
...
function fglobal_get_group_detail(strGroupID)
{
	...
	var strSQL  = "select id,name,attrib1,attrib2,attrib3,attrib4,attrib5,attrib6,notes from swgroups where id = '" + g.pfs(strGroupID) + "'";
	var strDB = "syscache";
	var oRS  = this.get_recordset(strSQL,strDB);
	while (oRS.Fetch())
	{ 
