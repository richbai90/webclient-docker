...
{
	if(!session.HaveRight(ANALYST_RIGHT_D_GROUP, ANALYST_RIGHT_D_CANEDITKBDOCUMENTS,true))
	{
	...
	else
	{
		...
		var ap = {};
		ap.DOCREF = strDoc;
		var oRS = g.get_tablerecordset_bycol("swkb_articles",ap,false,"FLG_INDEXED"); 	
		if(oRS.Fetch())
		{
			intIndexed=  g.get_field(oRS,"FLG_INDEXED");
		}
		else
		{
			//-- do document (either upgraded or a default document) so create record
...
oSwKb.prototype.relate_articles = function(strParentDoc, strChildDocs)
{
	...
	for(var x=0;x<arrChildDocs.length;x++)
	{
		if(strParentDoc!=arrChildDocs[x])
		{
			...
			if(g.sqs_rowcountbycol("SWKB_RELATED",arrParams)==0)
			{
				//-- insert into swdb
				//-- nwj - 11.2012 - use sqs instead
				var strParams = "pd=" + pfu(strParentDoc) + "&cd=" + pfu(arrChildDocs[x]);
				g.submitsqs("knowledgebase/relate_articles.insert",strParams);
			}
		}
		else
		{
			alert("You have selected to relate this document to itself. This selection will be omitted.");
		}
	}
}
...
oSwKb.prototype.create_articleupdateentry = function(strDocRef, strText)
{
		...
		g.submitsqs("knowledgebase/create_articleupdateentry.insert",strParams);
}
...
oSwKb.prototype.unrelate_articles = function(strParentDoc, strChildDocs)
{
	...
	g.submitsqs("knowledgebase/unrelate_articles.delete",strParams);
}
...
oSwKb.prototype.get_related_article_refs = function (strDocRef, boolPFS)
{
	...
	var ap = {};
	ap.DOCREF = strDocRef;
	var oRS = g.get_tablerecordset_bycol("SWKB_RELATED",ap,false,"RELATEDDOCREF"); 	
	...
	while(oRS.Fetch())
	{
		...
		var strVal = g.get_field(oRS,"RELATEDDOCREF");
	...
	var oRS = g.get_tablerecordset_bycol("SWKB_RELATED",ap,false,"DOCREF"); 
	...
	while(oRS.Fetch())
	{
		...
		var strVal = g.get_field(oRS,"DOCREF");
		strReturnString += strVal;
	}
...
//-- copy a sys kb document over to swkb
oSwKb.prototype.copy_syskbdoc_to_swdata = function (strDocRef)
{
	...
	if(xmlmc.Invoke("knowledgebase", "documentGetInfo"))
	{
		...
		if(!objRes.success)
		{
			alert("Failed to load knowledgebase document (" + objRes.message + "). Please contact your Supportworks Supervisor.");
			return -1;
		}       
		else
		{
			...
			{  
				...
				else if(strColName=="catalogId")
				{
					...
					//-- get catalog name
					var oRec = g.get_record("SWKB_CATALOGS", varValue);
					if(oRec)
					{
				...
				else if(strColName=="docDate" || strColName=="lastmodifieddate" || strColName==="sourceDate" || strColName==="expiryDate")
				{
					...
					varValue = g.convert_isodate_epoch(varValue);
			...
			//	strValues += g.encapsulate("swkb_articles",strColName,varValue);
			...
			if(!g.submittableinsert("SWKB_ARTICLES",arrCols))
				return -1;
		}
	}
	else
...
oSwKb.prototype.relate_ci_to_kb = function (fk_ci_id,docref)
{
	...
	return g.submitsqs("knowledgebase/relate_ci_to_kb.insert",strParams);
}
...
oSwKb.prototype.get_cis_kbs = function (fk_ci_id)
{
	...
	var oRS = g.get_tablerecordset_bycol("CONFIG_RELKB",ap,false,"KB_DOCREF"); 

	//-- nwj we are returning values for filter so do not pfs or encaps as sqs filter script will take care of that.
	while(oRS.Fetch())
	{
		currKey = g.get_field(oRS,"KB_DOCREF");
...
oSwKb.prototype.get_srs_kbs_portfolio = function (fk_ci_id, strFolio)
{
	...
	var oRS = g.get_tablerecordset_bycol("SWKB_ARTICLES",ap,false,"docref"); 

	//-- nwj we are returning values for filter so do not pfs or encaps as sqs filter script will take care of that.
	while(oRS.Fetch())
	{
		currKey = g.get_field(oRS,"docref");
		if (strKBDocRefs!="")strKBDocRefs +=",";
		strKBDocRefs += currKey;
	}
	if(strKBDocRefs=="")strKBDocRefs="-1";
...
oSwKb.prototype.unrelate_ci_from_kb = function (strKBdocref,intCIkeys)
{
	...
	return g.submitsqs("knowledgebase/unrelate_ci_from_kb.delete",strParams);
}
...
oSwKb.prototype.get_kb_cis = function (kb_docref)
{
	...
	var oRS = g.get_tablerecordset_bycol("CONFIG_RELKB",ap,false,"FK_CI_ID"); 


	while(oRS.Fetch())
	{
		currKey = g.get_field(oRS,"FK_CI_ID");
...
oSwKb.prototype.relate_ci_type_to_kb = function (fk_config_type,type_display,fk_docref)
{
	...
	return g.submitsqs("knowledgebase/relate_ci_type_to_kb.insert",strParams);
...
oSwKb.prototype.unrelate_ci_type_from_kb = function (strKBdocref,strCITypes)
{
	...
	return g.submitsqs("knowledgebase/unrelate_ci_type_from_kb.delete",strParams);
...
oSwKb.prototype.get_kb_ci_types = function (kb_docref)
{
	...
	var ap = {};
	ap.KB_DOCREF = kb_docref;
	var oRS = g.get_tablerecordset_bycol("CI_TYPE_RELKB",ap,false,"FK_CONFIG_TYPE"); 

	//-- nwj we are returning values for filter so do not pfs or encaps as sqs filter script will take care of that.
	while(oRS.Fetch())
	{
		currKey = g.get_field(oRS,"FK_CONFIG_TYPE");
		if (strCIIds!="")strCIIds +=",";
		strCIIds += currKey;
	}
	if(strCIIds=="")strCIIds="-1";
...
oSwKb.prototype.get_ci_types_kbs = function (fk_config_type)
{
	...
	var oRS = g.get_tablerecordset_bycol("CI_TYPE_RELKB",ap,false,"KB_DOCREF"); 
...
oSwKb.prototype.unrelate_calls_from_kb = function (strKBdocref,strCallrefs)
{
	...
	return g.submitsqs("knowledgebase/unrelate_calls_from_kb.delete",strParams);
...
oSwKb.prototype.relate_call_to_kb = function (strKBdocref,intCallRef,strRelCode)
{
	...
	return g.submitsqs("knowledgebase/relate_call_to_kb.insert",strParams);
...
oSwKb.prototype.get_kb_calls = function (kb_docref, strCode)
{
	...
	ap.relcode = strCode;
	var oRS = g.get_tablerecordset_bycol("cmn_rel_opencall_kb",ap,false,"fk_callref"); 

	//-- nwj we are returning values for filter so do not pfs or encaps as sqs filter script will take care of that.
	while(oRS.Fetch())
	{
		currKey = g.get_field(oRS,"fk_callref");
...
oSwKb.prototype.get_call_kbs = function (intCallref)
{
	...
	var oRS = g.get_tablerecordset_bycol("cmn_rel_opencall_kb",ap,false,"fk_kbdoc"); 

	//-- nwj we are returning values for filter so do not pfs or encaps as sqs filter script will take care of that.
	while(oRS.Fetch())
	{
		currKey = g.get_field(oRS,"fk_kbdoc");
...
{
	var addressRec = g.get_form_recorddata("address",int_address_id,aForm,true,"",false);
	if (addressRec)
	{
...
function cfg_process_call_contract_check(strCIIDs, boolLogging,oDoc)
{
	...
	var oRS = g.get_tablerecordset_bycol("CONFIG_RELI",ap); 

	//-- nwj we are returning values for filter so do not pfs or encaps as sqs filter script will take care of that.
	while(oRS.Fetch())
	{
		if(strContractIDs!="")strContractIDs +=",";
		strContractIDs +=  g.get_field(oRS,"FK_CHILD_ID");
	}
	...
	var oRS = g.get_sqrecordset("general/process_call_contract_check.select",oParams);
	//-- F0075922 
	var array_badcis = [];
	while(oRS.Fetch())
	{
	
		var strItemText = g.get_field(oRS,"FK_CHILD_ITEMTEXT");

		var strExpires = g.get_field(oRS,"EXPIRESONX");
		...
		var strAlwaysOn = g.get_field(oRS,"FLG_ALWAYSSUPPORT");
		var boolSupport = (strValid=="1");
		
		//-- store sla & contract
		var currSLA = g.get_field(oRS,"FK_SSLA");
		array_slacontracts[currSLA] = g.get_field(oRS,"PK_SLAD_ID");

		//-- if not support record so we can alert
		if(!boolSupport)
		{
			//-- F0075922 
			array_badcis[g.get_field(oRS,"FK_CHILD_ID")] = strItemText + " in SLA '" + g.get_field(oRS,"SLAD_ID") + "' managed by "+g.get_field(oRS,"FK_AID");

		}
		else
		{
			if(array_badcis[g.get_field(oRS,"FK_CHILD_ID")]!=undefined)
			{

				delete(array_badcis[g.get_field(oRS,"FK_CHILD_ID")]);
			}
	...
	if(strInvalidItems!="")
	{
		alert("The following items do not have valid support agreements please process as required :\n\n" + strInvalidItems)
	}

	//-- get the shortest sla i.e. least overall time
	var strBestSLA = g.get_shortest_sla(strSLAS);
	if((strBestSLA!=oDoc.opencall.priority)&&(strBestSLA!=""))
	{
		if(confirm("This request currently has a longer overall response and fix time than the best contracted Priority. Would you like to change the Priority?"))
		{
			...
			{
				if(g.new_dairyevent(oDoc.opencall.callref,"The Priority on this request was changed from (" + oDoc.opencall.priority + ") to (" + strBestSLA + ") due to configuration item SLA associatation","CI Change","Priority Update (SLA)",strBestSLA))
				{
					oDoc.opencall.fk_contract_id = array_slacontracts[strBestSLA];
					oDoc.Save(0);
...
//-- add call and its document to array
...
{
	var oRec = g.get_record("COMPANY", strCompanyID);
	if(!oRec)return "";
	...
	var strKeyValues = "";
	var oRS = g.get_sqrecordset("cmdb/get_ci_site.select",strParams );

	//-- nwj we are returning values for filter so do not pfs or encaps as sqs filter script will take care of that.
	while(oRS.Fetch())
	{
		if(strKeyValues!="")strKeyValues+=",";	
		strKeyValues += g.get_field(oRS,strGetCol);
	}	
	...
	if(boolIncludeOwnedCIs)
	{
		...
		var oRS = g.get_tablerecordset_bycol("CONFIG_ITEMI",ap,false,"PK_AUTO_ID");

		while(oRS.Fetch())
		{
			if(strKeyValues!="")strKeyValues+=",";
			strKeyValues += g.get_field(oRS,"PK_AUTO_ID");
		}	
	}
...
{
	var oRec = g.get_record("SUPPLIER", strCompanyID);
	if(!oRec)return "";
...
{
	return g.get_contract_cikeys(strContract,boolGetChildren);
...
{
	var strKeyValues = "";
	var oRS = g.get_sqrecordset("general/get_ci_contractkeys.select","cids=" +inCMDBID );

	//-- nwj we are returning values for filter so do not pfs or encaps as sqs filter script will take care of that.
	while(oRS.Fetch())
	{
		if(strKeyValues!="")strKeyValues+=",";	
		var strVal = g.get_field(oRS,"FK_PARENT_ITEMTEXT");
		strKeyValues += strVal;
	}	
...
function cfg_get_company_relatedkeys(strCompanyID,strChildType, strChildTable, boolIncludeMECIs,boolEncaps,boolPFS)
{
	...
	var strChildPrimaryKeyCol = UC(g.dd_primarykey(strChildTable)); 

	//-- get direct link - assumes child table has column called fk_company_id
	var ap = {};
	ap.fk_company_id = strCompanyID;
	var oRS = g.get_tablerecordset_bycol(strChildTable,ap,false,strChildPrimaryKeyCol);
	...
	while(oRS.Fetch())
	{
		...
		var strValue = g.get_field(oRS,strChildPrimaryKeyCol);
	...
	if(boolIncludeMECIs)
	{
		...
		var oRS = g.get_distincttablerecordset_bycol("CONFIG_RELI",ap,false,"FK_CHILD_ITEMTEXT");

		while(oRS.Fetch())
		{
			if(strKeyValues!="")strKeyValues+=",";

			var strValue = g.get_field(oRS,"FK_CHILD_ITEMTEXT");
			strKeyValues += strValue;
		}	
	}
...
function cfg_get_customer_sites(strCustID, boolPFS, strE, intCMDBID)
{
	...
	{
		var oRec = g.get_record("USERDB", strCustID);
		if(!oRec)return "";
	...
	var oRS = g.get_distincttablerecordset_bycol("CONFIG_RELI",ap,false,"FK_PARENT_ITEMTEXT");
	...
	while(oRS.Fetch())
	{
		...
		var strValue = g.get_field(oRS,"FK_PARENT_ITEMTEXT");
		strKeyValues += strValue;
	}	
	if(strKeyValues=="")strKeyValues="-1";
	return strKeyValues;
...
function cfg_get_site_customers(strSiteID, boolPFS, strE, intCMDBID)
{
	if((intCMDBID==undefined)||(intCMDBID<1))
	{
		var oRec = g.get_record("SITE", strSiteID);
		if(!oRec)return "";
		intCMDBID = oRec.fk_cmdb_id;
	}

	var	strKeyValues = "";
	var oRS = g.get_sqrecordset("general/get_ci_customers.select","cid="+intCMDBID);
	...
	while(oRS.Fetch())
	{
		...
		var strValue = g.get_field(oRS,"KEYSEARCH");
		strKeyValues += strValue;
	}	
	if(strKeyValues=="")strKeyValues="-1";
	return strKeyValues;
...
function cfg_get_dept_customers(strDeptID, boolPFS, strE, intCMDBID)
{
	if((intCMDBID==undefined)||(intCMDBID<1))
	{
		var oRec = g.get_record("DEPARTMENT", strDeptID);
		if(!oRec)return "";
		intCMDBID = oRec.fk_cmdb_id;
	}
...
function itsm_get_sla_frommatrix(strImpact, strUrgency)
{
	...
	var oRS = g.get_tablerecordset_bycol("itsm_slamatrix",ap,false,"fk_sla");
	if(oRS.Fetch())
	{
		strSLA = g.get_field(oRS,"fk_sla");
	}
	return strSLA;
}
...
function itsm_get_sla_from_slad_matrix(intSLAD, strImpact, strUrgency, intSLA)
{
	...
	ap.fk_urgency = strUrgency
	ap.flg_sla = intSLA;
	var oRS = g.get_tablerecordset_bycol("itsmsp_slad_matrix",ap,false,"fk_priority");

	if(oRS.Fetch())
	{
		strSLA = g.get_field(oRS,"fk_priority");
	}
	return strSLA;
}
...
function itsm_search_calls(strType, strAppendURL, boolMulti, funcCallback)
{
	...
	g.popup("search_calls",strURL,function(aForm)
	{
...
function itsm_can_alter_ila()
{
	...
	boolGranted = session.HaveAppRight(PG_RFC, RFC_CDF_ILA_ALTER, session.dataDictionary);
	return boolGranted;
}
...
{
	var boolGranted = false;
	boolGranted = session.HaveAppRight(PG_REL, REL_CDF_ILA_ALTER , session.dataDictionary);
	return boolGranted;
}
...
{
	var boolGranted = false;
	boolGranted = session.HaveAppRight(PG_RFC, RFC_COORDINATOR, session.dataDictionary);
	return boolGranted;
}
...
{
	var boolGranted = false;
	boolGranted = session.HaveAppRight(PG_RFC, RFC_MANAGER, session.dataDictionary);
	return boolGranted;
}
...
function itsm_can_edit(strCallClass,boolMSG)
{
	...
	{
		case "incident":
			boolEdit = session.HaveAppRight(PG_INC, INC_EDIT,session.dataDictionary);
			break;
		case "problem":
			boolEdit = session.HaveAppRight(PG_PRB, PRB_EDIT,session.dataDictionary);
			break;
		case "known error":
			boolEdit = session.HaveAppRight(PG_PRB, KE_EDIT,session.dataDictionary);
			break;
		case "change request":
			boolEdit = session.HaveAppRight(PG_RFC, RFC_EDIT,session.dataDictionary);
			break;
		case "release request":
			boolEdit = session.HaveAppRight(PG_REL, REL_EDIT,session.dataDictionary);
			break;
		case "service request":
			boolEdit = session.HaveAppRight(PG_SC, SC_EDIT_CALL,session.dataDictionary);
			break;
		case "ola task":
			boolEdit = session.HaveAppRight(PG_INC, INC_OLA_EDIT,session.dataDictionary);
			break;
		case "change proposal":
			boolEdit = session.HaveAppRight(PG_RFC, CPR_EDIT,session.dataDictionary);
			break;
	}

	if((!boolEdit)&&(boolMSG))
	{
		alert("You are not authorised to modify " + strCallClass + " records.\nIf you require authorisation please contact your Supportworks Administrator.");
	}
...
function itsm_can_create(strCallClass,boolMSG)
{
	...
	{
		case "incident":
			boolCreate = session.HaveAppRight(PG_INC, INC_CREATE,session.dataDictionary);
			break;
		case "problem":
			boolCreate = session.HaveAppRight(PG_PRB, PRB_CREATE,session.dataDictionary);
			break;
		case "known error":
			boolCreate = session.HaveAppRight(PG_PRB, KE_CREATE,session.dataDictionary);
			break;
		case "change request":
			boolCreate = session.HaveAppRight(PG_RFC, RFC_CREATE,session.dataDictionary);
			break;
		case "release request":
			boolCreate = session.HaveAppRight(PG_REL, REL_CREATE,session.dataDictionary);
			break;
		case "service request":
			boolCreate = session.HaveAppRight(PG_SC, SC_CREATE,session.dataDictionary);
			break;
		case "ola task":
			boolCreate = session.HaveAppRight(PG_INC, INC_OLA_CREATE,session.dataDictionary);
			break;
		case "change proposal":
			boolCreate = session.HaveAppRight(PG_RFC, CPR_CREATE,session.dataDictionary);
			break;
	}

	if((!boolCreate)&&(boolMSG))
	{
		alert("You are not authorised to create " + strCallClass + " records.\nIf you require authorisation please contact your Supportworks Administrator.");
	}
...
function itsm_can_cancel(strCallClass,boolMSG)
{
	...
	{
		case "incident":
			boolCancel = session.HaveAppRight(PG_INC, INC_CANCEL,session.dataDictionary);
			break;
		case "problem":
			boolCancel = session.HaveAppRight(PG_PRB, PRB_CANCEL,session.dataDictionary);
			break;
		case "known error":
			boolCancel = session.HaveAppRight(PG_PRB, KE_CANCEL,session.dataDictionary);
			break;
		case "change request":
			boolCancel = session.HaveAppRight(PG_RFC, RFC_CANCEL,session.dataDictionary);
			break;
		case "release request":
			boolCancel = session.HaveAppRight(PG_REL, REL_CANCEL,session.dataDictionary);
			break;
		case "service request":
			boolCancel = session.HaveAppRight(PG_SC, SC_CANCEL,session.dataDictionary);
			break;
		case "ola task":
			boolCancel = session.HaveAppRight(PG_INC, INC_OLA_CANCEL,session.dataDictionary);
			break;
		case "change proposal":
			boolCancel = session.HaveAppRight(PG_RFC, CPR_CANCEL,session.dataDictionary);
			break;
	}

	if((!boolCancel)&&(boolMSG))
	{
		alert("You are not authorised to cancel " + strCallClass + " records.\nIf you require authorisation please contact your Supportworks Administrator.");
	}
...
function itsm_can_resolve(strCallClass,boolMSG)
{
	...
	{
		case "incident":
			boolResolve = session.HaveAppRight(PG_INC, INC_RESOLVE,session.dataDictionary);
			break;
		case "problem":
			boolResolve = session.HaveAppRight(PG_PRB, PRB_RESOLVE,session.dataDictionary);
			break;
		case "known error":
			boolResolve = session.HaveAppRight(PG_PRB, KE_RESOLVE,session.dataDictionary);
			break;
		case "change request":
			boolResolve = session.HaveAppRight(PG_RFC, RFC_RESOLVE,session.dataDictionary);
			break;
		case "release request":
			boolResolve = session.HaveAppRight(PG_REL, REL_RESOLVE,session.dataDictionary);
			break;
		case "service request":
			boolResolve = session.HaveAppRight(PG_SC, SC_RESOLVE,session.dataDictionary);
			break;
		case "ola task":
			boolResolve = session.HaveAppRight(PG_INC, INC_OLA_RESOLVE,session.dataDictionary);
			break;
		case "change proposal":
			boolResolve = session.HaveAppRight(PG_RFC, CPR_RESOLVE,session.dataDictionary);
			break;
	}

	if((!boolResolve)&&(boolMSG))
	{
		alert("You are not authorised to resolve " + strCallClass + " records.\nIf you require authorisation please contact your Supportworks Administrator.");
	}
...
function itsm_can_view(strCallClass,boolMSG)
{
	...
	{
		case "incident":
			boolView = session.HaveAppRight(PG_INC, INC_VIEW,session.dataDictionary);
			break;
		case "problem":
			boolView = session.HaveAppRight(PG_PRB, PRB_VIEW,session.dataDictionary);
			break;
		case "known error":
			boolView = session.HaveAppRight(PG_PRB, KE_VIEW,session.dataDictionary);
			break;
		case "change request":
			boolView = session.HaveAppRight(PG_RFC, RFC_VIEW,session.dataDictionary);
			break;
		case "release request":
			boolView = session.HaveAppRight(PG_REL, REL_VIEW,session.dataDictionary);
			break;
		case "service request":
			boolView = session.HaveAppRight(PG_SC, SC_VIEW_CALL,session.dataDictionary);
			break;
		case "ola task":
			boolView = session.HaveAppRight(PG_INC, INC_OLA_VIEW,session.dataDictionary);
			break;
		case "change proposal":
			boolView = session.HaveAppRight(PG_RFC, CPR_VIEW,session.dataDictionary);
			break;
	}

	if((!boolView)&&(boolMSG))
	{
		alert("You are not authorised to view " + strCallClass + " records.\nIf you require authorisation please contact your Supportworks Administrator.");
	}
...
function itsm_get_problem_impact_customerids(intCallrefs,boolPFS)
{
	...
	var strLinkedImpactCIs = cmdb.get_call_cis(intCallrefs , "PROBLEM-AFFECT", "");
	var strCiImpactedCustomerIDS = cmdb.get_ci_customer_ids(strLinkedImpactCIs,boolPFS)
	...
	var intIncidentCallrefs = g.get_slave_calls(intCallrefs,"PROBLEM-INCIDENT",false);
	if(intIncidentCallrefs!="")intIncidentCallrefs+=",";
	intIncidentCallrefs+=intCallrefs;
	var strImpactedCustomerIDS = g.get_customers_on_calls(intIncidentCallrefs);
...
function itsm_get_problem_impact_orgids(intCallrefs,boolPFS)
{
	...
	var strLinkedImpactCIs = cmdb.get_call_cis(intCallrefs , "PROBLEM-AFFECT", "");
	var strCiImpactedCustomerIDS = cmdb.get_ci_company_ids(strLinkedImpactCIs,boolPFS)
	if(strCiImpactedCustomerIDS=="")strCiImpactedCustomerIDS="-1";
	...
	if(intIncidentCallrefs!="")intIncidentCallrefs+=",";
	intIncidentCallrefs+=intCallrefs;
	var strIncImpactedCustomerIDS = g.get_companies_on_calls(intIncidentCallrefs);
...
function itsm_get_rfc_impact_customerids(intCallrefs,boolPFS)
{
	...
	//-- then get customers that use the impacted cis
	var strLinkedImpactCIs = cmdb.get_call_cis(intCallrefs , "RFC-AFFECT", "");
	var strCiImpactedCustomerIDS = cmdb.get_ci_customer_ids(strLinkedImpactCIs,boolPFS)
	if(strCiImpactedCustomerIDS=="")strCiImpactedCustomerIDS="-1";

	//-- get incs linked to rfc
	//-- then get customers linked to incidents and this problem
	var intIncidentCallrefs = g.get_slave_calls(intCallrefs,"RFC-INCIDENT",false);
	if(intIncidentCallrefs!="")intIncidentCallrefs+=",";
	intIncidentCallrefs+=intCallrefs;
	var strImpactedCustomerIDS = g.get_customers_on_calls(intIncidentCallrefs);
	if(strImpactedCustomerIDS=="")strImpactedCustomerIDS="-1";
	...
	var intProblemCallrefs = g.get_slave_calls(intCallrefs,"RFC-ERROR",false);
...
oCiForm.prototype.OnSaveData = function()
{
	...
	if(cmdb.process_form_ciattribute_regexcheck(this.doc)==false) return false;

	//-- when inserting check that ck_config_item and ck_config_type is unique
	if(this.doc.boolInsertingCI)
	{
		if (!cmdb.can_create(true))return false;
	
		if(cmdb.ci_exists(this.oCI().ck_config_item,this.oCI().ck_config_type))
		{
			//-- this ci id and type already exists warn user and cancel save
			alert("The " + this.oCI().ck_config_type + " item " + this.oCI().ck_config_item + " already exists in the CMDB, a duplicate cannot be created");
			return false;
		}
	}
	else
	{
		...
		if(!cmdb.can_update(true)) return false;
		
		//-- if not baselining and not current then warn user
		if(!this.doc.boolBaselining)
		{
			if((this.oCI().isactivebaseline!="Yes") && (cmdb.can_baseline(true)))
			{
...
oCiForm.prototype.OnDataSaved = function()
{
	...
	if(objCIFormDoc.doc.boolInsertingCI)
	{
		...
		if(objCIFormDoc.oCI().fk_site != "")    		
		{
			...
			var recSite = g.get_record("site", objCIFormDoc.oCI().fk_site)
			cmdb.generate_meconfigrelation(recSite.fk_cmdb_id,objCIFormDoc.oCI().pk_auto_id, strRel);  
		}
		if(objCIFormDoc.oCI().fk_company_id!="")
		{
            ...
            var recOrg = g.get_record("company", objCIFormDoc.oCI().fk_company_id)                               
			cmdb.generate_meconfigrelation(recOrg.fk_cmdb_id,objCIFormDoc.oCI().pk_auto_id,strRel);  
   		}
	
		if(objCIFormDoc.oCI().fk_sld>0)
		{
        	var strRel = "Uses";
        	var recSLA = g.get_record("itsmsp_slad", objCIFormDoc.oCI().fk_sld);    
			cmdb.generate_meconfigrelation(recSLA.fk_cmdb_id,objCIFormDoc.oCI().pk_auto_id,strRel); 
		}
		...
		var oRS = g.get_tablerecordset_bycol("ct_reltypes",ap);

		if(oRS.Fetch())
		{
			strDependancy = g.get_field(oRS,"fk_reltype");
			ynOperational = "No";
		}
		else
		{
			alert("Could not create dependency because no relationship types exist");
		...
		for (var x=0; x < olist.rowCount; x++)
		{
			...
			var sqlResult = false;
			sqlResult = cmdb.insert_configrelation(objCIFormDoc.oCI().pk_auto_id, objCIFormDoc.oCI().ck_config_type, objCIFormDoc.oCI().ck_config_item, fetchKey, fetchType, fetchText, strDependancy, ynOperational, objCIFormDoc.oCI().description, fetchDesc);
		}
		...
		for (var x=0; x < olist.rowCount; x++)
		{
			...
			var sqlResult = false;
			sqlResult = cmdb.insert_configrelation(fetchKey, fetchType, fetchText, objCIFormDoc.oCI().pk_auto_id, objCIFormDoc.oCI().ck_config_type, objCIFormDoc.oCI().ck_config_item, strDependancy, ynOperational, fetchDesc, objCIFormDoc.oCI().description);
		}
	...
	else
	{
		...
		if(objCIFormDoc.oCI().fk_sld>0)
		{
			...
			if(strCurrentSLAID==0)
			{
				...
				var recSLA = g.get_record("itsmsp_slad", strNewSLAID);
				cmdb.generate_meconfigrelation(recSLA.fk_cmdb_id,objCIFormDoc.oCI().pk_auto_id,"Uses");
			}
			else
			{
				...
				{		
					...
					g.submitsqs("cmdb/sla_ci_relationship.delete",strParams);
		...
		if(objCIFormDoc.doc.boolForceDiaryUpdate && objCIFormDoc.doc.boolInserted)
		{
			...
			var strURL = "config_diary.updatetxt=" + objCIFormDoc.doc.strAutoDiaryText + "&config_diary.analystid=" + session.analystId + "&config_diary.fk_ci_id=" + objCIFormDoc.oCI().pk_auto_id;
			cmdb.create_diaryentry(objCIFormDoc.oCI().pk_auto_id, strURL,function()
			{
		...
		{
			//-- add diary entry
			cmdb.insert_diaryentry(objCIFormDoc.oCI().pk_auto_id, "Manual Update", "General Update", session.analystId , global.GetCurrentEpocTime(), objCIFormDoc.doc.strSilentDiaryText, 0);
			objCIFormDoc.doc.boolInserted = true;
		}
		else
		{
...
oCiForm.prototype.setup_form_fields=function(boolInserting)
{
	...
	cmdb.load_control_fields(this.oCI().ck_config_type);
	...
	if(boolInserting)
	{
		...
		g.popuplate_form_fromargs("config_itemi", this.doc, true,true);
	...
	else
	{
		...
		cmdb.process_form_ciattribute_disablecheck(this.doc);
	...
	//-- disable form
	cmdb.disable_for_blackout(this.MF(),this.doc);
...
oCiForm.prototype.load_ci_sr_types=function()
{
	...
	{
		var strFilter = "swc=pk_code&kvs=" + g.get_ci_service_request_type(this.oCI().ck_config_type,false,",");
	}
...
oCiForm.prototype.change_config_type=function(funcCallback)
{
	...
	cmdb.select_configtype("",true,function(strNewType)
	{
		if(strNewType=="")
		{
			if ((funcCallback != undefined) && (funcCallback != null))
		...
		else
		{
			if(strOrigType != strNewType)
			{
				...
				var strNewFormName = cmdb.get_configtype_formname(strNewType);
				if(strNewFormName != objCIFormDoc.oCIType().extended_form)
				{
					...
					{	
						...
						cmdb.popup_configitem(objCIFormDoc.oCI().pk_auto_id,true,"",true,function()
						{
							if ((funcCallback != undefined) && (funcCallback != null))
							{
								funcCallback();
...
{
	this.oCI().fk_sla = itsm.get_sla_from_slad_matrix(this.oCI().fk_sld, this.oCI().fk_impact_level, this.oCI().fk_urgency_level);
	this.doc.UpdateFormFromData();
}
...
oCiForm.prototype.OnFormLoaded = function ()
{
	...
	// RJC F0104320
	//--g.apply_appcode_sla_filter(this.MF().fld_fk_sld1,'SLA-CMDB');
...
oCiForm.prototype.OnFieldValueChanged = function (strName,strValue)
{
	...
	{
		var passbackvalue = cmdb.check_ci_element_attribute(oEle,this.doc,undefined,strValue);
...
oCiForm.prototype.OnTimer = function (nTimer)
{
	...
	if(nTimer==1)
	{
		g.set_doc_record_fieldvalue(this.doc, this.doc.strChangeBackBinding, this.doc.strChangeBackToValue);
	...
	if(nTimer==2)
	{
		...
		//-- re enable form
		cmdb.enable_for_blackout(this.MF(),this.doc);
		this.doc.UpdateFormFromData();
...
oCiForm.prototype.tabCI_OnTabSelected = function (nTab)
{
	...
	else if(nTab==tBaselines)
	{
		...
		var boolEnableChangeControl = g.check_change_control(this.oCI().pk_auto_id,this.oCI().ck_config_type);
		if(boolEnableChangeControl && !cmdb.is_specialist())
		{
		...
		this.MF().sl_baselines.filter = "_swc_baselineid="+this.oCI().baselineid;
	...
	{
		var strOLAs = g.get_ci_olas(this.oCI().pk_auto_id);	
		g.sqllist_filter(this.MF().sl_olas1, "swc=pk_ola_id&kvs=" + strOLAs, null, null);
	...
	}else if(nTab==tKB)
	{
		...
		{
			var strDocs = oKB.get_cis_kbs(this.oCI().pk_auto_id);
			if(strDocs=="")strDocs="-1";
			g.sqllist_filter(this.MF().sl_kb1, "swc=docref&kvs=" + strDocs, null, null);
		}
		this.MF().sl_kb1.Refresh();
		
		//ES - F0112292
		this.MF().Text5.text = "Knowledgebase Articles associated to Configuration Type: " + this.MF().tb_type.text;
		if(this.MF().sl_kb2.rowCount<1)
		{
			var strDocsT = oKB.get_ci_types_kbs(this.oCIType().pk_config_type);
			if(strDocsT=="")strDocsT="-1";
			g.sqllist_filter(this.MF().sl_kb2, "swc=docref&kvs=" + strDocsT, null, null);
		}
		this.MF().sl_kb2.Refresh();
		  
	}else if(nTab==tAction)
	{
		g.sqllist_filter(this.MF().sl_diary, "_swc_fk_ci_id="+this.oCI().pk_auto_id, null, null);
...
oCiForm.prototype.tabCom_OnTabSelected = function (nTab)
{
	...
	if(nTab==tRequestTypes)
	{
		...
		g.sqllist_filter(this.MF().sl_parentprofiles, strFilter, null, null);
	...
	{	
		...
		var strRels = cmdb.get_parent_dependencies(this.oCI().pk_auto_id,bActiveParents,bActiveServices);				
		this.MF().sl_parentrels.filter = "pids=" + strRels;
	...
	else if(nTab==tChildRel)
	{
		...
		var strRels = cmdb.get_child_dependencies(this.oCI().pk_auto_id,bActiveChildren);	
		this.MF().sl_childrels.filter = "pids=" + strRels;
		this.MF().sl_childrels.Refresh();		
		_een(this.MF().btn_del_child, (this.MF().sl_childrels.curSel>-1));
	}
	else if(nTab==tMErel)
	{
		this.MF().sl_merel.filter = "_swc_fk_ci_id="+this.oCI().pk_auto_id;
...
oCiForm.prototype.tabComAdd_OnTabSelected = function (nTab)
{
	...
	if(nTab==tContract)
	{
		...
		var strContracts = cfg.get_ci_contractkeys(this.oCI().pk_auto_id,false);
		
		g.sqllist_filter(this.MF().sl_contracts1, "swc=pk_contract_id&kvs="+ strContracts+"&sf=type_supplier", null, null);
		this.MF().sl_contracts1.Refresh();
	}else if(nTab==tSLA)
	{
		//-- get current sla list to append
		var strSLAs = g.get_ci_slas(this.oCI().pk_auto_id);
		
		// RJC F0104320
		g.apply_appcode_sla_filter(this.MF().sl_slad1,'SLA-CMDB',"swc=pk_slad_id&kvs="+strSLAs);
	...
	}else if(nTab==tOLA)
	{
		...
		// RJC F0104320
		g.apply_appcode_sla_filter(this.MF().sl_olas1,'OLA',"swc=pk_ola_id&kvs="+strOLAs);	
		this.MF().sl_olas1.Refresh();
	}
	//-- Default SLAs
...
oCiForm.prototype.btn_remove_slad_OnPressed = function()
{
	...
	if(confirm(strConfirm))
	{
		var fk_sla_id = g.sl_getprikeyvalue(this.MF().sl_slad1, this.MF().sl_slad1.curSel);
		var strSLACMDBID = g.sl_getcolvalue(this.MF().sl_slad1, -1, "fk_cmdb_id");
		cmdb.delete_meconfigrelation(strSLACMDBID,this.oCI().pk_auto_id);
		// RJC F0104320
		var strSLAs = g.get_ci_slas(this.oCI().pk_auto_id);
		g.apply_appcode_sla_filter(this.MF().sl_slad1,'SLA-CMDB',"swc=pk_slad_id&kvs="+strSLAs);
		this.MF().sl_slad1.Refresh();
...
oCiForm.prototype.btn_add_slad_OnPressed = function()
{
	...
	if(!oSettings.get_setting("boolFilterAppcodeSLALoaded"))g.load_appcode_sla();
	...
	app.OpenForm("browse_slas", strURL, true, function(x)
	{
		...
		for (var x=0; x< arrKeys.length;x++)
		{
			...
			if(g.sl_findrow_byvalue(objCIFormDoc.MF().sl_slad1,fk_key,"pk_slad_id")==-1)
			{
				var strRel = "Uses";
				var recSLA = g.get_record("itsmsp_slad", fk_key);                               
				cmdb.generate_meconfigrelation(recSLA.fk_cmdb_id,objCIFormDoc.oCI().pk_auto_id,strRel); 
		...
		if(appendKeys!="")	
		{
			g.sqllist_appendfilter(objCIFormDoc.MF().sl_slad1, appendKeys, "pk_slad_id");		
...
oCiForm.prototype.mbtn_rt_actions_OnMenuItem = function (strName,nItemID)
{
	...
	if(nItemID == iAssignType)
	{
		...
		app.OpenFormForAdd("browse_requesttypes", "", strURL, true, function(oDoc)
		{
			if(oDoc.strSelectedKey!="")
			{
				...
				if(g.submittableinsert("ct_profiles",arrCols))
				{
					g.sl_refresh(this.MF().sl_profiles,null,0); 
				}
			}	
			return;
		});
	...
	else if(nItemID == iRemoveType)
	{
		...
		g.sl_deldbrow(this.MF().sl_profiles,this.MF().sl_profiles.curSel,'pk_auto_id',null,strConfirm);		
	}
...
oCiForm.prototype.mbtn_ola_actions_OnMenuItem = function (strName,nItemID)
{
	...
	if(nItemID == iAssignCI)
	{
		...
		app.OpenFormForAdd("browse_olas", "", strURL, true, function(x)
		{
			...
			{
				var fk_key = arrKeys[x];
				if(g.sl_findrow_byvalue(objFormDoc.MF().sl_olas1,fk_key,"pk_ola_id")==-1)
				{
					var strRel = "Uses";
					cmdb.generate_meconfigrelation(arrCMDBIDs[x],objFormDoc.oCI().pk_auto_id,strRel);  
			...
			if(appendKeys!="")	
			{
				g.sqllist_appendfilter(objFormDoc.MF().sl_olas1, appendKeys, "pk_ola_id");		
	...
	else if(nItemID == iRemoveCI)
	{
		...
		if(confirm(strConfirm))
		{
			var fk_cmdb_id = g.sl_getcolvalue(this.MF().sl_olas1, this.MF().sl_olas1.curSel, "fk_cmdb_id");//g.sl_getprikeyvalue(sl_olas1, sl_olas1.curSel);
			cmdb.delete_meconfigrelation(fk_cmdb_id,this.oCI().pk_auto_id);
...
oCiForm.prototype.mbtn_ci_contract_actions_OnMenuItem = function (strName,nItemID)
{
	...
	{
		//-- refresh list
		var strContracts = cfg.get_ci_contractkeys(objCIFormDoc.oCI().pk_auto_id,false);
	...
	if(nItemID == iRemoveContract)
	{
		...
		if (!confirm("Are you sure you want to remove the selected contract from this ci ?")) return;
		{
			...
			cmdb.delete_meconfigrelation(strContractCMDBID, this.oCI().pk_auto_id);
			this.MF().sl_contracts1.Refresh();
    	} 		
	}
	else if(nItemID == iSearchContract)
	{
		...
		g.search_for("contract",true,strURL,function(oRes)
		{
			...
			var strCICompanyKeys = cmdb.get_ci_company_ids(objCIFormDoc.oCI().pk_auto_id,false);
			//--End
			...
			for(var x=0; x< arrKeys.length;x++)
			{
				...
				var strCIS = cfg.get_supplier_cikeys(arrCompanyKeys[x],true,true);
				var strCIS = cfg.get_contract_cikeys(arrContractKeys[x],true);
				...
				cmdb.generate_meconfigrelation(arrKeys[x], objCIFormDoc.oCI().pk_auto_id, strRel);		
				...
				if(strCICompanyKeys.indexOf(arrCompanyKeys[x])==-1)
				{
					...
					oOrgRec = g.get_record("company", arrCompanyKeys[x]);
					if(oOrgRec)
					{
						cmdb.generate_meconfigrelation(oOrgRec.fk_cmdb_id, objCIFormDoc.oCI().pk_auto_id, strCompanyRel);
						objCIFormDoc.MF().sl_contracts1.Refresh();
					}
				}
			}
			...
			if(boolAlertDupe)
			{
				alert("One or more of the contracts selected is already associated with this configuration item.");
			}
				//-- if not a ci used by org and we havent told user tell them and then dont bother
			if(boolAlertOwnership)
			{
				boolAlerted = true; 
				alert("One or more of the contracts selected are not provided by the supplier of this Configuration Item.\nSuch contracts will not be associated to the configuration item.");
			}
...
oCiForm.prototype.btn_pro_up_OnPressed=function()
{
	...
	if(g.submittableupdate("cmdb_uri_link",intUpID,p))
	{
		p.nindex = intUpSeq;
		if(g.submittableupdate("cmdb_uri_link",intDownID,p))		
		{
			//-- refresh list
			this.MF().sl_uri1.Refresh();
...
oCiForm.prototype.btn_del_pro_OnPressed=function()
{
	...
	if(g.sl_deldbrow(this.MF().sl_uri1,this.MF().sl_uri1.curSel,'pk_uri_id',null,strConfirm))
	{
		...
		for(var x=0; x < this.MF().sl_uri1.rowCount;x++)
		{
			...
			g.submittableupdate("cmdb_uri_link",intID,p);				
		}
...
oCiForm.prototype.mbtn_aba_actions_OnMenuItem = function (strName,nItemID)
{
	...
	{
        var objFormDoc = this;
		g.search_for("bus_area", true,"", function(oRes)
		{
			...
			{
				var fk_key = arrKeys[x];
				if(g.sl_findrow_byvalue(objFormDoc.MF().sl_bus_area,fk_key,"fk_bus_area_id")==-1)
				{			
					cmdb.insert_affected_bus_area(objFormDoc.oCI().ck_config_item, fk_key);
				}
			}
	...
	else if(nItemID == iRemoveBusArea)
	{
		...
		if(strID!="")
		{
			...
			if(confirm(strConfirm))
			{
				cmdb.delete_affected_bus_area(this.oCI().ck_config_item, strID, "");
				this.MF().sl_bus_area.Refresh();
			}
		}
	}
...
oCiForm.prototype.mbtn_kb_actions_OnMenuItem = function (strName,nItemID)
{
	...
	{
        var objFormDoc = this;
		g.search_for("swkb", true,"", function(oRes)
		{
			var strKeys = oRes.selectedkeys;
			if(strKeys=="")return;
			...
			{
				var fk_key = arrKeys[x];
				if(g.sl_findrow_byvalue(objFormDoc.MF().sl_kb1,fk_key,"docref")==-1)
				{
					oKB.relate_ci_to_kb(objFormDoc.oCI().pk_auto_id,fk_key);
				}
			}
			
			//-- re-fetch list
			var strDocs = oKB.get_cis_kbs(objFormDoc.oCI().pk_auto_id);
			if(strDocs=="")	strDocs="-1";
			objFormDoc.MF().sl_kb1.filter = "swc=docref&kvs=" + strDocs;
	...
	{
		var fk_kb_id = g.sl_getprikeyvalue(this.MF().sl_kb1, this.MF().sl_kb1.curSel);
		if(fk_kb_id!="")
		{
			var strConfirm = "Are you sure you want to remove the selected Knowledgebase article from the Configuration Item?";
			if(confirm(strConfirm))
			{
				oKB.unrelate_ci_from_kb(fk_kb_id,this.oCI().pk_auto_id);	
			}
			var strDocs = oKB.get_cis_kbs(this.oCI().pk_auto_id);
			if(strDocs=="")	strDocs="-1";
			this.MF().sl_kb1.filter = "swc=docref&kvs=" + strDocs;
...
oCiForm.prototype.btn_new_me_OnPressed=function()
{
	...
	g.search_for("customers",true,strURL, function(oRes)
	{
		if(oRes.selectedkeys=="")return;
		...
		for (var x=0; x< arrKeys.length;x++)
		{
			...
			if(g.sl_findrow_byvalue(objCIFormDoc.MF().sl_merel,fk_key,"fk_me_key")==-1)
			{
				//-- nwj 25.11.2012 - use sqs
				var arrCols = [];
				...
				g.submittableinsert("config_relme",arrCols);		
			}
			...
			if(arrCMDBIDs[x]>0)
			{
				//cmdb.insert_meconfigrelation(arrCMDBIDs[x], this.oCI().pk_auto_id, this.oCI().ck_config_type, this.oCI().ck_config_item, "uses", 'No');
				cmdb.insert_meconfigrelation(arrCMDBIDs[x], objCIFormDoc.oCI().pk_auto_id, objCIFormDoc.oCI().ck_config_type, objCIFormDoc.oCI().ck_config_item, "uses", 'No', objCIFormDoc.oCI().description);
...
function cmdb_compare_baseline_data(oRecMasterCI, intOtherCIID, funcCallback)
{
	...
	{
		var oRecOtherCI = g.get_record("config_itemi", intOtherCIID);
		if(oRecOtherCI)
		{
			...
			{
				var strSessID = session.analystid + "_" + global.GetCurrentEpocTime();
				...
				for (x=0; x < dd.tables.config_itemi.columns.length;x++)
				{
					...
					if(g.dd_fieldtype('config_itemi',colName)==5)
					{
						strMasterValue = g.fixepoch(strMasterValue);
						strOtherValue = g.fix_epoch(strOtherValue);
					}
					...
					if( ((LC(strOtherValue) != LC(strMasterValue))) && (strSkipCols.indexOf(colName)<0) )
					{
						...
						var strFormattedMasterValue = g.dd_format('config_itemi',colName,oRecMasterCI[colName]);
						var strFormattedOtherValue = g.dd_format('config_itemi',colName,oRecOtherCI[colName]);
				...
				var oRS = g.get_sqrecordset("cmdb/common/select.config_type.exttable","ct=" + oRecMasterCI.ck_config_type );
				if(oRS.Fetch())
				{
					strConfigExtTable = LC(g.get_field(oRS,"EXTENDED_TABLE"));
					if( (strConfigExtTable!="")&&(dd.tables[strConfigExtTable]) )
					{
						//-- get extended data
						var oRecMasterCIExtData = g.get_record(strConfigExtTable, oRecMasterCI.pk_auto_id);
						var oRecOtherCIExtData = g.get_record(strConfigExtTable, intOtherCIID);
						...
						for (x=0; x < dd.tables[strConfigExtTable].columns.length;x++)
						{
							...
							var boolSet=false;
							var strFormattedMasterValue = g.dd_format(strConfigExtTable,colName,oRecMasterCIExtData[colName]);
							if(!boolSameExtTable)
							{
							...
							else
							{
								...
								{
									//-- format values i.e. if an epoch with give us date setting 
									var strFormattedOtherValue = g.dd_format(strConfigExtTable,colName,oRecOtherCIExtData[colName]);
									boolSet=true;
								}
							}
			...
			{
				alert("You cannot compare a configuration item against itself. Please select another configuration item for comparison");
			}
		}
		else
		{
			alert("Failed to load the comparision configuration item. Please contact your Supportworks Administrator.");
		}
	}
}
...
function cmdb_ci_relationship_exists(strRelType,boolCreate)
{
	...
	//if(g.gxet_rowcount("CONFIG_RELTYPE","PK_RELTYPE = '" + g.pfs(strRelType) + "'")<1)
	if(g.sqs_rowcount("cmdb/ci_relationship_exists.count","rel=" + pfu(strRelType))<1)
	{
		//-- does not exist - do we create
		if(boolCreate)
		{
			//var strInsert = "ixnsert into CONFIG_RELTYPE (PK_RELTYPE,LINE_STYLE,LINE_COLOUR) VALUES ('" + g.pfs(strRelType) + "',2,'#000000')";
			//return g.sxubmitsql(strInsert,true);
			//- -nwj - use sqs - 11.2012
			var strParams = "rel=" + pfu(strRelType);
			return g.submitsqs("cmdb/ci_relationship_exists.insert",strParams);
		}
		else
		{
			return false;
...
function cmdb_load_control_fields(strConfigType, boolSC)
{
	...
	if(boolSC==1)
		strTable = "SC_TYPEI";
	cmdb.arr_control_atts[boolSC][strConfigType] = [];

	//-- fetch rows
	var ap = {};
	ap.fk_config_type = strConfigType;
	ap.flg_sc=boolSC;
	var oRS = g.get_tablerecordset_bycol("ct_ctrl_fields",ap);
	while(oRS.Fetch())
	{
		var oBj = {};
		oBj.type	= g.get_field(oRS,"fk_config_type");
		oBj.databinding	= g.get_field(oRS,"databinding");
		oBj.regexcheck	= g.get_field(oRS,"regexcheck");
		oBj.regexerrormsg = g.get_field(oRS,"regexerrormsg");
		oBj.flg_autodiary	= g.get_field(oRS,"flg_autodiary");
		oBj.flg_silent_diary	= g.get_field(oRS,"flg_silent_diary");
		oBj.flg_diary	= g.get_field(oRS,"flg_diary");
		oBj.flg_ctrl_flag	= g.get_field(oRS,"flg_ctrl_flag");

		if(cmdb.arr_control_atts[boolSC][oBj.type]==undefined)cmdb.arr_control_atts[boolSC][oBj.type] = [];
		cmdb.arr_control_atts[boolSC][oBj.type][oBj.databinding] = oBj;
	}

	var currCIType = g.get_record(strTable, strConfigType);
	while(currCIType.flg_inherit_autodiary==1 || currCIType.flg_inherit_regex==1 || currCIType.flg_inherit_protected==1)
	{
		...
		{
			var oBj = {};
			oBj.type	= strConfigType;//g.get_field(oRS,"fk_config_type");
			oBj.databinding	= g.get_field(oRS,"databinding");
			oBj.regexcheck	= g.get_field(oRS,"regexcheck");
			oBj.regexerrormsg = g.get_field(oRS,"regexerrormsg");
			oBj.flg_autodiary	= g.get_field(oRS,"flg_autodiary");
			...
			if(cmdb.arr_control_atts[boolSC][strConfigType][oBj.databinding]==undefined){
				//limit to inherit
				...
				{
					cmdb.arr_control_atts[boolSC][strConfigType][oBj.databinding] = oBj;
				}
			}
		}
		...
		{
			currCIType = g.get_record(strTable, currCIType.fk_config_type);
			if(!inheritRegex)
...
{
	//F0086202 - if boolSC is undefined, assume cmdb. 
	if(boolSC==undefined)boolSC=0;
...
function cmdb_process_ci_attribute_check(strBinding , varValue, strConfigType, oDoc, boolSC)
{
	...
	var originalValue = g.get_doc_record_fieldvalue(oDoc, strBinding);
		...
		if(this.arr_control_atts[boolSC][strConfigType][strBinding])
		{
			...
			if(strRegEx!="")
			{
				if(!testregex(varValue,strRegEx))
				{
					...
					{
						alert("The value entered does not meet the field requirement. Please try again.");
					}
			...
			if(this.arr_control_atts[boolSC][strConfigType][strBinding].flg_autodiary == "1")
			{
				...
				oDoc.strAutoDiaryText += "Field [" + g.dd_fieldlabel_frombinding(strBinding) + "] was changed from [" + originalValue + "] to [" + varValue+ "]\n";
			}
			...
			if(this.arr_control_atts[boolSC][strConfigType][strBinding].flg_silent_diary == "1")
			{
				...
				oDoc.strSilentDiaryText += "Field [" + g.dd_fieldlabel_frombinding(strBinding) + "] was changed from [" + originalValue + "] to [" + varValue+ "]\n";
			}
		}
	}
...
function cmdb_ci_exists(strConfigItem,strConfigType)
{
	...
	if(g.sqs_rowcountbycol("config_itemi",arrParams)>0)return true;
...
function cmdb_ci_type_exists(strConfigType)
{
	...
	if(g.sqs_rowcountbycol("config_typei",arrParams)>0)return true;
...
function cmdb_service_type_exists(strServiceType)
{
	...
	if(g.sqs_rowcountbycol("sc_typei",arrParams)>0)return true;
...
function cmdb_form_field_exists(strField,strConfigType)
{
	...
	if(g.sqs_rowcountbycol("ct_ctrl_fields",arrParams)>0)return true;
...
function cmdb_default_value_exists(strField,strConfigType)
{
	...
	if(strConfigType!="")arrParams.fk_config_type = strConfigType;
	if(g.sqs_rowcountbycol("CONFIG_TYPE_DEF",arrParams)>0)return true;
	return false;
}
...
function cmdb_view_inventory(intCI,strInventoryID)
{
	...
	//-- Create base URL
	var strServer = "http://" + session.server + ":" + session.httpPort;

	//-- DJH : Check if connector is enabled, if not then return message
	if(!bConnectorEnabled)
	{
		alert("The inventory view for your server has not been setup. Please contact your Administrator.")
		return false;
	}
	...
	if(!((strConnectorLookupField=="ck_config_item") && (strInventoryID!=undefined) && (strInventoryID!="")))
	{
		...
		var ciRec = g.get_record("config_itemi", intCI);
		strInventoryID = ciRec[strConnectorLookupField];

		if(strInventoryID=="" || strInventoryID==undefined)
		{
			alert("The external connector identifier for the selected item is not set. Please contact your Administrator.");
			return false;
		}
	}
	...
	*/
	if(strConnectorURL=="")
	{
		alert("The CONNECTOR.URL setting for your server has not been setup. Please contact your Administrator.");
		return false;
	}
	...
	RunHIB(strUrl, "compname", strInventoryID);
	//-- TK Rework URL
	//RunHIB(strUrl, "strCompID=" + ciRec.inventory_tool_id + " strCompName"+strInventoryID);
}
...
function cmdb_view_schedule(intCIs, strItems, funcCallback)
{
	...
	{
	 	var ciRec = g.get_record("config_itemi", intCIs);
...
function cmdb_check_schedule(intCIs, startDatex, endDatex,strType)
{
	...
	var oRS = g.get_sqrecordset("cmdb/check_schedule.select",oParams);

	//-- fetch rows
	while(oRS.Fetch())
	{
		intCIid		= g.get_field(oRS,"fk_ci_auto_id");
		intCallref	= g.get_field(oRS,"fk_callref");
		intStartx	= g.get_field(oRS,"itsm_schedstartx");
		intEndx		= g.get_field(oRS,"itsm_schedendx");
...
function cmdb_check_activity_schedule(intCIs, oActivites, boolChange, requiredBy, callref)
{
	...
	if(intCIs!="")
	{
		...
		//var oRS = g.gxet_recordset(strSelect, "swdata");
		var strParams = "cc=" + strCallclass+"&rc="+strRelCode+"&cr="+callref+"&cids="+intCIs;
		var oRS = g.get_sqrecordset("cmdb/check_activity_schedule.select",strParams);
		while(oRS.Fetch())
		{
			if(strAlertMessage!="")
			{
			...
			var oCI = g.get_record("config_itemi", g.get_field(oRS,"fk_ci_auto_id"));
			strAlertMessage += "The item "+oCI.ck_config_item+" is also associated to "+strCallclass+" "+ g.callref_pad(g.get_field(oRS,"callref"))+".";		
		}		
	}

	var strParams = "call_reference=" + callref;
	var oRS = g.get_sqrecordset("cmdb/get_activities_ref.select", strParams);
	for(items in oActivites)
	{
		...
		if(intCIs!="")
		{
			...
			for(x in arrCIs)
			{			
				var intWorkingHours = cmdb.get_unavailable_workingtime(arrCIs[x], oActivity.startDatex, oActivity.endDatex);
				if(intWorkingHours>0)
				{
					...
					var oCI = g.get_record("config_itemi", arrCIs[x]);
			...
			for(x in arrCIs)
			{
				var arr_items = cmdb.get_bl_dates(oActivity.startDatex,oActivity.endDatex,arrCIs[x]);
				if(arr_items.length>0)
				{
					...
					for(y in arr_items)
					{
						...
						if(secondPartAlertMessage!="")
							secondPartAlertMessage+="\n";
						secondPartAlertMessage+=oBL.oCIName+" has blackout '"+oBL.oName+"' from "+g.convert_epochddmmyyyy(oBL.oStart)+" to "+g.convert_epochddmmyyyy(oBL.oEnd);
		...
		while (oRS.Fetch())
		{
			id_activity = g.get_field(oRS,'pk_auto_id');
			activity_end = g.fix_epoch(g.get_field(oRS,'endx'));
			activity_start= g.fix_epoch(g.get_field(oRS,'startx'));
		...
		var oItemRS = g.get_sqrecordset("form/fsc_item/get_scheduled_items.select","");
		while(oItemRS.Fetch())
		{
			strType = g.get_field(oItemRS,"type");
			strTitle = g.get_field(oItemRS,"title");
			intItemStart = g.fix_epoch(g.get_field(oItemRS,"startx"));
			intItemEnd = g.fix_epoch(g.get_field(oItemRS,"endx"));
			if (itsm.check_overlapping_periods(intItemStart,intItemEnd,oActivity.startDatex,oActivity.endDatex))
			{
				boolAlert = true;
				if (strAlertMessage != "")
				{
		...
		requiredBy = g.fix_epoch(requiredBy);
		...
		if(requiredBy>0)
		{
			if(requiredBy<oActivity.endDatex)
			{
				if(strAlertMessage!="")
					strAlertMessage+="\n\n";
				strAlertMessage+= "The activity '"+oActivity.title+"' is scheduled to end ("+g.convert_epochddmmyyyy(oActivity.endDatex)+") after the change is required to be completed ("+g.convert_epochddmmyyyy(requiredBy)+").";
			}
		}
	}
...
function get_unavail_workingtime_onday(initStartTime,initEndTime,tmpDayObject)
{
	...
	var slaStartTime = Number(g.string_replace(checkStartTime,":","",false));
	var slaEndTime = Number(g.string_replace(checkEndTime,":","",false));
	var unvStartTime = Number(g.string_replace(initStartTime,":","",false));
	var unvEndTime = Number(g.string_replace(initEndTime,":","",false));
...
function calc_minutes(startTime,endTime)
{
	if(startTime=="")startTime="00:00";
	if(endTime=="")endTime="00:00";
	startTime = g.string_replace(startTime,":","",false);
	startTime = g.string_replace(startTime,"30","50",false);
	endTime = g.string_replace(endTime,":","",false);
	endTime = g.string_replace(endTime,"30","50",false);
	...
	else
	{
		...
		totalTime = g.string_replace(totalTime,"50","",false);
		totalTime = g.string_replace(totalTime,"0","",true);
...
function cmdb_insert_diaryentry(intCI, strSource, strCode, strAnalystID, intDate, strDesc, fk_callref)
{
	...
	if(xmlmc.Invoke("data", "addRecord"))
	{
		...
		if(!objRes.success)
		{
	        alert("Failed to process record creation (" + objRes.message + "). Please contact your Supportworks Supervisor.");
	    }	
	    else
	    {
			boolSuccess = true;
    ...
    {
    	alert("Failed to process record creation. " + xmlmc.GetLastError() + ". Please contact your Supportworks Supervisor.");
    }		
	return boolSuccess;
}
...
function cmdb_popup_availform(intCI,boolCreate,funcCallback)
{
	...
	{	
		var p = {};
		p.fk_ci_id=intCI;
		var intHistID = g.get_tablecolmax_bycol("ci_avail_hist","pk_auto_id",p)
		//--
...
function cmdb_get_cmdbstatus(strType, strStatus)
{
	...
	var oRS = g.get_tablerecordset_bycol(strTable,ap,false,"CMDB_STATUS");

	while(oRS.Fetch())
	{
		strCMDBStatus = g.get_field(oRS,"CMDB_STATUS");
	}
...
function cmdb_select_configtype(strTitle,boolDefinition,funcCallback)
{
	...
	g.popup("cmdb_configtype_picker",strURL, function(aForm)
	{
		if ((funcCallback != undefined) && (funcCallback != null))
		{
			if (aForm)
...
function cmdb_create_cmdb_item_def(strTitle,funcCallback)
{
	...
	g.popup("cmdb_category_picker",strURL,function(aForm)
	{
		if (aForm)
		{
			if (IsObjectDefined("_bHtmlWebClient"))
			...
			if (strType!="") 
			{
				var oRec = g.get_record("config_typei",strType);
				if(oRec)
				{
...
function cmdb_search_ci(boolMulti, strAppendURL, funcCallback)
{
	...
	g.popup("search_ci",strURL,function(aForm)
	{
		var tmpObj = {};
		if (aForm)
		{
...
function cmdb_search_for_service(boolMulti, strAppendURL, funcCallback)
{
	...
	var strMulti = (boolMulti)?"1":"0";
	var strURL = "searchmode=1&multiselect=" + strMulti + "&" + strAppendURL;
	g.popup("search_for_service",strURL,function(aForm)
	{
		var tmpObj = {};
		if (aForm)
		{
...
function cmdb_search_service(boolMulti, strAppendURL, funcCallback)
{
	...
	var strMulti = (boolMulti)?"1":"0";
	var strURL = "searchmode=1&multiselect=" + strMulti + "&" + strAppendURL;
	g.popup("search_service",strURL,function(aForm)
	{
		var tmpObj = {};
		if (aForm)
		{
...
function cmdb_relate_ci_to_call(intCallref,intCIkey,strCode)
{
	...
	return g.submitsqs("cmdb/relate_ci_to_call.insert",strParams);
...
function cmdb_unrelate_ci_from_call(intCallref,intCIkey,strCode)
{
	...
	return g.submitsqs("cmdb/unrelate_ci_from_call.delete",strParams);
...
function cmdb_get_citype_parentssrequests(strStartAtType)
{
	...
	var oRS = g.get_sqrecordset("cmdb/get_citype_parentssrequests.select",p);

	//-- nwj we are returning values for filter so do not pfs or encaps as sqs filter script will take care of that.
	while(oRS.Fetch())
	{
		currKey = g.get_field(oRS,"fk_typecode");
...
function cmdb_remove_me_ci_link(strMEKeys)
{
	...
	g.submitsqs("cmdb/remove_me_ci_link.delete",strParams);
}
...
function cmdb_remove_ci_me_link(strCIids, strMeKeys)
{
	...
	g.submitsqs("cmdb/remove_ci_me_link.delete",strParams);
...
function cmdb_get_ci_me_ids(intCI,strMEType,boolPFS)
{
	...
	p.pt = strMEType;
	p.cids = intCI;
	var oRS = g.get_sqrecordset("cmdb/get_ci_me_ids.select",p);

	//-- nwj we are returning values for filter so do not pfs or encaps as sqs filter script will take care of that.
	while(oRS.Fetch())
	{
		currKey = g.get_field(oRS,"FK_PARENT_ITEMTEXT");
...
function cmdb_get_me_cis(strMEKeys , strCode, strAddWhere , boolGetCustOwnedCis)
{
	...
	p.rc = strCode;
	p.sf = strAddWhere;
	var oRS = g.get_sqrecordset("cmdb/get_customers_cikeys.select",p);
	while(oRS.Fetch())
	{
		currKey = g.get_field(oRS,"FK_CI_ID");
	...
	if((strCode=="CUSTOMER")&&(boolGetCustOwnedCis))
	{
		...
		while(oRS.Fetch())
		{
			currKey = g.get_field(oRS,"PK_AUTO_ID");
			if (strCIlist!="")strCIlist +=",";
			strCIlist += currKey;
		}
		//-- get related cis
		...
		p.mids = strMEKeys;
		p.sf = strAddWhere;
		var oRS = g.get_sqrecordset("cmdb/get_customers_cikeys_related.select",p);
		while(oRS.Fetch())
		{
			currKey = g.get_field(oRS,"FK_CI_ID");
			if (strCIlist!="")strCIlist +=",";
...
function cmdb_get_site_cis(strSiteID)
{
	...
	var oRS = g.get_tablerecordset_bycol(strTable,ap,false,"pk_auto_id");
...
function cmdb_get_ci_mes(strCIKeys , strCode, boolPassbackNumericKeys,boolGetCustOwnedCis)
{
	...
	p.cids = strCIKeys;
	p.rc = strCode;
	var oRS = g.get_sqrecordset("cmdb/get_ci_mes.select",p);

	//-- nwj we are returning values for filter so do not pfs or encaps as sqs filter script will take care of that.
	while(oRS.Fetch())
	{
		currKey = g.get_field(oRS,"FK_ME_KEY");
	...
	if((strCode=="CUSTOMER")&&(boolGetCustOwnedCis))
	{
		...
		var p = {};
		p.cids = strCIKeys;
		var oRS = g.get_sqrecordset("cmdb/get_ci_userdbkey.select",p);
	
		//-- nwj we are returning values for filter so do not pfs or encaps as sqs filter script will take care of that.
		while(oRS.Fetch())
		{
			currKey = g.get_field(oRS,"FK_USERDB");
			if (strMElist!="")strMElist +=",";
			strMElist += currKey;
		}
	}
...
function cmdb_get_parent_dependencies(intCI,bActiveOnly,bExclArchServices)
{
		...
		var oRS = g.get_sqrecordset("cmdb/common/get_parent_dependencies",p);
		while(oRS.Fetch())
		{
			strRelKey = g.get_field(oRS,"PK_AUTO_ID");
			strRels +=",";
			strRels += strRelKey;
		}
...
function cmdb_get_child_dependencies(intCI,bActiveOnly)
{
		...
		p.cid = intCI;
		p.abln = (bActiveOnly)?"1":"0";
		var oRS = g.get_sqrecordset("cmdb/common/get_child_dependencies",p);
		while(oRS.Fetch())
		{
			strRelKey = g.get_field(oRS,"PK_AUTO_ID");
			strRels +=",";
...
function cmdb_get_call_cis(intCallref , strCode, strAdditionalFilter)
{
	...
	var oRS = g.get_sqrecordset("cmdb/get_call_cis.select",p);
	while(oRS.Fetch())
	{
		currKey = g.get_field(oRS,"FK_CI_AUTO_ID");
		if (strCIlist!="")strCIlist +=",";
		strCIlist += currKey;
	}
	if(strCIlist=="")strCIlist="0";
...
function cmdb_get_call_ci_info(intCallref , strCode,strAdditionalFilter)
{
	...
	var oRS = g.get_sqrecordset("cmdb/get_call_ci_info.select",p);
	while(oRS.Fetch())
	{
		currKey = g.get_field(oRS,"FK_CI_AUTO_ID");
		currText = g.get_field(oRS,"CK_CONFIG_ITEM");
		currType = g.get_field(oRS,"CK_CONFIG_TYPE");
		if (strCIlist!="")
		{
			strCIlist +=",";
			strCIlistText +=",";
...
function cmdb_sl_load_call_cis(oSqlList, intCallref , strCode,strAdditionalFilter, strFilter)
{
	...
	oSqlList.filter = "pids=" + strFilter + "&sf=" + strAdditionalFilter;
	oSqlList.Refresh();
...
function cmdb_get_ci_relations(intCI , boolLoadParents,strFilter)
{
	...
	var ap = {};
	ap[pKey] = intCI;
	var oRS = g.get_tablerecordset_bycol("CONFIG_RELI",ap,false,"PK_AUTO_ID");
...
function cmdb_get_availrecs_started_between(intCIID, intDateOneX, intDateTwoX)
{
	...
	p.d1x = intDateOneX;
	p.d2x = intDateTwoX
	return g.get_sqrecordset("cmdb/get_availrecs_started_between.select",p);
}
...
function cmdb_update_avail_hist_stats(intCI)
{
	...
	g.submitsqp("cmdb/process/update_avail_hist_stats",strParams);
...
/*
	...
	var oRS = g.gxet_recordset(strSQL,"swdata");
	while(oRS.Fetch())
	{
		iTotal = iTotal + Number(g.number_to_time(g.get_field(oRS,'downtime')));
		iOpTotal = iOpTotal + Number(g.number_to_time(g.get_field(oRS,'opdowntime')));
	}
	...
	iTotal = g.mins_from_perc(iTotal);
	iOpTotal = g.mins_from_perc(iOpTotal);
...
function cmdb_get_ci_related_cis(intCI , boolLoadParents,strFilter)
{
	...
	var ap = {};
	ap[pKey] = intCI;
	var oRS = g.get_tablerecordset_bycol("CONFIG_RELI",ap,false,selKey);

	while(oRS.Fetch())
	{
		currKey = g.get_field(oRS,selKey);
		if (strReflist!="")strReflist +=",";
		strReflist += currKey;
	}
	if(strReflist=="")strReflist="-1";
...
function cmdb_sl_load_ci_relations(oSqlList, intCI , boolLoadParents,strFilter)
{
	...
	//-- 21.11.2012 - nwj - dev is calling with a static filter - since we are now using sqs this needs to be migrated to server
	if(strFilter.indexOf(" ")>-1)alert("SqlTableList (" + oSqlList.name + ") Filter Set By : cmdb_sl_load_ci_relations\n\n" + strFilter + "\n\nThis passed in filter should be changed to a serverside static sql array key.");
	
	var p = {};
	p.cid = intCI;
	p.blp = boolLoadParents;
	p.sf = strFilter;
	var oRS = g.get_sqrecordset("cmdb/sl_load_ci_relations.select",p);
	while(oRS.Fetch())
	{
		currKey = g.get_field(oRS,"PK_AUTO_ID");
		if (strReflist!="")strReflist +=",";
	...
	var oldSQ = oSqlList.storedQuery;
	oSqlList.storedQuery = "common.load_by_pkautoid";
	oSqlList.filter = "pids=" + strReflist
	oSqlList.SetRowSelected(0);
...
function cmdb_sl_load_ci_calls(oSqlList, intCI , strCode, strClass,strAdditionalFilter)
{
	...
	if(strAdditionalFilter.indexOf(" ")>-1)
	{
		alert("SqlTableList (" + oSqlList.name + ") Filter Set By : cmdb_sl_load_ci_calls\n\n" + strAdditionalFilter + "\n\nThis passed in filter should be changed to a serverside static sql array key.");
	}

	//-- set to load by auto id - can use common script for this
	oSqlList.storedQuery = "common.load_by_callref";
	oSqlList.filter = "crs=" + strRefFilter + "&sf=" + strAdditionalFilter;
	oSqlList.SetRowSelected(0);
}
...
function cmdb_get_ci_calls(intCIid, strCallClass , strCode)
{
	...
	var oRS = g.get_sqrecordset("cmdb/get_ci_calls.select",p);
	while(oRS.Fetch())
	{
		currKey = g.get_field(oRS,"FK_CALLREF");
		if (strReflist!="")strReflist +=",";
...
function cmdb_delete_configitem(strItems,boolDeleteBaselines)
{
	...
	return g.submitsqs("cmdb/process/delete_configitem",strParams);
	/*
	...
	g.sxubmitsql(strDeleteOCs,true);
	g.sxubmitsql(strDeleteRels,true);
	g.sxubmitsql(strDeleteHistory,true);
	g.sxubmitsql(strDeleteAvailHistory,true);
	g.sxubmitsql(strDeleteOCs,true);
	g.sxubmitsql(strDeleteCIServiceYypes,true);
	g.sxubmitsql(strDeleteEventActions,true);
	g.sxubmitsql(strDeleteBlackout,true);
	g.sxubmitsql(strDeleteDML,true);
	...
	if(boolDeleteBaselines)
	{
		...
		var oRS = g.gxet_recordset(strSelectBaselines,"swdata");
		while(oRS.Fetch())
		{
			if (strDeleteBaselineKeys!="")strDeleteBaselineKeys +=",";
			strDeleteBaselineKeys = g.get_field(oRS,"BASELINEID");
		}
	...
	var oRS = g.gxet_recordset(strSelectExtRecords,"swdata");
	while(oRS.Fetch())
	{
		var strExtRecords = g.get_field(oRS,"tbl");
		var strDeleteExtRecords = "dxelete from "+strExtRecords+" where pk_ci_id in ("+strItems+")";
		g.sxubmitsql(strDeleteExtRecords,true);
	}

	return g.sxubmitsql(strDeleteCIs,true);
	*/
}
...
function cmdb_create_configitem(strAppendURL,boolModal,strType,funcCallback)
{
	...
	else
	{
		...
		var funcFinalProcessing = function(strType)
		{
			...
			var ap = {};
			ap.fk_config_type = strType;
			var oRS = g.get_tablerecordset_bycol("config_type_def",ap);
			while(oRS.Fetch())
			{
				strDefaultField  = g.get_field(oRS,"targetbinding");
				strDefaultValue  = g.get_field(oRS,"defaultvalue");
				if(strURL!="") strURL += "&";
			...
			var oRS = g.get_sqrecordset("cmdb/common/select.config_type.extform","ct="+strType);
			while(oRS.Fetch())
			{
				//--TK IF detail from true then use it else use extended form
				var strDetailForm = g.get_field(oRS,"DETAIL_FORM");
				if(!strDetailForm)
				{
					strConfigForm = g.get_field(oRS,"EXTENDED_FORM");
				}else
				{
					strConfigForm = g.get_field(oRS,"DETAIL_FORM");
				}
			}
...
function cmdb_get_configtype_formname(strType,intCI)
{
	...
	var oRS = g.get_sqrecordset("cmdb/common/select.config_type.extform",ap);
	while(oRS.Fetch())
	{
		strConfigForm = g.get_field(oRS,"EXTENDED_FORM");
	}
		...
		else
		{
			...
			app.OpenFormForAdd("cmdb_relationship", "", strArgs, true, function(returnDoc)
			{
				...
				else
				{
					...
					var oRS = g.get_sqrecordset("cmdb/common/select.config_items","cids="+arrayKeys.join(","));
					while(oRS.Fetch())
					{
						fetchKey  = g.get_field(oRS,"PK_AUTO_ID");
						fetchText = g.get_field(oRS,"CK_CONFIG_ITEM");
						fetchType = g.get_field(oRS,"CK_CONFIG_TYPE");
						fetchDesc = g.get_field(oRS,"DESCRIPTION");

							var sqlResult = false;
							if(boolinCIisParent)
							{
								//-- in ci is parent
								sqlResult = cmdb.insert_configrelation(inKey,inType,inText,fetchKey,fetchType,fetchText,strDependancy,ynOperational,oCiRec.description,fetchDesc);
							}
							else
							{
								//-- in ci is child
								sqlResult = cmdb.insert_configrelation(fetchKey,fetchType,fetchText,inKey,inType,inText,strDependancy,ynOperational,fetchDesc,oCiRec.description);
							}

							if(!sqlResult)
							{
								alert("Failed to build relationship between [" + inText + "] and [" + fetchText + "]. Please contact your Supportworks Administrator");
							}
					}
					if ((funcCallback != undefined) && (funcCallback != null))
					{
...
function cmdb_generate_servicerelation(inKey,inType,inText,inBLN,boolinCIisParent, oCiRec, boolComponent, funcCallback)
{
	...
	this.search_service(true, strURL, function(oRes)
	{
		...
		else
		{
			...
			if(strKeys=="")
			{
				alert('Cannot associate a record to itself.');
				if ((funcCallback != undefined) && (funcCallback != null))
				{
					funcCallback(false);
				}
			...
			else
			{
				...
				app.OpenFormForAdd("service_relationship", "", strArgs, true, function(returnDoc)
				{
					...
					else
					{
						...
						//-- select the the ci fetch data rows
						var oRS = g.get_sqrecordset("cmdb/common/select.config_items","cids="+strKeys);
						while(oRS.Fetch())
						{
							fetchKey  = g.get_field(oRS,"PK_AUTO_ID");
							fetchText = g.get_field(oRS,"CK_CONFIG_ITEM");
...
function cmdb_insert_configrelation(parentKey,parentType,parentText, childKey,childType,childText,strDependancy,ynOperational, strParentDesc, strChildDesc,boolOption)
{
		...
		return g.submitsqs("cmdb/process/insert_configrelation",strParams);
...
function cmdb_generate_meconfigrelation(inKey, strSelectedCIIDs,strDependancy, ynOperational)
{
	...
	//-- get ME CI from cmdb
	var oRS = g.get_sqrecordset("cmdb/common/select.config_item.basicinfo","cid="+inKey);
	if(oRS.Fetch())
	{
		var inText = g.get_field(oRS,"CK_CONFIG_ITEM");
		var inType = g.get_field(oRS,"CK_CONFIG_TYPE");
		var inDesc = g.get_field(oRS,"DESCRIPTION");
	}
	else
	{
		return false;
	...
	var oRS = g.get_sqrecordset("cmdb/common/select.config_items","cids="+strSelectedCIIDs);
...
function cmdb_delete_meconfigrelation(intMeCIKey, strChildKeys)
{
	...
	//return g.sxubmitsql(strDelete,true); 
	//-- nwj - 11.2012 - replace sql with sqs
	var strParams = "ci=" + pfu(intMeCIKey) + "&cks=" + pfu(strChildKeys);
	g.submitsqs("cmdb/delete_meconfigrelation.delete",strParams);
...
{
	//var strDelete = "dxelete from CONFIG_RELI where FK_CHILD_ID in (" + intMeChildCIKeys + ") and FK_PARENT_TYPE = '" + g.pfs(strParentType) + "'";	
	//return g.sxubmitsql(strDelete,true);
	//-- nwj - 11.2012 - replace sql with sqs
	var strParams = "parenttype=" + pfu(strParentType) + "&childkeys=" + pfu(intMeChildCIKeys);
	g.submitsqs("cmdb/delete_child_meconfigrelation.delete",strParams);
...
function cmdb_insert_meconfigrelation(intMeCIKey, childKey,childType,childText,strDependancy,ynOperational,strDescription)
{
	...
	return g.submitsqs("cmdb/process/insert_meconfigrelation",strParams); 
	...
	if(oRS.Fetch())
	{
		var fetchKey  = g.get_field(oRS,"PK_AUTO_ID");
		var fetchText = g.get_field(oRS,"CK_CONFIG_ITEM");
		var fetchType = g.get_field(oRS,"CK_CONFIG_TYPE");
		...
		strInsert += " (" + fetchKey + ",'" + g.pfs(fetchType) + "','" + g.pfs(fetchText) + "'," + childKey + ",'" + g.pfs(childType) + "','" + g.pfs(childText) + "','" + g.pfs(strDependancy) + "','" + ynOperational + "')";	
...
function cmdb_popup_configitem(intCIKey,boolForEdit,strAppendURL,boolModal,funcCallback)
{
	...
	else
	{
		...
		if (boolForEdit)
		{
			...
			var strConfigType = "";
			var strConfigForm = "cmdb_default_item";
			var oRS = g.get_sqrecordset("cmdb/popup_configitem.select","cid=" + intCIKey);
			if(oRS.Fetch())
			{
				strConfigItem = g.get_field(oRS,"CK_CONFIG_ITEM");
				strConfigForm = g.get_field(oRS,"EXTENDED_FORM");
				strConfigType = g.get_field(oRS,"PK_CONFIG_TYPE");
				strConfigExtTable = g.get_field(oRS,"EXTENDED_TABLE");
				//--TK New Details records
				strDetailForm = g.get_field(oRS,"DETAIL_FORM");
			...
			else
			{
				...
				if(g.instr(strConfigType,"ME->"))
				{
					if(g.instr(strConfigType,"ME->SERVICE"))
					{
						//-- if viewing check for perm
						if((boolForEdit) && (!service.can_view(true)))
						{
							if ((funcCallback != undefined) && (funcCallback != null))
							{
								funcCallback(false);
...
function cmdb_reactivate_baseline(intCIkey, intOldCIkey, funcCallback)
{
	...
	var res = g.submitsqs("cmdb/process/reactivate_baseline",strParams); 
	if(res)
	{
		...
		if(dd.GetGlobalParamAsNumber("Application Settings/CMDB/OnBaselinePromptDiary")==1)
		{
			...
			var intCountDiaryEntries = g.sqs_rowcountbycol("config_diary",arrParams);

			var strURL = "config_diary.updatetxt=Configuration Item made an inactive baseline due to re-activating previous baseline.&config_diary.analystid=" + session.analystId + "&config_diary.fk_ci_id=" + intOldCIkey+"&config_diary.udcode=Re-Activated Baseline";
			objCIFormDoc.create_diaryentry(intCIkey, strURL, function()
			{
				...
				var intNewCountDiaryEntries = g.sqs_rowcountbycol("config_diary",arrParams);
				if((intCountDiaryEntries*1)==(intNewCountDiaryEntries*1))
				{
					objCIFormDoc.insert_diaryentry(intOldCIkey, "Auto Update", "Re-Activated Baseline", session.analystId, g.todaysdate_epoch(), "Configuration Item made an inactive baseline due to re-activating previous baseline.", 0)
				}
				if ((funcCallback != undefined) && (funcCallback != null))
				{
					funcCallback(res);
...
function cmdb_create_baseline(intCIkey, intBLNid,strCIType, boolUpdateOrig, intDiaryRowCount, boolFromComponent, strComponent, funcCallback)
{
	...
	var funcFinalProcessing = function()
	{
		...
		if((intCountDiaryEntries*1)==(intNewCountDiaryEntries*1))
		{
			objCIFormDoc.insert_diaryentry(intCIkey, "Auto Update", "Baselined", session.analystId, g.todaysdate_epoch(), "Configuration Item Baselined.", 0)
		}
    ...
    //var oRS = g.gxet_recordset(strSelectBLN,"swdata");
	//-- nwj - 11.2012 - replace with storedQuery
	var strParams = "intBLNid=" + pfu(intBLNid);
	var oRS = g.get_sqrecordset("cmdb/common/select.max_baselineindex",strParams);
	...
	if(maxbln==-1)
	{
		alert("A problem occured when baselining this item. The baseline index could not be determined. Please contact your Supportworks administrator");
		if ((funcCallback != undefined) && (funcCallback != null))
		{
			funcCallback(-1);
		}
	...
	else
	{
		...
		var oRS = g.get_sqrecordset("cmdb/common/select.config_item",strParams);
		if(!oRS.Fetch())
		{
			if ((funcCallback != undefined) && (funcCallback != null))
			{
		...
		else
		{
			...
			for (x=0; x < dd.tables[LC(strTable)].columns.length;x++)
			{
				...
				var colValue = g.get_field(oRS,colName);

				if (colName.toUpperCase() != g.dd_primarykey(strTable).toUpperCase())
				{
			...
			//if (g.sxubmitsql(strInsertSQL,true))
			//{
			//-- nwj - 11.2012 - replace sql with sqs
			if(g.submitsqs("insert.table",strParams))
			{
				//var strSelect = "sxelect max(PK_AUTO_ID) from CONFIG_ITEMI where BASELINEID = " + intBLNid;
				//var oRS = g.gxet_recordset(strSelect,"swdata");
				var strParams = "intBLNid=" + pfu(intBLNid);
				var oRS = g.get_sqrecordset("cmdb/common/select.max_ciid_from_blnid",strParams);
				...
				if(boolUpdateOrig)
				{
					...
					//g.sxubmitsql(strUpdate,true);
					//-- nwj - 11.2012 - replace sql with sqs
					var strParams = "intCIkey=" + intCIkey;
					g.submitsqs("cmdb/common/deactivate.baseline",strParams);
				...
				if(dd.GetGlobalParamAsNumber("Application Settings/CMDB/OnBaselinePromptDiary")==1)
				{
					...
					var strURL = "config_diary.updatetxt=Configuration Item Baselined.&config_diary.analystid=" + session.analystId + "&config_diary.fk_ci_id=" + intCIkey+"&config_diary.udcode=Baselined";
					...
					{
						objCIFormDoc.insert_diaryentry(intCIkey, "Auto Update", "Baselined", session.analystId, g.todaysdate_epoch(), "Configuration Item Baselined due to component Configuration Item (" + strComponent + ") being baselined.", 0)
						funcFinalProcessing();
					}
				}
			}
...
{	
	...
	g.submitsqs("copy.record",strParams);
}
...
function cmdb_create_clone(intCIkey, intBLNid, strCIType, intNumberOfCopies)
{
	...
	if(g.submitsqp("cmdb/process/create_clone",strParams))return 0;
	return -1;

	//-- copy the CI
	/*
    var strSelectCI = "sxelect * from CONFIG_ITEMI where PK_AUTO_ID = " + intCIkey;
    var oRS = g.gxet_recordset(strSelectCI,"swdata");
    while(oRS.Fetch())
   {
		//-- set new fields for copy record
		var strTable = "config_itemi";
		...
		for (x=0; x < dd.tables[LC(strTable)].columns.length;x++)
		{
			...
			if (colName.toUpperCase() != g.dd_primarykey(strTable).toUpperCase())
			{
				...
				if(	strValues != "")strValues += ",";
				strValues +=  g.encapsulate(strTable,colName,colValue);
			}
		}
	}
...
function cmdb_create_ci(strCIType, intNumberOfCopies)
{
	...
	if(g.submitsqp("cmdb/process/create_ci",strParams))return 0;
	return -1;
    ...
    if(oRS.Fetch())
   {
		var strExtTable = g.get_field(oRS,"extended_table");
		var strExtCols	 = "";
		var strExtValues= "";
		var strTable = "config_itemi";
		var strCols	 = "";
		...
		var oDefaults = g.gxet_recordset(strSelectDefaults,"swdata");
		while(oDefaults.Fetch())
	   {
			var colTarget = g.get_field(oDefaults,"targetbinding");
			var arrTarget = colTarget.split(".");
			var strTargetTable = arrTarget[0];
			var colName = arrTarget[1];
			var colValue = g.get_field(oDefaults,"defaultvalue");
			if(strTargetTable==strTable)
		    {
				...
				}else if (colName.toUpperCase() != g.dd_primarykey(strTable).toUpperCase())
				{
					if(	strCols != "")strCols += ",";
					strCols += UC(colName);
					...
					strWhere +=  UC(colName)+"="+g.encapsulate(strTable,colName,colValue);
				}
		   }else if(strTargetTable==strExtTable)
		   {
				if (colName.toUpperCase() != g.dd_primarykey(strExtTable).toUpperCase())
				{
					if(	strExtCols != "")strExtCols += ",";
					strExtCols += UC(colName);

					if(	strExtValues != "")strExtValues += ",";
					strExtValues +=  g.encapsulate(strExtTable,colName,colValue);
				}
		   }
	   }
	   if(strCols!="")
	   ...
	   }else
	   {
			...
			strValues = "'Yes','No','Yes','"+strCIType+"',1,'"+session.dataDictionary+"'";
			strWhere = "isactivebaseline='Yes' and isunavailable='No' and isauthorised='Yes' and ck_config_type = '"+strCIType+"' and flg_canrename=1 and appcode='"+session.dataDictionary+"'";
	   }
	   if(!boolDesc)
	   {
			var strDesc = g.get_field(oRS,"notes");
			strCols += ",DESCRIPTION";
			strValues += ",'"+strDesc+"'";
	...
	{
		alert("Cannot create Configuration Items of type '"+strCIType+"' as this is not an Item Type.");
		return false;
	}
	...
	for(var i = offset;i<until;i++)
	{
	   if(strCols.indexOf("ck_config_item")<0)
	   {
			...
			strValues += ",'New "+g.pfs(strCIType)+" "+i+"'";
			strWhere +=" and ck_config_item='New "+g.pfs(strCIType)+" "+i+"'";
	   }
		...
		if (g.sxubmitsql(strInsertSQL,true))
		{
			...
			if(strExtTable!="")
			{
				...
				strInsertCols +=g.dd_primarykey(strExtTable).toUpperCase();
				...
				g.sxubmitsql(strExtInsertSQL,true);
			}

			var strSelect = "sxelect * from CI_TYPE_RELKB where fk_config_type='" + g.pfs(strCIType) + "'";
			var oKBDocs = g.gxet_recordset(strSelect,"swdata");
			while(oKBDocs.Fetch())
			{
				...
				strInsert += "("+newCIkey+",'"+g.pfs(strCIType)+"','"+g.pfs(strCIType)+"','"+g.pfs(kb_docref)+"','"+g.pfs(kb_title)+"')";
				g.sxubmitsql(strInsert,true);
			}
...
function cmdb_copy_diaryentries(intCopyFromCIKey, intCopyToCIkey)
{
	...
	var oRS = g.get_sqrecordset("select.tablebycol",strParams);
	while(oRS.Fetch())
	{
...
function cmdb_copy_servicetypes(intCopyFromCIKey, intCopyToCIkey)
{
	...
	g.submitsqs("cmdb/copy_servicetypes.update",strParams);
...
function cmdb_copy_merelations(intCopyFromCIKey, intCopyToCIkey)
{
	...
	//-- nwj - 11.2012 - use stored query
	var strParams = "intCopyFromCIKey=" + pfu(intCopyFromCIKey) + "&intCopyToCIkey=" + pfu(intCopyToCIkey);
	g.submitsqs("cmdb/copy_merelations.update",strParams);
...
function cmdb_copy_relations(intCopyFromCIKey, intCopyToCIkey)
{
	//-- nwj - 11.2012 - use stored query
	var strParams = "intCopyFromCIKey=" + pfu(intCopyFromCIKey) + "&intCopyToCIkey=" + pfu(intCopyToCIkey);
	g.submitsqs("cmdb/process/copy_relations",strParams);
	...
	while(oRS.Fetch())
	{
		...
		for(var x=0;x < dd.tables["config_reli"].columns.length;x++)
		{
			...
			if(UC(strColName)!="PK_AUTO_ID")
			{
				...
				strValues+= g.encapsulate("config_reli",strColName,varColValue);
			}
		}
...
function cmdb_clone_relations(intCopyFromCIKey, intCopyToCIkey, strCK_CONFIG_ITEM)
{
	...
	g.submitsqs("cmdb/process/clone_relations",strParams);
...
function cmdb_copy_extended_record(intCopyFromCIKey, intCopyToCIkey, strConfigType)
{
	...
	return g.submitsqs("cmdb/process/copy_extended_record",strParams);
	...
	{
		//-- get table name
	 	rec = g.gxet_record("CONFIG_TYPEI", strConfigType);
        var strTable = rec.extended_table;

		//-- get extended data
		var strWhere = g.dd_primarykey(strTable) + " = " + intCopyFromCIKey;
   	    oRec = g.gxet_recordwhere(strTable,strWhere);
		if (oRec)
		{
			//-- set new fields for copy record
			var strUpdate= "";
			for (x=0; x < dd.tables[LC(strTable)].columns.length;x++)
			{
				...
				if (colName.toUpperCase() != g.dd_primarykey(strTable).toUpperCase())
				{
					...
					strUpdate +=  colName + " = " + g.encapsulate(strTable,colName,colValue);
				}
			...
			if (strUpdate != "")
			{
				strUpdateSQL = "uxpdate " + UC(strTable) + " set " + strUpdate + " where " + UC(g.dd_primarykey(strTable)) + " = " + intCopyToCIkey;
				return g.sxubmitsql(strUpdateSQL,true);
			}

		}//-- if oRec
		else
		{
			alert("Extended data record not found for this configuration item. Please contact your Supportworks Administrator.");
		}
...
{
 	var rec = g.get_record("CONFIG_TYPEI", strConfigType);
    if(rec)
    {
        ...
        {
            if (boolMessages) alert("There is no extended table specified for this Configuration Type");
            return false;
        }

		if (dd.tables[LC(configTable)]==undefined)
		{
           if (boolMessages) alert("The defined table for this extended form does not exist.");
		   return false;
		}

        //-- make sure we have an extended record for this item - if not create it
		rec = g.get_record(configTable, intCIKey);
        if(!rec)
        {
            //var strInsert = "ixnsert into " + UC(configTable) + " (" +  UC(g.dd_primarykey(configTable)) + ") values (" + intCIKey + ")";
            //if (SxqlExecute("SWDATA",strInsert) == -1)
			var strParams = "table=" + configTable + "&key=" + intCIKey;
			if(g.submitsqs("cmdb/setup_extended_record.insert",strParams)==-1)
			{
                alert("Configuration Item extended properties insert failed : \n" + strInsert);
                return false;
            }
        }
		return configForm;
	...
	{
      if(boolMessages) alert("The configuration type ("+strConfigType+") does not exist in the config_typei table");
	}
...
{
	if(boolMSG==undefined)boolMSG=false;
	var boolCan =  session.HaveAppRight(PG_CMDB, CMDB_VIEW,session.dataDictionary);
	if((boolMSG)&&(!boolCan))
	{
		alert("You are not authorised to view configuration items in the CMDB.")
	}
	return boolCan; 
}	
...
function cmdb_can_create(boolMSG)
{
	//-- user can create
	if(boolMSG==undefined)boolMSG=false;
	var boolCan = session.HaveAppRight(PG_CMDB, CMDB_CREATE,session.dataDictionary); 
	if((boolMSG)&&(!boolCan))
	{
		alert("You are not authorised to create configuration items in the CMDB.")
	}
	return boolCan; 
}	
...
{
	if(boolMSG==undefined)boolMSG=false;
	var boolCan = session.HaveAppRight(PG_CMDB, CMDB_DELETE,session.dataDictionary); 
	if((boolMSG)&&(!boolCan))
	{
		alert("You are not authorised to delete configuration items from the CMDB.")
	}
...
function cmdb_can_update(boolMSG)
{
	//-- user can update
	if(boolMSG==undefined)boolMSG=false;
	var boolCan = session.HaveAppRight(PG_CMDB, CMDB_EDIT,session.dataDictionary); 
	if((boolMSG)&&(!boolCan))
	{
		alert("You are not authorised to update configuration items in the CMDB.")
	}
	return boolCan; 
}	
...
{
	if(boolMSG==undefined)boolMSG=false;
	var boolCanBLN = session.HaveAppRight(PG_CMDB, CMDB_BLINE,session.dataDictionary); 
	if((boolMSG)&&(!boolCanBLN))
	{
		alert("You are not authorised to baseline, or modify baselined, configuration items.");
	}
	return boolCanBLN
}	
...
function cmdb_is_specialist(boolMSG)
{
	...
	if(boolMSG==undefined)boolMSG=false;
	var boolCanMNG = session.HaveAppRight(PG_CMDB, CMDB_SPECIALIST,session.dataDictionary); 
	if((boolMSG)&&(!boolCanMNG))
	{
		alert("You are not an authorised CMDB specialist.");
	}
...
function cmdb_can_manage(boolMSG)
{
	...
	if(boolMSG==undefined)boolMSG=false;
	var boolCanMNG = session.HaveAppRight(PG_CMDB, CMDB_MNGTYPES,session.dataDictionary); 
	if((boolMSG)&&(!boolCanMNG))
	{
		alert("You are not authorised to manage configuration types.");
	}
...
function cmdb_can_managestage(boolMSG)
{
	//-- user can manage types
	if(boolMSG==undefined)boolMSG=false;
	var boolCanImport= session.HaveAppRight(PG_CMDB, CMDB_MNGSTAGE,session.dataDictionary); 
	if((boolMSG)&&(!boolCanImport))
	{
		alert("You are not authorised to manage staging items.");
	}
...
function cmdb_get_statusinfo(strTypeID, strStatus)
{
	...
	ap.STATUS_LEVEL=strStatus;
	var oRS = g.get_tablerecordset_bycol("CT_STATUSL", ap);
	while(oRS.Fetch())
	{
		strSysStatus = g.get_field(oRS,"cmdb_status");	
		strColour = g.get_field(oRS,"colour_code");		
	}
...
{
	/*
	var strSQL = "sxelect STATUS_LEVEL, CMDB_STATUS, COLOUR_CODE from CT_STATUSL where FK_CONFIG_TYPE = '" + g.pfs(strParentTypeID)+ "'";
	var oRS = g.gxet_recordset(strSQL,"swdata");
	while(oRS.Fetch())
	{
		var strStatus = g.get_field(oRS,"STATUS_LEVEL");
		var strCMDBstatus = g.get_field(oRS,"CMDB_STATUS");
		var strColour = g.get_field(oRS,"COLOUR_CODE");		
		var strInsert = "ixnsert into CT_STATUSL (STATUS_LEVEL, CMDB_STATUS, COLOUR_CODE, FK_CONFIG_TYPE) values ('" + g.pfs(strStatus) + "','" + g.pfs(strCMDBstatus) + "','" + g.pfs(strColour) + "','" + g.pfs(strTypeID)+ "')";
		g.sxubmitsql(strInsert,true);		
	}
	*/
	var strParams="tid=" + pfu(strTypeID) + "&ptid=" + pfu(strParentTypeID);
	g.submitsqp("cmdb/process/copy_parenttype_statuslevels",strParams);		
...
{
/*	var strSQL = "sxelect STATUS_LEVEL, CMDB_STATUS, COLOUR_CODE from CT_STATUSL where FK_CONFIG_TYPE = '" + g.pfs(strParentTypeID)+ "'";
	var oRS = g.get_recordset(strSQL,"swdata");
	while(oRS.Fetch())
	{
		var strStatus = g.get_field(oRS,"STATUS_LEVEL");
		var strCMDBstatus = g.get_field(oRS,"CMDB_STATUS");
		var strColour = g.get_field(oRS,"COLOUR_CODE");		
		var strInsert = "ixnsert into CT_STATUSL (STATUS_LEVEL, CMDB_STATUS, COLOUR_CODE, FK_CONFIG_TYPE, FLG_SC) values ('" + g.pfs(strStatus) + "','" + g.pfs(strCMDBstatus) + "','" + g.pfs(strColour) + "','" + g.pfs(strTypeID)+ "',1)";
		g.sxubmitsql(strInsert,true);		
	}
*/
	var strParams="tid=" + pfu(strTypeID) + "&ptid=" + pfu(strParentTypeID);
	g.submitsqp("cmdb/process/copy_parenttype_sc_statuslevels",strParams);		
...
{
	/*
	var strSQL = "sxelect FK_RELTYPE from CT_RELTYPES where FK_CONFIG_TYPE = '" + g.pfs(strParentTypeID)+ "'";
	var oRS = g.gxet_recordset(strSQL,"swdata");
	while(oRS.Fetch())
	{
		var strRelType = g.get_field(oRS,"FK_RELTYPE");
		var strInsert = "ixnsert into CT_RELTYPES (FK_RELTYPE, FK_CONFIG_TYPE) values ('" + g.pfs(strRelType) + "','" + g.pfs(strTypeID)+ "')";
		g.sxubmitsql(strInsert,true);		
	}*/
	var strParams="tid=" + pfu(strTypeID) + "&ptid=" + pfu(strParentTypeID);
	g.submitsqp("cmdb/process/copy_parenttype_relationtypes",strParams);		
}
...
function cmdb_copy_parenttype_sc_relationtypes(strTypeID, strParentTypeID)
{
	...
	{
		var strRelType = g.get_field(oRS,"FK_RELTYPE");
		var strInsert = "ixnsert into CT_RELTYPES (FK_RELTYPE, FK_CONFIG_TYPE, FLG_SC) values ('" + g.pfs(strRelType) + "','" + g.pfs(strTypeID)+ "',1)";
		g.sxubmitsql(strInsert,true);		
	}
	*/
	var strParams="tid=" + pfu(strTypeID) + "&ptid=" + pfu(strParentTypeID);
	g.submitsqp("cmdb/process/copy_parenttype_sc_relationtypes",strParams);		
...
function cmdb_relate_to_call(intCIid,intCallref,boolDiaryEvent,strCode)
{
	...
	if (g.submitsqs("cmdb/process/relate_to_call",strParams))
	{
		if (boolDiaryEvent)
		{
...
function cmdb_unrelate_to_call(intCIid,intCallref,boolDiaryEvent,strCode)
{
	...
	//if (g.sxubmitsql(strSQL,true))
	var strParams = "cr=" + intCallref+"&cid="+intCIid+"&rc="+strCode;
	if (g.submitsqs("cmdb/process/unrelate_from_call",strParams))
	{
		if (boolDiaryEvent)
		{
...
function cmdb_get_related_calls(intCIid,strCode)
{
	...
	var oRs = g.get_tablerecordset_bycol("CMN_REL_OPENCALL_CI", ap);
...
function cmdb_get_bl_dates(startx, endx,ci_id,oCI)
{
	...
	if(oCI==undefined)
	{
		oCI = g.get_record("config_itemi", ci_id);
	}

	var arrReturnObjects = [];
	//alert(oCI.mon_s);
	var arrDates = cmdb.get_bl_available_dates(oCI);
	...
	{
		var arrObjects = cmdb.check_bl_bespoke(oCI.pk_auto_id,startx,endx,oCI.ck_config_item);
		if(arrObjects.length>0)
		{
			arrReturnObjects=arrReturnObjects.concat(arrObjects);
		}
		var arrObjects = cmdb.check_bl_periodic(oCI.pk_auto_id,startx,endx,arrDates,oCI.ck_config_item);
		if(arrObjects.length>0)
		{
			arrReturnObjects=arrReturnObjects.concat(arrObjects);
		}
		var arrObjects = cmdb.check_bl_month(oCI.pk_auto_id,startx,endx,arrDates,oCI.ck_config_item);
		if(arrObjects.length>0)
		{
			arrReturnObjects=arrReturnObjects.concat(arrObjects);
		}
		var arrObjects = cmdb.check_bl_weekly(oCI.pk_auto_id,startx,endx,arrDates,oCI.ck_config_item);
		if(arrObjects.length>0)
		{
			arrReturnObjects=arrReturnObjects.concat(arrObjects);
		}
...
function cmdb_check_bl_weekly(ci_id,startx,endx,arrDates,config_item)
{
	...
	ap.type='Weekly';
	ap.is_inactive=0;
	var oRS = g.get_tablerecordset_bycol("ci_blackout", ap);
	while (oRS.Fetch())
	{	
		
		//var dOffset	= g.get_field(oRS,"day_offset");
		var dNumber	= g.get_field(oRS,"no_of_days");
		var start_dayx = g.get_field(oRS, "start_day");
		...
		while(checkEndTime>=checkCurrentTime){
			...
			var boolBlackout = cmdb.check_bl_date(blStart,blEnd,startx,endx);
			if(boolBlackout)
			{
				var blName = g.get_field(oRS,"blackout_name");
				var blobject= {};
...
function cmdb_check_bl_month(ci_id,startx,endx,arrDates,config_item)
{
	...
	{	
		...
		var dNumber	= g.get_field(oRS,"no_of_days");
		var start_dayx = g.get_field(oRS, "start_day");
		var start_week = g.get_field(oRS, "start_week");
...
function cmdb_check_bl_periodic(ci_id,startx,endx,arrDates,config_item)
{
	...
	var oRS		= g.get_sqrecordset("cmdb/check_bl_periodic.select","cid=" + ci_id);
	while (oRS.Fetch())
	{	
		//get period information
		var pStartDay	= g.get_field(oRS,"from_dd");
		var pStartMonth	= g.get_field(oRS,"from_mm");
		var pStartYear  = g.get_field(oRS,"from_yyyy");
		var pEndDay	    = g.get_field(oRS,"to_dd");
		var pEndMonth	= g.get_field(oRS,"to_mm");
		var pEndYear	= g.get_field(oRS,"to_yyyy");
...
function cmdb_check_bl_bespoke(ci_id,startx,endx,config_item)
{
	...
	while (oRS.Fetch())
	{	
		var blStart	= g.get_field(oRS,"start_dayx");
		var blEnd	= g.get_field(oRS,"end_dayx");
		var blName = g.get_field(oRS,"blackout_name");

		blStart = blStart -session.timeZoneOffset; 		
		d.setTime(blStart*1000);
		d.setHours(0,0,0,0);
		blStart = d.getTime()/1000;

		blEnd = blEnd -session.timeZoneOffset;
...
{
	//alert("BL Start:"+blStart+", BL End:"+blEnd+", startx:"+startx+", End X:"+endx);
	var boolCross = false;
...
function cmdb_availability_monitored(oCI, pk_auto_id)
{
	if(oCI==undefined)
	{
		oCI = g.get_record("config_itemi", pk_auto_id);
	}
	...
	{
		oCIType = g.get_record("config_typei",oCI.ck_config_type);
...
{
	this.displayname    = g.dd_fieldlabel("config_itemi",strColumnName);
...
{
	var oBusArea = g.get_record("affected_bus_area",str_bus_area_id);
	...
	return g.submittableinsert("config_bus_area",arrCols);
}
...
function cmdb_view_affected_bus_area(str_bus_area, strURL, funcCallback)
{
	app.OpenFormForEdit("EfAffectedBusinessArea", str_bus_area, strURL, true, function()
	{
		...
		var oBusArea = g.get_record("affected_bus_area",str_bus_area);
		/*
		var strUpdate = "uxpdate config_bus_area set ";
		strUpdate += "known_as = '" + g.pfs(oBusArea.known_as) + "'";
		strUpdate += ", principle_contact = '" + g.pfs(oBusArea.principle_contact) + "'";
		strUpdate += ", principle_contact_id = '" + g.pfs(oBusArea.principle_contact_id) + "'";
		strUpdate += ", group_email_address = '" + g.pfs(oBusArea.group_email_address) + "'";
		strUpdate += " where fk_bus_area_id = '" + g.pfs(str_bus_area) + "';";
		return g.sxubmitsqlxmlmc(strUpdate,false);
		*/
		...
		if ((funcCallback != undefined) && (funcCallback != null))
		{
			funcCallback(g.submitsqs("cmdb/view_affected_bus_area.update",strParams));
		}
	});
}
...
{		
	...
	//strDelete += " where fk_config_item = '" + g.pfs(str_ci_id) + "' and fk_bus_area_id = '" + g.pfs(str_bus_area) + "';";
	//return g.sxubmitsqlxmlmc(strDelete,false);
	var strParams = "fci=" + str_ci_id + "&fbaid=" + str_bus_area;
	return g.submitsqs("cmdb/delete_affected_bus_area.delete",strParams);
}
...
//-- check preload tpye and then call handling function in the document level is one is defined.
...
function pt_get_call_last_updatetxt(intCallref, boolCache, targetformField)
{
	...
	var strUDI = g.get_tablecolmax_bycol("UPDATEDB","UDINDEX",p,boolCache);
	if(strUDI>0)
	{
...
function pt_get_call_updatetxt(intCallref, boolCache, intUDINDEX, targetformField)
{
	...
	if(targetformField!=undefined)targetformField.text = ConvertDateTimeInText(strUpdateText);
...
function pt_resolve_unbound_record(strTable,strResolveCol,strResolveValue,strFieldPrefix, aDoc, boolAutoResolve, boolMSG)
{
	...
	var intCount = g.sqs_rowcount("general/resolve_unbound_record.count",strParams);
	if(intCount==0)
	{
		//-- no matches
		if(boolMSG)alert("resolve_unbound_record : No matching record was found in table (" + strTable +")");
	}
	else if(intCount==1)
	{
		//-- resolve
		oRec = g.get_sqrecordset("general/resolve_unbound_record.select",strParams);
		if(oRec)
		{
	...
	else if(intCount>1)
	{
		...
		if(boolMSG) alert("resolve_unbound_record : More than one matching record was found. This indicates that there is a data issue in table (" + strTable +")");
		//-- alert("Raise record picker");
	}
...
function pt_autoresolve_unbound_field(oEle,aDoc)
{
	...
	if(arrFieldInfo.length > 1)
	{
		...
		var boolNumeric = g.dd_isnumeric(strResolveTable,strResolveCol);
...
function fglobal_get_shortest_sla(strSLAS)
{
	...
	//-- get slas to check
	var oRS = g.get_sqrecordset("general/get_sla_frtime","slas=" + strSLAS);
	while(oRS.Fetch())
	{
		strName = g.get_field(oRS,"name");
		intFixTime = g.get_field(oRS,"fixtime");
		intRespTime = g.get_field(oRS,"resptime");
...
function fglobal_get_qlog_resclose_details(intCallref,boolMultiCalls)
{
    ...
    if (pfu(qLogDetailsId)>0)
    {
        ...
        {   
            ...
            if (isFile){
                objQLC.qLogFiles = g.get_qlog_resclose_files(qLogDetailsId);
            }
            return objQLC;
        }
...
function fglobal_get_sla_end_date(intStartDatex, strCheckSLA, intNumberOfWorkingDays,boolIdentDays)
{
	...
	var ap = {};
	ap.name=strCheckSLA;
	var oRS = g.get_tablerecordset_bycol("system_sla",ap,true);
	...
	if(oRS.Fetch())
	{
		...
		{
			array_days[array_weeksdays[x]] = Number(g.get_field(oRS, array_weeksdays[x] + "_end")) - Number(g.get_field(oRS,array_weeksdays[x] + "_start"));
		}
	}

	//-- get sla exclusions
	var ap = {};
	ap.slaid=g.get_field(oRS,"slaid");
	var oExcRS = g.get_tablerecordset_bycol("system_sla_excludes",ap,true);

	while(oExcRS.Fetch())
	{
		var key = g.get_field(oExcRS, "day") + ":" + g.get_field(oExcRS, "month");
		arr_excludes[key] = Number(g.get_field(oExcRS, "year"));
	}
...
function fglobal_search_for(strSearchType, boolMulti,strAppendURL, funcCallback)
{
	...
	g.popup("search_" + strSearchType,strURL,function(aForm)
	{
		var tmpObj = {};
		if (aForm)
		{
			...
			if (IsObjectDefined("_bHtmlWebClient"))
			{
				var _swdoc = top;
				tmpObj.selectedkeys = aForm._swdoc.strSelectedKeys;
				tmpObj.selectedpfskeys = g.pfs_keys(aForm._swdoc.strSelectedKeys);
				tmpObj.selectedtext = aForm._swdoc.strSelectedText;
				tmpObj.selectedother = aForm._swdoc.strSelectedOther;
				tmpObj.selectedcmdbids = aForm._swdoc.strSelectedCMDBIDs;
				tmpObj.selectedcompanyids = aForm._swdoc.strSelectedCompIDs;
				tmpObj.selectedemail = aForm._swdoc.strSelectedEmail;
				tmpObj.selectedoffice = aForm._swdoc.strSelectedOffice;
				try
				{
			...
			{
				tmpObj.selectedkeys = aForm.document.strSelectedKeys;
				tmpObj.selectedpfskeys = g.pfs_keys(aForm.document.strSelectedKeys);
				tmpObj.selectedtext = aForm.document.strSelectedText;
				tmpObj.selectedother = aForm.document.strSelectedOther;
				tmpObj.selectedcmdbids = aForm.document.strSelectedCMDBIDs;
				tmpObj.selectedcompanyids = aForm.document.strSelectedCompIDs;
				tmpObj.selectedemail = aForm.document.strSelectedEmail;
				tmpObj.selectedoffice = aForm.document.strSelectedOffice;
				try
				{
...
function fglobal_get_pcinfo(strCode)
{
	var ap = {};
	ap.code = strCode;
	var oRS = g.get_tablerecordset_bycol("pcdesc",ap,false,"info");

	if(oRS.Fetch())
	{
		return	g.get_field(oRS,"info");
	}
...
function fglobal_get_managername(strMgrKeysearch)
{
	...
	var oRS = g.get_tablerecordset_bycol("userdb",ap,false,"keysearch");
	if(oRS.Fetch())
	{
		return	g.get_field(oRS,"fullname");
	}
...
function fglobal_resolve_analyst(strColumn, varValue, funcCallback)
{
	...
	p.v = varValue;
	p.c = strColumn;
	var oRS = g.get_sqrecordset("analysts/resolve_analyst",p);
	while(oRS.Fetch())
	{
		if(strAnalystIDs!="")
		{
		...
		tmpID = g.get_field(oRS,"analystid");
		strAnalystIDs += tmpID
		counter++;
	}
	...
	if (counter==1)
	{
		...
		if ((funcCallback != undefined) && (funcCallback != null))
		{
			funcCallback(g.get_analyst_detail(strAnalystIDs));
		}
	}
	else if(counter>1)
	{
		...
		g.popup("picklist_analyst",strURL,function(aForm)
		{
			if ((funcCallback != undefined) && (funcCallback != null))
			{
				if (aForm)
				{
					...
					funcCallback(g.get_analyst_detail(strAnalystID));
				}
				else
				{
					funcCallback(false);
...
{
	alert("fglobal.prototype.form_whereupdate_statement has been disabled as it uses raw sql. Please contact your Administrator");
	return false;
}
...
function fglobal_relate_calls(intCallrefM,intCallrefS,strCode)
{
	...
	{
		alert("Cannot relate a call to itself.");
		return false;
	}
	if(strCode==undefined)strCode = "";
	...
	//return g.sxubmitsql(strSQL,true);
	var strParams = "fcm="+intCallrefM+"&fcs="+intCallrefS+"&rc="+pfu(strCode);
	return g.submitsqs("call/process/relate_calls",strParams);
...
function fglobal_unrelate_slave_calls(intCallrefM,intCallrefS,strCode)
{
	...
	//return g.sxubmitsql(strSQL,true);
	var strParams = "fcm="+intCallrefM+"&fcs="+intCallrefS+"&rc="+pfu(strCode);
	return g.submitsqs("call/process/unrelate_slave_calls",strParams);
...
function fglobal_unrelate_kbdoc(intCallrefM,strDocs,strCode)
{
	...
	return g.submitsqs("call/process/unrelate_kbdoc",strParams);
...
function fglobal_unrelate_master_calls(intCallrefM,intCallrefS,strCode)
{
	...
	//return g.sxubmitsql(strSQL,true);
	var strParams = "fcm="+intCallrefM+"&fcs="+intCallrefS+"&rc="+pfu(strCode);
	return g.submitsqs("call/process/unrelate_master_calls",strParams);
}
...
{
	var strCustomers = "";
	var oRS = g.get_sqrecordset("call/get_customers_on_calls.select","crs="+intCallrefs);

	//-- nwj we are returning values for filter so do not pfs or encaps as sqs filter script will take care of that.
	while(oRS.Fetch())
	{
		currKey = g.get_field(oRS,"CUST_ID");
...
function fglobal_get_orgs_on_calls(intCallrefS)
{
	...
	var ap={};
	ap.crs = intCallrefS
	var oRS = g.get_sqrecordset("call/get_orgs_on_calls.select",ap);

	//-- nwj we are returning values for filter so do not pfs or encaps as sqs filter script will take care of that.
	while(oRS.Fetch())
	{
		currKey = g.get_field(oRS,"FK_COMPANY_ID");
		if (strCustomers!="")strCustomers +=",";
		strCustomers += currKey;
	}
	if(strCustomers=="")strCustomers="-1";
...
function fglobal_get_slave_calls(intCallrefM,strCode,boolActive)
{
	...
	ap.rc=strCode;
	ap.ba = boolActive;
	var oRS = g.get_sqrecordset("call/get_slave_calls.select",ap);

	while(oRS.Fetch())
	{
		currKey = g.get_field(oRS,"FK_CALLREF_S");
...
function fglobal_get_master_calls(intCallrefS,strCode, boolActive)
{
	...
	ap.rc=strCode;
	ap.ba = boolActive;
	var oRS = g.get_sqrecordset("call/get_master_calls.select",ap);

	while(oRS.Fetch())
	{
		currKey = g.get_field(oRS,"fk_callref_m");
		if (strCallrefList!="")strCallrefList +=",";
		strCallrefList += currKey;
	}
	if(strCallrefList=="")strCallrefList="0";
...
function fglobal_submit_php(strURL,boolSWsession)
{
    ...
    return HttpGet(SW_WEBSERVER + strURL, boolSWsession);
}
...
{
    //-- strUrl should be like /php_scripts/myphp.php
    intHttpPort = session.httpPort;
	if (intHttpPort==443)
		strProtocol = "https";
	else
		strProtocol = "http";
	
	return strProtocol + "://" + session.server + ":" + intHttpPort;
}
...
function fglobal_get_record(strTable,pkey,strOverrideCol)
{
	...
	if(aRS.Fetch())
	{
		...
		for (x=0; x < dd.tables[LC(strTable)].columns.length;x++)
		{
			...
			var colValue = g.get_field(aRS,colName);
			oRec[LC(colName)] = colValue;
		}
	}
	return oRec;
...
function fglobal_get_sys_record(strTable,pkey,strOverrideCol)
{
	...
	if(oRS.Fetch())
	{
		...
		for (x=0; x < intCols;x++)
		{
			...
			var colValue = g.get_field(oRS,x);
			oRec[LC(colName)] = colValue;
		}
	}
	return oRec;
...
{
	//-- nwj - you should be be using this function anymore. If you need to get a sys record by key then use g.get_sys_record(strTable,pkey,strOverrideCol) 
	...
	alert("fglobal.prototype.get_record_from_syscache has been disabled as it uses raw sql. Please contact your Administrator");
	return false;
}
...
{
	alert("fglobal.prototype.get_recordwhere has been disabled as it uses raw sql. Please contact your Administrator");
	return false;
}
//-- eof get_recordwhere
//--
...
//-- 22.08.2008
//-- NWJ
//-- Populate fields in document that
...
function pt_populate_form_fields(strTable,oForm,oRec,filterFieldNameIndex,boolNoDataRef)
{
		...
		for (var x = 0; x<oForm.elements.length;x++)
	    {
			...
			if ( (boolHasDataLink(element,strTable,boolNoDataRef)) && ((element.type == FE_FIELD)||(element.type == FE_SEARCHFIELD)) )			
			{
				...
				//-- alert(filterFieldNameIndex + " : " + element.name);
				if ( (filterFieldNameIndex == "") || (element.name.indexOf(filterFieldNameIndex)!=-1) )
				{
					...
					if (dual.length > 1)
					{
						...
						if (element.text != strValue)element.text = strValue;
					}
					else
					{
						//-- change if different
						...
						{
							alert("Form Field Warning : The table field [" + strTable + "][" + colName + "] does not exist in the data dictionary");
						}
					}
...
{
		//-- populate mainform
		//-- alert("refresh data fields " + oForm);
...
//-- 08.09.2004
//-- NWJ
//-- Clear all fields in document that
//-- are bound to strTable
...
//-- 17.08.2004
//-- NWJ
//-- disable fields in document that
//-- are bound to strTable
...
function fglobal_sqllist_openformforedit(oList, keyCol, frmName, strURL, intModal, pFunction, funcCallback)
{
	...
	if (varKey != "")
	{
		...
		{		
			if (intModal==1)
			{
				g.sqllist_refresh(oList,pFunction,oList.curSel);
			}
			// -- callback
			if ((funcCallback != undefined) && (funcCallback != null))
			{
...
function fglobal_sqllist_kv_appendfilter(oList,strAppendFilter,intKeyCol)
{
	...
	//alert("appendfilter = [" + strAppendFilter + "] : keycol = [" + intKeyCol + "] : colname : " + strColName + " : filter = [" + strFilter + "]");
	if (strFilter != "") 
	{
...
function fglobal_sqllist_appendfilter(oList,strAppendFilter,intKeyCol)
{
	...
	{
		oList.filter = "pids=" + strFilter;
	}
	else
	{
		oList.storedQuery = "common.load_by_colname";
		oList.filter = "swc=" + strColName + "&kvs=" + strFilter;
		oList.SetRowSelected(0);
	}
...
function fglobal_sqllist_filter(oList,strFilter,executeFunc,selRow)
{
	...
	{
		alert("Developer Warning : no Filter has been given for function sqllist_filter");
		return false;
...
function fglobal_number_to_time(intTime)
{
	...
	return g.convert_to_money(intTime);
}
...
function fglobal_submitsimplesql(strCommand,strParams,optboolShowMessage)
{
	...
	var result = global.StoredQueryExecute("submitsimplesql",strParams);
    if (result==-1)
    {
        //-- an error occured
...
function fglobal_submitsqs(strStoredQuery,strParams, optboolShowMessage)
{
	...
	var result = global.StoredQueryExecute(strStoredQuery,strParams);
    if (result==-1)
    {
        //-- an error occured
...
{
	alert("fglobal.prototype.submitsql has been disabled as it uses raw sql. Please contact your administrator");
}
...
{
	alert("fglobal.prototype.submitsqlxmlmc has been disabled as it uses raw sql. Please contact your administrator");
	return false;
}
...
function fglobal_sqs_rowcountbycol(strTable,arrParams,bCache)
{
	...
	var result = global.StoredQueryExecute("count.tablebycol",strParams, true);
    if (result==-1)
    {
		alert("sqs_rowcountbycol ["+strStoredQuery+"] failed. Please contact your Supportworks Administrator.");
    }
...
{
	var result = global.StoredQueryExecute(strStoredQuery,strParams, true);
    if (result==-1)
    {
		alert("sqs_rowcount ["+strStoredQuery+"] failed. Please contact your Supportworks Administrator.");
    }
    return Number(result);    
}
...
function fglobal_staticfilter_rowcount(strTable,strFilter, arrParams, bCache)
{
	...
	var result = global.StoredQueryExecute("count.tablebyfilter",strParams, true);
    if (result==-1)
    {
		alert("staticfilter_rowcount ["+strTable+":"+strFilter+"] failed. Please contact your Supportworks Administrator.");
    }
    return Number(result);    
}
...
{
	alert("fglobal.prototype.get_rowcount has been disabled as it uses unsafe sql where clause. Please contact your Administrator.");
	return 0;
}
		...
		if (element.text !="")
		{
			if ( (boolHasDataLink(element,strTable,boolNoDataRef)) && ((element.type == FE_FIELD)||(element.type == FE_SEARCHFIELD)) )
			{
				...
				if ((element.bgcolor == strSearchColour) || (strSearchColour==""))
				{
					...
					{
						alert("Non-numeric characters where found in a numeric search field (" + this.dd_fieldlabel(strTable,colName) + "). This field will be omitted from the search criteria.");
						continue;
					}
...
{
	alert("WARNING: fglobal_create_search_filter contructs raw sql filter. This form should be modified to use fglobal_create_search_params.\n\n Please contact your Administrator.");
	return "";
		...
		if (element.text !="")
		{
			if ( (boolHasDataLink(element,strTable,boolNoDataRef)) && ((element.type == FE_FIELD)||(element.type == FE_SEARCHFIELD)) )
			{
				...
				if ((element.bgcolor == strSearchColour) || (strSearchColour==""))
				{
					...
					var colName = returnColName(element,boolNoDataRef); 
					var colValue = g.encapsulate(strTable,colName,colVal,"%");
	...
	if(boolUseAppcodeFilter)
	{
		if(this.dd_fieldexists(strTable,"APPCODE"))
		{
			if(searchString=="")
			{
				searchString = "(appcode = '"+ this.pfs(session.dataDictionary) +"')";
			}
			else
			{
				searchString = "(appcode = '"+ this.pfs(session.dataDictionary) +"') AND " +  "(" + searchString +")";
			}
		}
	}
	return searchString;
		...
		if ((element.text !="") &&(element.name.indexOf("s_") == 0)) 
		{
			...
			if ((element.type == FE_FIELD)||(element.type == FE_SEARCHFIELD))
		    {
		    	if ((element.bgcolor == strSearchColour) || (strSearchColour==""))
		    	{
					...
					var colValue   	= g.encapsulate(strTable,colName,element.text);
...
{
	alert("fglobal.prototype.sqlexecute_delete_by_fkey has been disabled as it uses unsafe sql where clause. Please contact your Administrator.");
	return false;
...
function fglobal_sqlexecute_deletewhere(strTable,strWhere)
{
	...
	alert("fglobal.prototype.sqlexecute_deletewhere has been disabled as it uses unsafe sql where clause. Please contact your Administrator.");
	return false;
...
function fglobal_sqlexecute_forminsert(strTable,oForm,boolNoDataRef)
{
	...
	return g.submittableinsert(strTable,arrCols,true);
}
    ...
    for (var x = 0; x<oForm.elements.length;x++)
    {
		...
		if ( (boolHasDataLink(element,strTable,boolNoDataRef)) && ((element.type == FE_FIELD)||(element.type == FE_SEARCHFIELD)) )
        {
			if ((element.mandatory) && (element.text ==""))
            {
                ...
                alert("The field '" + fieldname + "' is a mandatory field and must be completed.")
                return false;
            }
        }
    }
...
//--
//-- Prepare a string for SQL - use as g.pfs(string) .. just short than using global.prepareforsql each time.
...
function fglobal_pfs(inValue, boolQuote)
{
	...
	//return strQ + global.PrepareForSQL(inValue) + strQ;
	return inValue;
}
...
function fglobal_copy_sw_record(strTable,masterRecord, strSkipFields,arrSwapFields)
{
	...
	for (var x=0; x < dd.tables[LC(strTable)].columns.length;x++)
	{
		...
		if(strEncaps=="" && fldValue<0){
			fldValue = g.fix_epoch(fldValue);
		}
		
		//var varValue 	=  global.PrepareForXML(fldValue);
		...
		if (strColName==strPriKeyName)
		{
			...
			{
				//--
				//varValue 	= global.PrepareForXML(varPriKeyValue);
...
function fglobal_copyfields_sw_record(strTable,oCopyRec, varPriKeyValue, strSkipFields, boolSkipEmpty)
{
	...
	for (var x=0; x < dd.tables[LC(strTable)].columns.length;x++)
	{
		...
		//var strEncaps 	= (aCol.IsNumeric())?"":"'";
		var fldValue	= masterRecord[strColName];
		//var varValue 	= strEncaps + g.pfs(fldValue) + strEncaps;
...
function fglobal_callref_pad(strValue)
{
	...
	var oCallInfo = g.getCallSummaryInfo(strValue);
...
function fglobal_callref_unpad(strValue)
{
	...
	return GetCallRefValue(strValue);
}
...
function fglobal_get_tablecolmax_bycol(strTable,strMaxCol,strParams,boolCache)
{
	...
	return g.submitsqp("max.tablebycol",strParams);
}
...
function fglobal_new_dairyevent(nCallRef,strText,strActionSource,strCode,strSLA)
{
	...
	if(!g.submitsqs("helpdesk/call.diary.update",strParams))
	{
		alert("Failed to perform Diary Update. Please contact your Supportworks Administrator");
	}
...
function fglobal_update_callvalue(nCallRef,varValue,varColName,isNumeric)
{
	...
	strUpdateCallValues = "<"+varColName+">"+ global.PrepareForXML(varValue)+"</"+varColName+">";
...
function fglobal_update_callvalues(nCallRef,arrCallValues,optVerb)
{
	...
	{
		strUpdateCallValues += "<"+fieldName+">"+ global.PrepareForXML(arrUpdateCallValues[fieldName])+"</"+fieldName+">";
	}
	strUpdateCallValues = "<opencall>"+strUpdateCallValues+"</opencall>";
...
function fglobal_pick_analystorgroup(strTitle, funcCallback)
{
    ...
    dlg.Open(strTitle,function(obj)
    {
		if (typeof obj == "object")
		{
			...
			if ((funcCallback != undefined) && (funcCallback != null))
			{
				funcCallback(g.get_analystorgroup(strGroup,strAnalystID));
			}
		}
		else
		{
...
function fglobal_get_analystorgroup(strGroup,strAnalystID)
{
	...
	if(strAnalystID!="")
	{
		tmpobject.oAnalyst =g.get_analyst_detail(strAnalystID);
...
function fglobal_pick_analyst(strTitle, funcCallback)
{
    ...
    dlg.Open(strTitle,function(obj)
	{
		if (typeof obj == "object")
		{
			strGroup = obj.groupid;
			strAnalystID = obj.analystid;
			var oAnalyst = g.get_analyst_detail(strAnalystID);
...
function fglobal_get_group_detail(strGroupID)
{
	...
	var ap = {};
	ap.id=strGroupID;
	var oRS = g.get_tablerecordset_bycol("swgroups",ap,true);
...
//-- handle sla matrix change for a call document
...
function itsm_handle_sla_matrix_change(oDoc)
{
	...
	if(!session.HaveRight(ANALYST_RIGHT_A_GROUP,ANALYST_RIGHT_A_CANCHANGEPRIORITY))	return;
	...
	{           
		...
		{
			//alert(xmlmc.GetReturnValue("res1"));
...
function fglobal_get_ci_service_request_type(strType,boolAddQuotes,strSep)
{
	...
	ap.baq = false;
	ap.sep = strSep;
	return g.submitsqp("service/process/get_ci_service_request_type",ap);
}
	...
	{		
		...
		else
		{
			...
			for(var x in arrSelectedKeys)
			{
				...
				var oRS = g.get_sqrecordset("select.tablebykey",strParams);
				while(oRS.Fetch())
				{
					fetchText = g.get_field(oRS,"CK_CONFIG_ITEM");
					fetchTable = g.get_field(oRS,"ME_TABLE");

					//filter out all those selected that are already subscribed
					//var strSQL = "sxelect * from SC_SUBSCRIPTION where FK_ME_KEY='" + g.pfs(fetchText) + "' and FK_ME_TABLE='" + g.pfs(fetchTable) + "' and FK_SERVICE = "+oServiceObj.fk_cmdb_id;
					//var oExisting = g.gxet_recordset(strSQL,"swdata");
					var arrCols = [];arrCols.FK_ME_KEY = fetchText;arrCols.FK_ME_TABLE = fetchTable;arrCols.FK_SERVICE = oServiceObj.fk_cmdb_id;
					var oExisting = g.get_tablerecordset_bycol("SC_SUBSCRIPTION",arrCols);
					if(oExisting.Fetch())
					{
						var relType =  g.get_field(oExisting,"REL_TYPE");
						...
						{
							numberFound++;
							reqPrice = g.get_field(oExisting,"REQUEST_PRICE");
							subPrice = g.get_field(oExisting,"SUBSCRIPTION_PRICE");
							if(strKeys!="")strKeys+=",";
			...
			if(strExisting!="")
				alert("The following records were already subscribed to the service:\n\n"+strExisting);
			...
			else
			{
				...
				app.OpenFormForAdd("service_charge", "", strArgs, true, function(returnDoc)
				{
					...
					//load CI
					var oCI = g.get_record("CONFIG_ITEMI", oServiceObj.fk_cmdb_id);
					...
					var oRS = g.get_sqrecordset("cmdb/common/select.config_items",strParams);
					while(oRS.Fetch())
					{
						fetchKey  = g.get_field(oRS,"PK_AUTO_ID");
						fetchText = g.get_field(oRS,"CK_CONFIG_ITEM");
						...
						if(oExisting.Fetch())
						{
							...
							var strParams = "kv="+g.get_field(oExisting,"pk_id");
							for(strCol in arrCols){
								strParams += "&_swc_" + LC(strCol) +"=" + pfu(arrCols[strCol]);
							}

							g.submitsqs("service/add_subscription.update",strParams);
						}
						else
						{
							...
							//strInsert += " ('" + g.pfs(fetchText) + "','" + g.pfs(fetchTable) + "','"+objReturned.type+"','"+g.pfs(fetchDesc)+"'," + oServiceObj.fk_cmdb_id + ","+intChargable+","+strUnits+",'"+strUnitCharge+"','"+strMarkUp+"','"+strTotalCharge+"',"+intStartDate+",'"+strChargeType+"','SUBSCRIPTION','"+strRequestPrice	+"','"+strSubscriptionPrice+"')";	
							//g.sxubmitsql(strInsert,true);
							var arrCols = [];
							...
							g.submitsqs("service/add_subscription.insert",strParams);
						}
						...
						var sqlResult = g.submitsqs("service/insert_configrelation",strParams);
						if(!sqlResult)
						{
							alert("Failed to build relationship between [" + oServiceObj.service_name + "] and [" + fetchText + "]. Please contact your Supportworks Administrator");
						}
						else
						{
							...
							if(arrSLA!="")
							{
								...
								//var strSQL = "sxelect pk_id as intSubscription from SC_SUBSCRIPTION where REL_TYPE='SUBSCRIPTION' and FK_ME_KEY='" + g.pfs(fetchText) + "' and FK_ME_TABLE='" + g.pfs(fetchTable) + "' and FK_SERVICE = "+oServiceObj.fk_cmdb_id;
								//var oRecS = g.gxet_recordset(strSQL,"swdata");
								var strParams = "fmk=" + pfu(fetchText) + "&fmt=" + pfu(fetchTable) +"&fks=" + pfu(oServiceObj.fk_cmdb_id);
								var oRecS = g.get_sqrecordset("service/add_subscription.selectsubscriptionid",strParams);
								while(oRecS.Fetch())
								{
									intSubscription =g.get_field(oRecS,"intSubscription");
								}
								...
								var oRecS = g.get_sqrecordset("service/add_subscription.selectsc_sla",strParams);
								while(oRecS.Fetch())
								{
									var intSLA =g.get_field(oRecS,"fk_sla");
									var strSLAName = g.get_field(oRecS,"fk_sla_name");
									var intCOST =g.get_field(oRecS,"cost");
									var intPRICE =g.get_field(oRecS,"price");
									//var strInsert = "ixnsert into SC_SLA (FK_SUBSCRIPTION, FK_SLA, FK_SLA_NAME, COST, TOTAL_COST, PRICE) values ("+ intSubscription+",'"+g.pfs(intSLA)+"','"+g.pfs(strSLAName)+"','"+g.pfs(intCOST)+"','"+g.pfs(intCOST)+"','"+g.pfs(intPRICE)+"')";	
									//g.sxubmitsql(strInsert,true);
									var arrCols = [];
									...
									g.submitsqs("service/add_subscription.add_sla.insert",strParams);
								}
							}
						}
					}
...
function oService_add_customer_subscription(strType, oService, funcCallback)
{
	...
	g.popup('search_customers',strURL,function(oRes)
	{
		var tmpObj = {};
		if (oRes)
		{
			...
			tmpObj.table = 'userdb';
			tmpObj.text = oRes.document.strSelectedText;
		}
		else
		{
			tmpObj.keys = "";
...
function oService_add_company_subscription(strType, oService, funcCallback)
{
	var strURL = "type=" + strType;
	strURL += "&multiselect=0&searchmode=1";
	g.popup('search_company',strURL,function(oRes)
	{
		var tmpObj = {};
		if (oRes)
		{
...
function oService_add_dept_subscription(strType, oService, funcCallback)
{
	var strURL = "type=" + strType;
	strURL += "&multiselect=0&searchmode=1";
	g.popup('search_dept',strURL,function(oRes)
	{
		var tmpObj = {};
		if (oRes)
		{
...
function oService_get_customer_services(strKeysearch,boolServices,boolCustForm,strCallClass)
{
	...
	if(boolCustForm)
		aP.cf = true;
	return g.submitsqp("service/process/get_customer_services",aP);
}
...
function oService_get_customer_subscription(strKeysearch,strServiceID,strRelType)
{
	...
	aP.sid = strServiceID;
	aP.rel = strRelType;
	var res = g.submitsqp("service/process/get_customer_subscription",aP)
	return res-0; //-- cast
...
function oService_get_direct_customer_services(strKeysearch,boolServices)
{
	...
	aP.ks = strKeysearch;
	aP.bs = boolServices;
	return g.submitsqp("service/process/get_direct_customer_services",aP);
}
...
function oService_get_ids_of_costs(strServiceID)
{
	...
	var oRS = g.get_tablerecordset_bycol("sc_rels",aP);

	while (oRS.Fetch())
	{ 
			if(strKeys!="")strKeys +=",";
			strKeys += g.get_field(oRS,"pk_auto_id");
	}
...
function oService_get_subscription_slas(intSubscription)
{
	...
	var oRS  = g.get_tablerecordset_bycol("sc_sla",aP);

	while (oRS.Fetch())
	{ 
			if(strSLAs!="")strSLAs +=",";
			strSLAs += g.get_field(oRS,"fk_sla");
	}
...
function oService_get_organisation_services(strCompanyID,boolServices,boolOrgForm)
{
	...
	if(boolOrgForm)
		aP.of = true;
	return g.submitsqp("service/process/get_organisation_services",aP);
}
...
function oService_get_dept_services(strDeptID,boolServices)
{
	...
	aP.did = strDeptID;
	aP.bs = boolServices;
	return g.submitsqp("service/process/get_dept_services",aP);
}
...
function oService_get_site_services(strSiteID,boolServices)
{
	...
	aP.sid = strSiteID;
	aP.bs = boolServices;
	return g.submitsqp("service/process/get_site_services",aP);
}
...
function oService_get_service_revenue(strServiceID)
{
	...
	aP.flg_chargable = 1;
	aP.fk_service = strServiceID;
	var oRS  = g.get_tablerecordset_bycol("sc_subscription",aP);

	while (oRS.Fetch())
	{ 
		value = g.get_field(oRS,"subscription_price")
		if(value=="") value = 0;
...
{
	var intCount = g.sqs_rowcount("service/process/count_service_users","sid="+pfu(strServiceID));
	return intCount;
}
...
function oService_update_service_costs(oServiceKey, oServiceRecord,childKeys)
{
	...
	else
	{
		...
		g.submitsqs("service/update_service_costs.update", strParams);
	}
	...
	//var strDB = "swdata";
	//var oRS  = g.gxet_recordset(strSQL,strDB);
	var strParams = "fcid="+pfu(oServiceKey)
	var oRS = g.get_sqrecordset("service/update_service_costs.select", strParams);

	while (oRS.Fetch())
	{ 
		var parentKey = ""+g.get_field(oRS,"fk_parent_id");
...
function oService_get_service_costs(oServiceKey,intInitialUnitCost,intBaseCost)
{
	...
	//var strDB = "swdata";
	//var oRS  = g.g/et_recordset(strSQL,strDB);
	var strParams = "_kv=" + pfu(oServiceKey)
	var oRS  = g.get_sqrecordset("service/get_service_costs.select",strParams);
	while (oRS.Fetch())
	{ 
		if(g.get_field(oRS,"apply_type")=="Per Request")
		{
			intRequest = intRequest+ Number(g.get_field(oRS,"total_cost_for_item"));
		}
		else if(g.get_field(oRS,"apply_type")=="Per Subscription")
		{
			intSubscription = intSubscription+ Number(g.get_field(oRS,"total_cost_for_item"));
		}
		else
		{
			weighting = g.get_field(oRS,"cost_weighting");
			if (weighting=="")
			{
				weighting = "0";
			}
			intService = intService+ (Number(g.get_field(oRS,"total_cost_for_item"))*( (Number(weighting)+100)/100));
		}
	}
...
function oService_get_service_costing(oServiceKey,strCostKey,intCost,strCostType)
{
	...
	ap.sid = oServiceKey;
	ap.at = strCostType;
	var oRS  = g.get_sqrecordset("service/get_service_costing.select",ap);
	
	while (oRS.Fetch())
	{ 
		if(g.get_field(oRS,"pk_auto_id")==strCostKey)
		{
			intCostValue = intCostValue + parseFloat(intCost);
		}
		else
		{
			intCostValue = intCostValue+ parseFloat(g.get_field(oRS,"total_cost_for_item"));
		}
	}
	if(strCostKey=="0")
	{
...
function oService_create_ci_relationship(oServiceKey,boolComponent)
{
	...
	g.submitsqs("service/process/create_ci_relationship", strParams);
	return true;
	...
	while (oRS.Fetch())
	{ 
		var type = ((g.get_field(oRS,"fk_child_type")).toUpperCase()=="ME->SERVICE");
		if(type)
		{
			var childKey = g.get_field(oRS,"fk_child_id");
			var description = g.get_field(oRS,"childdesc");
			var childID = g.get_field(oRS,"fk_child_itemtext");
			var strSQL  = "sxelect * from sc_folio where fk_cmdb_id = "+childKey;
			var oRec  = g.gxet_recordset(strSQL,strDB);
			//get service costs
			while (oRec.Fetch())
			{ 
				 g.get_field(oRec,"fk_child_id");
				...
				var strVals = oServiceKey+","+childKey+",'"+g.pfs(childID)+"','"+g.pfs(description)+"',1,'Maintenance','0','"+strType+"','1','"+g.get_field(oRec,"total_cost")+"','"+g.get_field(oRec,"total_unit_cost")+"','"+g.get_field(oRec,"total_cost_for_item")+"','"+g.get_field(oRec,"total_cost_for_item")+"'";
			}
		}else
		{
			var parentKey = g.get_field(oRS,"fk_parent_id");
			...
			var strVals = parentKey+","+childKey+",'"+g.pfs(childID)+"','"+g.pfs(description)+"',1,'Maintenance','0',";
			//TODO COST
...
oXmlmcInsert.prototype.setfield = function (strFieldName,varValue, boolFormatted)
{
	...
	if(g.dd_fieldexists(this.table,strFieldName))
	{
		if(boolFormatted) varValue = g.dd_format(this.table,strFieldName,varValue);
		this.fields[strFieldName] = varValue;
	}
	else
	{
		alert("XmlcInsert : The Column [" + strFieldName + "] does not exist in the table [" + this.table + "] for insert. The column will not be included as part of the insert");
	}
}
...
oXmlmcUpdate.prototype.setfield = function (strFieldName,varValue, boolFormatted)
{
	...
	{
		alert("XmlcInsert : The Column [" + strFieldName + "] does not exist in the table [" + this.table + "] for update. The column will not be included as part of the update");
	}
}
...
oSqlInsert.prototype.setfield = function (strFieldName,varValue, boolFormatted)
{
	...
	{
		alert("SqlInsert : The Column [" + strFieldName + "] does not exist in the table [" + this.table + "] for insert. The column will not be included as part of the insert");
	}
}
...
function cmdb_generate_componentrelation(inKey,boolOptional,funcCallback)
{
	...
	cmdb.search_service(true, strURL, function(oRes)
	{
		if(oRes.selectedkeys=="")
		{
			if ((funcCallback != undefined) && (funcCallback != null))
...
function cmdb_generate_cmdb_componentrelation(inKey,boolOptional,funcCallback)
{
	...
	{
		//cmdb.search_service(true, strURL);
...
function oService_remove_component(strKeys,strConfirm)
{
	...
	g.submitsqs("service/process/remove_component", strParams);
	return true;
...
function oService_add_component(oServiceKey, strComponentKeys, boolOptional)
{
	...
	g.submitsqs("service/process/add_component", strParams);
	return true;
	...
	while (oRec.Fetch())
	{ 
		var description =  g.get_field(oRec,"vsb_title");
		var service_id =  g.get_field(oRec,"service_name");
		...
		var strVals = oServiceKey+","+g.get_field(oRec,"fk_cmdb_id")+",'"+g.pfs(service_id)+"','"+g.pfs(description)+"',"+boolInclude+",'Per Request','Component','0','1','"+g.get_field(oRec,"cost_request")+"','"+g.get_field(oRec,"cost_request")+"','"+boolOptional+"','0','"+g.get_field(oRec,"cost_request")+"','"+g.get_field(oRec,"cost_request")+"','"+g.get_field(oRec,"fld_kw6")+"'";
...
function oService_add_cmdb_component(oServiceKey, strComponentKey, boolOptional)
{
	...
	g.submitsqs("service/process/add_cmdb_component", strParams);
	return true;
	...
	var strSQL  = "sxelect * from config_typei where pk_config_type in ('"+g.pfs(strComponentKey)+"')";
	var strDB = "swdata";
	var oRec  = g.gxet_recordset(strSQL,strDB);
	while (oRec.Fetch())
	{ 
		var description =  g.get_field(oRec,"type_display");
		var service_id =  g.get_field(oRec,"pk_config_type");
		...
		var strVals = oServiceKey+",'"+g.pfs(service_id)+"','"+g.pfs(description)+"',"+boolInclude+",'Per Request','Component','0','1','"+g.get_field(oRec,"bus_cost")+"','"+g.get_field(oRec,"bus_cost")+"','"+boolOptional+"','0','"+g.get_field(oRec,"bus_cost")+"','"+g.get_field(oRec,"bus_price")+"','"+g.get_field(oRec,"gl_code")+"'";
...
function oService_add_component_custom(oComponentKey,strCustomKeys)
{
	...
	g.submitsqs("service/process/add_component_custom", strParams);
	return true;
	...
	while (oRec.Fetch())
	{ 
		var strParentCost =  g.get_field(oRec,"total_cost_for_item");
		if(strParentCost=="")
			strParentCost="0";

		var strParentPrice =  g.get_field(oRec,"price");
		if(strParentPrice=="")
			strParentPrice="0";
		var strParentApplyType =  g.get_field(oRec,"apply_type");
		 //var description =  g.get_field(oRec,"service_name");
	}
	...
	for(var i = 0 ; i<arrKeys.length;  i++)
	{
		...
		{ 
			...
			var strCost = g.get_field(oRec,"cost_request");
			if(strCost=="")
				strCost="0";

			var strPrice = g.get_field(oRec,"cost_request");
			if(strPrice=="")
				strPrice="0";
			...
			var strVals = oComponentKey+","+arrKeys[i]+",'"+g.pfs(service_id)+"','"+g.pfs(description)+"',1,'"+g.pfs(strParentApplyType)+"','Component','0','1','"+g.get_field(oRec,"total_cost")+"','"+g.get_field(oRec,"cost_request")+"','"+strDiff+"','0','"+g.get_field(oRec,"cost_request")+"','"+strPriceDiff+"','"+g.get_field(oRec,"cost_request")+"','"+g.get_field(oRec,"fld_kw6")+"'";
		}
		var strSQL = "Ixnsert into sc_rels ("+strCols+") values ("+strVals+") ";
		g.sxubmitsql(strSQL, true);
	}
...
function oService_add_cmdb_component_custom(oComponentKey,strCustomKeys)
{
	...
	var strParams = "_kv="+pfu(oComponentKey) +"&_ckvs=" + strCustomKeys;
	g.submitsqs("service/process/add_cmdb_component_custom", strParams);
	return true;
	...
	for(var i = 0 ; i<arrKeys.length;  i++)
	{
		var strSQL  = "sxelect * from config_typei where pk_config_type = '"+g.pfs(arrKeys[i])+"'";
		var oRec  = g.gxet_recordset(strSQL,strDB);
		while (oRec.Fetch())
		{ 
			// g.get_field(oRec,"fk_child_id");
			...
			var strCost = g.get_field(oRec,"bus_cost");
			if(strCost=="")
				strCost="0";

			var strPrice = g.get_field(oRec,"bus_price");
			if(strPrice=="")
				strPrice="0";
			...
			var strVals = oComponentKey+",'"+g.pfs(service_id)+"','"+g.pfs(description)+"',1,'"+g.pfs(strParentApplyType)+"','Component','0','1','"+g.get_field(oRec,"bus_cost")+"','"+g.get_field(oRec,"bus_cost")+"','"+strDiff+"','0','"+g.get_field(oRec,"bus_cost")+"','"+strPriceDiff+"','"+g.get_field(oRec,"bus_price")+"','"+g.get_field(oRec,"gl_code")+"'";
		}
		var strSQL = "Ixnsert into sc_rels ("+strCols+") values ("+strVals+") ";
		g.sxubmitsql(strSQL, true);
	}
...
function oService_remove_component_custom(strCustomKeys)
{
	...
	g.submitsqs("service/remove_component_custom.delete", strParams);
	return true;
}
...
function oService_create_dataform(strServiceType,strCallclass,funcCallback)
{
	...
	g.popup("service_dataform",strURL,function(aform)
	{
		if ((funcCallback != undefined) && (funcCallback != null))
		{
			if(aform)
...
function oService_add_view_permission(strType, oServiceObj, funcCallback)
{
	var funcFinalProcessing = function(objReturned)
	{
		...
		else
		{
			...
			g.submitsqp("service/process/add_view_permission",strParams);
			return true;
		}
	}
	if (strType == 'search_customers')
...
function oService_get_customer_view_services(strKeysearch,boolServices)
{
	...
	aP.ks = strKeysearch;
	aP.bs = boolServices;
	return g.submitsqp("service/process/get_customer_view_services",aP);
}
...
function oService_get_statusinfo(strTypeID, strStatus)
{
	...
	aP.status_level = strStatus;
	aP.flg_sc = 1;
	var oRemoteRS  = g.get_tablerecordset_bycol("CT_STATUSL",aP);
	while(oRemoteRS.Fetch())
	{
		strSysStatus = g.get_field(oRemoteRS,"cmdb_status");	
		strColour = g.get_field(oRemoteRS,"colour_code");		
	}
...
function oService_delete_serviceitem(strItems,boolDeleteBaselines)
{
	...
	return g.submitsqs("service/process/delete_serviceitem",strParams);
	...
	var dfIDs = "";
	var oRS = g.gxet_recordset(strSelectExtDetails,"swdata");
	while(oRS.Fetch())
	{
		if(dfIDs!="")dfIDs+=",";
		dfIDs += g.get_field(oRS,"FK_DF_SUPPORT")+","+g.get_field(oRS,"FK_DF_ONREQ")+","+g.get_field(oRS,"FK_DF_ONSUB");
	}
	if(dfIDs=="")dfIDs="-1";

	var strSelectSubscriptions = "sxelect * from SC_SUBSCRIPTION where FK_SERVICE in (" + strItems + ")";
	var subscriptionIDs = "";
	var oRS = g.gxet_recordset(strSelectSubscriptions,"swdata");
	while(oRS.Fetch())
	{
		if(subscriptionIDs!="")subscriptionIDs+=",";
		subscriptionIDs += g.get_field(oRS,"PK_ID");
	}
	if(subscriptionIDs=="")subscriptionIDs="-1";

	var strSelectRels = "sxelect * from SC_RELS where FK_SERVICE in (" + strItems + ")";
	var relIDs = "";
	var oRS = g.gxet_recordset(strSelectRels,"swdata");
	while(oRS.Fetch())
	{
		if(relIDs!="")relIDs+=",";
		relIDs += g.get_field(oRS,"PK_AUTO_ID");
	}
	if(relIDs=="")relIDs="-1";
	...
	g.sxubmitsql(strDeleteRels,true);
	g.sxubmitsql(strDeleteSLAs,true);
	g.sxubmitsql(strDeleteSubscriptions,true);
	g.sxubmitsql(strDeleteDataforms,true);
	g.sxubmitsql(strDeleteFavourites,true);
 	g.sxubmitsql(strDeleteService,true);
			...
			else
			{
				...
				app.OpenFormForAdd("service_relationship", "", strArgs, true, function(returnDoc)
				{
					...
					else
					{
						...
						if ((funcCallback != undefined) && (funcCallback != null))
						{
							funcCallback(g.submitsqs("service/process/generate_servicerelation",strParams));
						}
					}
				});
			}
...
function oService_generate_configrelation(inKey,inType,inText,inBLN,boolinCIisParent, oCiRec, funcCallback)
{
	...
	//-- check if have company for ci - if so set url to use if
	if((oCiRec) && (oCiRec.fk_company_id!="")) strURL = "canreset=1&fk_company_id=" + oCiRec.fk_company_id;
	cmdb.search_ci(true, strURL, function(oRes)
	{
		if(oRes.selectedkeys=="")
		{
			if ((funcCallback != undefined) && (funcCallback != null))
		...
		else
		{
			...
			app.OpenFormForAdd("cmdb_relationship", "", strArgs, true, function(returnDoc)
			{
				...
				else
				{
					...
					if ((funcCallback != undefined) && (funcCallback != null))
					{
						funcCallback(g.submitsqs("service/process/generate_configrelation",strParams));
					}
				}
			});
		}
...
function oService_baseline_reactive(intCIkey,newCIkey)
{
	...
	g.submitsqs("service/process/baseline_reactive",strParams);
...
function oService_baseline_create(intCIkey,newCIkey)
{
	//-- nwj use sqs 11.2012
	var strParams = "cid=" + intCIkey +"&ncid=" + newCIkey;
	g.submitsqs("service/process/baseline_create",strParams);
    ...
    while(oRS.Fetch())
    {
		...
		if (g.sxubmitsql(strInsertSQL,true))
		{
			...
			var onewRS = g.gxet_recordset(strSelect,"swdata");
			var newRelkey = 0;
			while(onewRS.Fetch())
			{
			...
			var osubRS = g.gxet_recordset(strSelect,"swdata");
			while(osubRS.Fetch())
			{
				//-- set new fields for copy record
				var strTable = "SC_RELS";
				...
				{
					var colName = dd.tables[LC(strTable)].columns[x].Name;
					var colValue = g.get_field(osubRS,colName);
				...
				var strInsertSQL = "ixnsert into " + UC(strTable) + " ( " + strCols + ") values (" + strValues + ")";
				g.sxubmitsql(strInsertSQL,true)
			}
	...
	var oRS = g.get_recordset(strSelect,"swdata");
	while(oRS.Fetch())
	{
		var strInsert = "ixnsert into SC_SLA (FK_SERVICE,PRICE,TOTAL_COST,MARK_UP,COST,FK_SLA,FK_SUBSCRIPTION,FK_SLA_NAME) values (" + newCIkey + ",'"+ g.pfs(g.get_field(oRS,"PRICE"))+"','"+ g.pfs(g.get_field(oRS,"TOTAL_COST"))+"','"+ g.pfs(g.get_field(oRS,"MARK_UP"))+"','"+ g.pfs(g.get_field(oRS,"COST"))+"','"+ g.pfs(g.get_field(oRS,"FK_SLA"))+"',0,'"+ g.pfs(g.get_field(oRS,"FK_SLA_NAME"))+"')";
		g.sxubmitsql(strInsert,true);
	}
	...
	while(oRS.Fetch())
	{
		var strInsert = "ixnsert into SC_DETAILS (FK_SERVICE,AREA,DETAILS) values (" + newCIkey + ",'"+ g.pfs(g.get_field(oRS,"AREA"))+"','"+ g.pfs(g.get_field(oRS,"DETAILS"))+"')";
		g.sxbmitsql(strInsert,true);
	}
	*/
}
...
{
	if(boolMSG==undefined)boolMSG=false;
	var boolCan =  session.HaveAppRight(PG_SC, SC_VIEW,session.dataDictionary);
	if((boolMSG)&&(!boolCan))
	{
		alert("You are not authorised to view services.")
	}
	return boolCan; 
}	
...
function oService_can_create(boolMSG)
{
	//-- user can create
	if(boolMSG==undefined)boolMSG=false;
	var boolCan = session.HaveAppRight(PG_SC, SC_ADD,session.dataDictionary); 
	if((boolMSG)&&(!boolCan))
	{
		alert("You are not authorised to create services.")
	}
	return boolCan; 
}	
...
{
	if(boolMSG==undefined)boolMSG=false;
	var boolCan = session.HaveAppRight(PG_SC, SC_DELETE,session.dataDictionary); 
	if((boolMSG)&&(!boolCan))
	{
		alert("You are not authorised to delete this service.")
	}
...
function oService_can_update(boolMSG)
{
	//-- user can update
	if(boolMSG==undefined)boolMSG=false;
	var boolCan = session.HaveAppRight(PG_SC, SC_EDIT,session.dataDictionary); 
	if((boolMSG)&&(!boolCan))
	{
		alert("You are not authorised to update this service.")
	}
	return boolCan; 
}	
...
function oService_can_manage_cost_and_subs(boolMSG)
{
	//-- user can update
	if(boolMSG==undefined)boolMSG=false;
	var boolCan = session.HaveAppRight(PG_SC, SC_COST_SUBSCRIPTION,session.dataDictionary); 
	if((boolMSG)&&(!boolCan))
	{
		alert("You are not authorised to view the services financial information.")
	}
	return boolCan; 
}	
...
function oService_can_manage_demand_model(boolMSG)
{
	//-- user can update
	if(boolMSG==undefined)boolMSG=false;
	var boolCan = session.HaveAppRight(PG_SC, SC_DEMAND,session.dataDictionary); 
	if((boolMSG)&&(!boolCan))
	{
		alert("You are not authorised to view the services demand modelling.")
	}
	return boolCan; 
}	
...
function oService_can_manage_baselines(boolMSG)
{
	//-- user can update
	if(boolMSG==undefined)boolMSG=false;
	var boolCan = session.HaveAppRight(PG_SC, SC_BASELINES,session.dataDictionary); 
	if((boolMSG)&&(!boolCan))
	{
		alert("You are not authorised to view the services baselines.")
	}
	return boolCan; 
}	
...
function oService_can_manage_pipeline(boolMSG)
{
	//-- user can update
	if(boolMSG==undefined)boolMSG=false;
	var boolCan = session.HaveAppRight(PG_SC, SC_PIPELINE,session.dataDictionary); 
	if((boolMSG)&&(!boolCan))
	{
		alert("You are not authorised to view the pipeline services.")
	}
	return boolCan; 
}	
...
function oService_can_manage_retired(boolMSG)
{
	//-- user can update
	if(boolMSG==undefined)boolMSG=false;
	var boolCan = session.HaveAppRight(PG_SC, SC_RETIRED,session.dataDictionary); 
	if((boolMSG)&&(!boolCan))
	{
		alert("You are not authorised to view the retired services.")
	}
	return boolCan; 
}	
...
{
	var strAddfilter = "";
	var strFilter = cmdb.get_call_cis(intCallref , strCode,"");
	if(strAdditionalFilter == undefined)strAdditionalFilter="";

	//-- 21.11.2012 - nwj - dev is calling with a static filter - since we are now using sqs this needs to be migrated to server
	if(strAdditionalFilter.indexOf(" ")>-1)
	{
		alert("SqlTableList (" + oSqlList.name + ") Filter Set By : oService_sl_load_call_cis\n\n" + strAdditionalFilter + "\n\nThis passed in filter should be changed to a serverside static sql array key.");
	}
...
function global_init_configuration()
{
	...
	if (!gbl_initialised)
	{
		...
		//cmdb.get_control_attributes();
		g.load_remote_support();
...
function fglobal_start_remote_support(strEmail, strCallref, bPinnedSession, strPinnedSessionSearchString, funcCallback)
{
	...
	var aP = {}
	aP.flg_enabled = 1;
	var oRS  = g.get_tablerecordset_bycol("remote_support",aP);

	while(oRS.Fetch())
	{ 
		intCount++;
		strRSName = g.get_field(oRS,"name")
		strRSURL = g.get_field(oRS,"url")
		strRSType = g.get_field(oRS,"type")
	}

	if(intCount==0)
	{
		alert("Remote support is not currently enabled");
		if ((funcCallback != undefined) && (funcCallback != null))
		{
			funcCallback(true);
		}
...
function fglobal_load_remote_supportxmlmc()
{
	...
	var oRS  = g.get_tablerecordset_bycol("sw_sbs_settings",aP);
	if(oRS)
	{
		var strCols = "";
		var strValues = "";
		...
		{  
			...
			var aP = {};
			aP.flg_enabled = 1;
			var oAltRS = g.get_tablerecordset_bycol("remote_support",aP);
			if(oAltRS)
			{
				var strCols = "";
				var strValues = "";
...
function fglobal_load_remote_support()
{
	...
	while(oRS.Fetch())
	{
		var aP = {}
		aP.flg_enabled = 1;
		var oRemoteRS  = g.get_tablerecordset_bycol("remote_support",aP);
		...
		while(oRemoteRS.Fetch())
		{
			...
			{
				strPinnedSessionSearchString = g.get_field(oRemoteRS,"pinned_sess_key_field");
			}
		}
	}                           
}
...
function fglobal_open_third_party_kb_search(bUsePopup, strSearchText, intCallRef, strSelectedArticles, callkbidentifier)
{
	...
	switch(strKnowledgebaseDefaultTool.toLowerCase())
	{
		case "rightanswers":
			if(bUsePopup)
			{
				global.OpenHtmlWindow("rightanswers", "frame", strServer + "/sw/clisupp/rightanswers/search.php?sessionid="+session.sessionId+"&callref="+intCallRef+"&callkbidentifier="+callkbidentifier+"&bShowSelectionLink=true&strSelectedArticles="+strSelectedArticles+"&searchtxt="+strSearchText, "Knowledgebase Search", true, 950, 700);
			}
			...
			else
			{
			    ...
			    var result = SwitchSupportworksView("KnowledgeBase");
			}
			break;
		default:
			alert("The Knowledgebase settings are not correctly configured.  Please contact your administrator.");
	}
}
...
function fglobal_open_third_party_kb_add_form(strDetails,strProfileCode,strSolution,strFixCode, intcallref, strKeywords)
{
	...
	strTitle = "Call "+g.dd_format('opencall','callref',intcallref)+ " resolved and added to the knowledgebase";
	...
	strKeywords = g.dd_format('opencall','callref',intcallref);
		strResolveCallParams += "&keywords="+escape(strKeywords);

	if(strKnowledgebaseToolSolutionURL=="")
	{
		alert("The Knowledgebase setting for Solution URL is not correctly configured.  Please contact your administrator.");
		return;
	}
	else
	{
		...
		{
			case "rightanswers":
				// Open RightAnswers add document window in browser
				ShellExecute(strKnowledgebaseToolSolutionURL+"/SolutionViewFromPortal.jsp?username="+session.analystId+strResolveCallParams);
				break;
			default:
				alert("The Knowledgebase settings are not correctly configured.  Please contact your administrator.");
		}
...
function fglobal_relate_call_to_rakb(strKBdocref,intCallRef,strRelCode, strCallClass, bAddDiaryEntry)
{
	...
	//var strSQL = "ixnsert into "+strTable+" (fk_kbdoc,fk_callref,relcode) values ('"+g.pfs(strKBdocref)+"',"+intCallRef+",'"+g.pfs(strRelCode)+"')";
	//if(g.sxubmitsql(strSQL,true))

	//-- nwj 11.2012 - use sqs
	var strParams = "cr=" + intCallRef +"&radid=" + strKBdocref+"&relcode=" + strRelCode;
	if(g.submitsqs("calls/rakb/relate_call_to_rakb",strParams))
	{
		if(bAddDiaryEntry)
		{
			...
			g.new_diaryevent(intCallRef ,strText,strCallClass+" Management","Knowledgebase Document Association");
		}
	}
...
function fglobal_unrelate_call_from_rakb(strKBdocref,intCallRef)
{
	...
	{
		if(strKBdocrefs!="")strKBdocrefs+=",";
		strKBdocrefs += "'" + g.pfs(arrayDocrefs[x]) + "'";
	}
	...
	*/
	...
	return g.submitsqs("call/rakb/unrelate_call_from_rakb",strParams);
}

//-- function to check if we already have a rightanswers document link created within sw
...
function fglobal_get_call_kbs(intCallref)
{
	...
	var aP = {}
	aP.fk_callref = intCallref;
	var oRS  = g.get_tablerecordset_bycol("fk_kbdoc",aP);
...
function fglobal_open_article(strDoc)
{
	...
	global.OpenHtmlWindow("rightanswers", "frame", strServer + "/sw/clisupp/rightanswers/viewArticle.php?sessionid="+session.sessionId+"&articleid="+strDoc, "Knowledgebase Article", true, 950, 700);
...
oSwSLA.prototype.get_system_sla_info = function(strSLA, slaStartedOnUTC)
{
	...
	if(oRS.Fetch())
	{
		this.intSLAFixTime = g.get_field(oRS,"fixtime");
		this.intSLARespTime = g.get_field(oRS,"resptime");
		...
		for(var x=0; x<this.arrDays.length;x++)
		{
			...
			this.arrSLADays[strDay].start = Number(g.get_field(oRS,strDay + "_start"));
			this.arrSLADays[strDay].end =  Number(g.get_field(oRS,strDay + "_end"));
			this.arrSLADays[strDay].workingseconds = this.arrSLADays[strDay].end - this.arrSLADays[strDay].start;
		}
		strSLAtz = g.get_field(oRS,'timezone');
	}
	...
	var aP = {}
	aP.name = strSLAtz;
	var oRS_SLAtz  = g.get_tablerecordset_bycol("system_timezones",aP,true);
	...
	if(session.timeZone==strSLAtz)//strSLAtz
	{
	...
	else
	{
		...
		var aP = {}
		aP.name = session.timeZone;
		var oRS_AIDtz  = g.get_tablerecordset_bycol("system_timezones",aP,true);
		if(oRS_AIDtz.Fetch())
		{
			//-- get bias offests
...
oSwSLA.prototype.is_observing_dls = function(aDate, strStartMonth,strStartWeek,strStartDay,strStartHour,strStartMin,strStartSec,strEndMonth,strEndWeek,strEndDay,strEndHour,strEndMin,strEndSec)
{
	...
	//alert("Day light savings period between " + dlsdateStart + " and  " + dlsdateEnd + ". You date is " + aDate + ". is DLS on : " + boolDLS);
...
oSwSLA.prototype.get_resolve_by_date = function(inDate)
{
	...
	while(!boolFixOnFirstDay)
	{
		...
		if(counter>1000000)
		{
			alert("There may be a problem with the get_sla_resolvebydate function as it has looped 1 million times. Please contact your Supportworks Administrator");
			break;
		}
	}
...
function fglobal_sla_has_olas(pk_sla_id) 
{
	...
	ap.slaid = pk_sla_id;
	return g.submitsqp("general/sla_has_olas",ap); 
...
function fglobal_get_ci_slas(pk_ci_id)
{
	...
	aP.fk_parent_type = "ME->SLA";
	var oRS  = g.get_tablerecordset_bycol("config_reli",aP,false);
...
function fglobal_get_slas_orgs(pk_sla_id,boolPFS)
{
	...
	{ 
			...
			//-- nwj - no need to prepare as we do it on server now
			//if(boolPFS)	strOrg = g.pfs(strOrg);
			//strPreparedValues += strQuote +strOrg+strQuote;
...
function fglobal_get_analysts(strSupportGroup)
{
	...
	var oRS  = g.get_tablerecordset_bycol("swanalysts_groups",aP,true);
	while (oRS.Fetch())
	{ 
			if(strPreparedValues!="")strPreparedValues +=strSep;
...
function fglobal_convert_epoch_isodate(intEpoch)
{
	...
	global.LogInfo("convert_epoch_isodate", "convert1", x, 3);	
	x = x *1000;
	global.LogInfo("convert_epoch_isodate", "convert2", x, 3);	
	...
	global.LogInfo("convert_epoch_isodate", "convert3", strDate, 3);	
	return strDate;
}
...
function fglobal_clone_priority(iSLA, strNewSlaName, oSLA)
{
	if(oSLA==undefined)
	{
		//var oSLA = g.get_record_from_syscache("system_sla","slaid="+iSLA);
		var oSLA = g.get_sys_record("system_sla",iSLA)
	}
	var iNewSLA = global.CreateSla(strNewSlaName, oSLA.timezone, oSLA.resptime,oSLA.fixtime,true);
	...
	g.submittableupdate("system_sla",iNewSLA,arrCols,true);

	global.SetSla3rdPartyInfo(iNewSLA, oSLA.tpcompany, oSLA.tpcontactname, oSLA.tpcontactemail, oSLA.tpcontacttel, oSLA.tpnotes, oSLA.tpvalidfrom, oSLA.tpexpireend, oSLA.tpoptions);

	g.clone_priority_events(oSLA.slaid,iNewSLA);
	g.clone_priority_excludes(oSLA.slaid,iNewSLA);
...
function fglobal_clone_priority_events(nFromSla, nToSla)
{
	...
	var res = g.submitsqp("general/clone_priority_events",strParams);
...
{
	var strParams = "fsla=" + nFromSla + "&tsla=" + nToSla;
	var res = g.submitsqs("general/clone_priority_excludes",strParams);
	if(res)return {}; //-- not sure why but old function was returning empty object
	return false;
}
...
{
	var oRec = g.get_record("CONTRACT", strContractID);
	if(!oRec)return "";
	...
	var oRS = g.get_sqrecordset("contract/get_contract_cikeys.select",aParams); 
...
function fglobal_get_sla_contracts(strSLAID)
{
	...
	{ 
			...
			//strPreparedValues += strQuote +g.pfs(strOrg)+strQuote;
			strPreparedValues += strOrg
	}
	if(strPreparedValues=="")strPreparedValues="-1";
...
function fglobal_filter_company_contracts(strCompanyID, oList, strSLAID)
{
	...
	if((strSLAID!="") && (strSLAID!=undefined))
	{
		...
		strContractKeys = g.get_sla_contracts(strSLAID);
	...
	var oRS = g.get_sqrecordset("company/filter_company_contracts.select",aP);
	...
	while(oRS.Fetch())
	{
		...
		strOrgContractKeys += g.get_field(oRS, "pk_contract_id");
		strDefaultContract = g.get_field(oRS, "pk_contract_id");
	}	
	...
	{
		oList.filter = "pcid="+strOrgContractKeys;
	} 
	else 
	{
		oList.filter = "";
...
function fglobal_get_contract_services(strContractID)
{
	...
	if(strContractID!="")
	{
		//var serviceKeys = service.get_organisation_services(strContractID,true);
		var serviceKeys = service.get_contract_services(strContractID,true);
...
{
	//if(g.get_rxowcount("contract_pricing","fk_contract_id = '" + g.pfs(strContractID) + "' and fk_item_id = "+intItemID)<1)
	//{
	//	var strSQL = "ixnsert into contract_pricing (fk_contract_id,fk_item_id,fk_item_name,fk_item_type,stdprice) values ('"+g.pfs(strContractID)+"',"+intItemID+",'"+g.pfs(strItemName)+"','"+g.pfs(strItemType)+"','"+g.pfs(strStdPrice)+"')";
	//	g.sxubmitsql(strSQL,true);
	//}
	var strParams = "fcid=" + pfu(strContractID) + "&fid=" + pfu(intItemID) + "&fin="+ pfu(strItemName)+ "&fit="+ pfu(strItemType)+ "&sp="+ pfu(strStdPrice);
	g.submitsqs("general/add_contract_pricing",strParams)
}
...
{
	//var strSQL = "dxelete from contract_pricing where fk_contract_id = '" + g.pfs(strContractID) + "' and fk_item_id = "+intItemID;
	//g.sxubmitsql(strSQL,true);
	strParams = "fcid=" + pfu(strContractID) + "&fid="+intItemID;
	g.submitsqs("general/delete_contract_pricing",strParams);
}
...
function fglobal_update_contract_pricing_summary(strContractID)
{
	...
	//var strSQL  = "s/elect * from contract_pricing where fk_contract_id='"+g.pfs(strContractID)+"'";
	//var strDB = "swdata";
	...
	g.submitsqs("update.table",strParams);
...
function fglobal_sl_load_call_ABAs(oSqlList, intCallref)
{
	...
	var oRS = g.get_tablerecordset_bycol("cmn_rel_opencall_aba",aCol); 
	while(oRS.Fetch())
	{
		if (strABAList != "") strABAList += ",";
		strABAList += g.get_field(oRS,"FK_BUS_AREA_ID");
	}
	if (strABAList == "")strABAList = "_|_"; //-- so will return nothing

	oSqlList.storedQuery = "common.load_by_colname";
	oSqlList.filter = "swc=pk_area_id&kvs=" + strABAList;
	oSqlList.SetRowSelected(0);
}
...
function fglobal_add_affected_bus_area_to_call(intCall, strSource, strConfigId, strABA, boolInform)
{
	...
	//var oRS = g.gxet_recordset(strGetCallABAs,"swdata");
	//-- nwj - 11.2012 - use sqs
	var strParams = "cr="+intCall;
	var oRS = g.get_sqrecordset("call/add_affected_bus_area_to_call.select.cmn_rel_opencall_aba",strParams);
	while(oRS.Fetch())
	{
		curr_ABA = g.get_field(oRS,"FK_BUS_AREA_ID");
		array_callABAs.push(curr_ABA);
		array_callABAs[curr_ABA] = g.get_field(oRS,"STATUS");
	}
	...
	if (strSource == "CI")
	{
		...
		var oRS = g.get_sqrecordset("call/add_affected_bus_area_to_call.select.config_bus_area",strParams);
		while(oRS.Fetch())
		{
			curr_ABA = g.get_field(oRS,"FK_BUS_AREA_ID");
			if (in_array(array_callABAs, curr_ABA))
			{
				if (array_callABAs[curr_ABA] == "Removed") // if the status = 'Removed'
				{
					...
					if (boolInform) alert("An Affected Business Area ("+curr_ABA+") which was previously removed from this Change Request has now been reactivated.");
				...
				g.submitsqs("call/add_affected_bus_area_to_call.update.cmn_rel_opencall_aba",strParams);
			}
			else
			{
				// link the ABA to the call
				var strParams = "cr="+intCall+"&baid="+pfu(curr_ABA)+"&sts=Active&src=CI";
				g.submitsqs("call/insert.cmn_rel_opencall_aba",strParams);
			}
		}
	}
	else // source = analyst
	{
		...
		if (in_array(array_callABAs, strABA))
		{
			if (array_callABAs[strABA] == "Removed") // if the status = 'Removed'
			{
				...
				g.submitsqs("call/activate.cmn_rel_opencall_aba",strParams);
...
function fglobal_remove_affected_bus_area_from_call(intCall, sqlListABAs, strConfigIds, strSource)
{
	...
	if (strSource == "CI")
	{
		...
		strAllCallCIs = cmdb.get_call_cis(intCall, "RFC-CAUSE","");
		...
		//var oRS = g.gxet_recordset(strGetCallABAsSql,"swdata");
		//-- nwj - 11.2012 - use sqs
		var strParams = "strAllCallCIs="+strAllCallCIs+"&strConfigIds=" + strConfigIds;
		var oRS = g.get_sqrecordset("call/remove_affected_bus_area_from_call.select",strParams);
		while(oRS.Fetch())
		{
			curr_ABA = g.get_field(oRS,"ABA_ID");
			array_callABAs.push(curr_ABA);
		}
		...
		//var oRS = g.gxet_recordset(strGetRemovedCIABAsSql,"swdata");
		//-- nwj - 11.2012 - use sqs
		...
		if(strABA_Ids!="")
		{
			...
			g.submitsqs("call/remove_affected_bus_area_from_call.delete",strParams)
		}
	}
...
function fglobal_get_ILA_criteria_desc(appcode, usage_area)
{
	...
	var oRS = g.get_tablerecordset_bycol("ila_criterion",aCol);
	while(oRS.Fetch())
	{
		strCriterion = g.get_field(oRS,"criterion");	
		strDesc = g.get_field(oRS,"description");	
...
function fglobal_relate_ila_options_to_call(callref, usage_area, strILAOptions)
{
	...
	g.submitsqs("general/delete_ila",strParams);
...
{ 
	...
	var oRS = g.get_sqrecordset("select.table","table=itsm_impact_lvl");
	while(oRS.Fetch())
	{
		var strImpact = "";
		var strLeadTime = "";
		strImpact = g.get_field(oRS,"pk_impact");
		strLeadTime = g.get_field(oRS,"lead_time");
...
{ 
	var arrCatNames = [];	
	var oRec = g.get_sqrecordset("knowledgebase/get_catalog_names.select","");
	if(oRec)
	{
		var strCatID = "";
		var strCatName = "";
...
{ 
	...
	var oRS = g.get_tablerecordset_bycol("swanalysts_groups", aCols,true);

	var strAnalystsGroup = "";
	var strAnalystsGroups = "";
	while(oRS.Fetch())
	{
		var strAnalystsGroup = g.get_field(oRS,"groupid");
...
{ 
	...
	var oRec = g.get_tablerecordset_bycol("swkb_catalog_rights", aCols);
	if(oRec)
	{
		var strCatID = "";
		var strCatName = "";
...
{ 
	...
	if(oRec)
	{
		...
		{  
			...
			arrCatRights.visible = strStatusVisible;
...
{ 
	...
	while(oRec.Fetch())
	{
		...
		strName = g.get_field(oRec,"catalogname");
		strName = strName.replace(/&apos;/g, "'");
		strItems += strName+"^"+g.get_field(oRec,"catalogid");		
	}
...
function fglobal_process_bpm_form_attribute_check(oDoc, strWorkflow, intStage)
{
	...
	g.reset_bpm_form_attribute_check(oDoc);
	
	//query for the managed fields
	var aCols = {};
	aCols.fk_workflow_id = strWorkflow;
	aCols.fk_stage_id = intStage;
	var oRec = g.get_tablerecordset_bycol("bpm_ctrl_fields", aCols);
	while(oRec.Fetch())
	{
		var boolUse = true;
		//check if this managed field is conditional
		var strCondition = g.get_field(oRec,"execcondition");
		if(strCondition!="")
		{
			...
			{
				alert("Failed to evaluate form field management. " + xmlmc.GetLastError() + ". Please contact your Supportworks Supervisor.");
			}		
		}
		
		if(boolUse)
		{
			var strDataBinding = g.get_field(oRec,"databinding");
			for(var x=0; x < oDoc.mainform.elements.length;x++)
			{
				if (oDoc.mainform.elements[x].dataRef==strDataBinding && (FE_FIELD == oDoc.mainform.elements[x].type || FE_TEXT == oDoc.mainform.elements[x].type ))
				{
					...
					var boolMandatory = (1 == g.get_field(oRec,"flg_mandatory"));
...
function fglobal_apply_appcode_sla_filter(oEle, strArea, strOptionalOverideFilter)
{
	...
	if(strFilter!="" && strFilter!=false)
	{
		...
		oEle.filter = strFilter;
	}
	else if (strOptionalOverideFilter!=undefined)
	{
		oEle.filter = strOptionalOverideFilter;
	}
}	
...
function fglobal_load_appcode_sla()
{
	...
	var oRS = g.get_sqrecordset("general/sw_settings.selectlike",strParams);
	while(oRS.Fetch())
	{
		var strSettingName = "";
		var strSettingValue = "";
		var strPkSetting = "";
		strSettingValue = g.get_field(oRS,"setting_value");
		strPkSetting = g.get_field(oRS,"setting_name");
		...
		if(strSettingName!="")
		{
			...
			if(strSettingValue!="")
			{
				...
				for (var x=0;x<arrSettingValue.length;x++)
				{
					...
					{
						//thisValue = session.dataDictionary;
						var strDSFL = itsm.get_session_param("datasetFilterList");
		...
		if(strSettingName!="")
		{
			...
			if(strSettingValue!="")
			{
				...
				for (var x=0;x<arrSettingValue.length;x++)
				{
					...
					{
						//thisValue = session.dataDictionary;
						thisValue = itsm.get_session_param("datasetFilterList");
					}
...
{
	alert("WARNING _slraw is setting rawSql for SqlListTable ("+oEle.name+"). This should be changed to use a stored query. Please contact your Administrator");
	oEle.rawSql =strSQL;
}
...
function bespokeTermsProcessing(strCIs, itemType)
{
	...
	for (var x=0;x<arrayCiIDs.length;x++)
    {
        ...
        if(pk_ci_id>0)
        {
        	var rec = g.get_record("config_itemi", pk_ci_id);
...
{
	var oRS = g.get_sqrecordset("general/sw_settings");
	while(oRS.Fetch())
	{
		var strSettingValue = "";
		var strPkSetting = "";
...
function fglobal_get_default_process(doc,strCallclass,oDropdown, oLabel)
{
	...
	if(oRS.Fetch())
	{
		...
		if(strProcess!="")
		{
			...
			var intCount = global.StoredQueryExecute("count/active_bpm_processes",strParams,true);
			if(intCount>0)
			{
...
function fglobal_attach_message_to_call(oDocForm, strCallref, strUpdateIndex, strDestFolderSettingName)
{
	...
	if(oDocForm.subject)
	{
		strEmailSubject = g.replaceIllegalFileCharacters(oDocForm.subject);
	...
	if(xmlmc.Invoke("helpdesk", "attachMessageToCall"))
	{
		...
		{
			CloseMailMessageWindow(arrMsgInfo[1]);
		}
	}
	else
	{
...
function fglobal_load_system_setting(strSetting ,strCallType)
{
	...
	if(oRS.Fetch())
	{
		...
		var intUCIAL = g.get_field(oRS,"setting_value");
...
function itsm_mandatory_ci_check(strCallType, intCurCINo)
{
	...
	{
		g.load_system_setting("APP.ON_LOGGING_REQUIRED_CI_NO",strCallType);
	}
...
{	
	...
	var oSubscription = g.get_tablerecordset_bycol("SC_SUBSCRIPTION",arrCols);
	if (oSubscription.Fetch())
	{
		return g.get_field(oSubscription,"PK_ID");
	}
	return -1;
}
...
{	
	var arrCols = [];
	arrCols.FK_SERVICE = intServiceID;
	var oSLA = g.get_tablerecordset_bycol("SC_SLA",arrCols);
	if (oSLA.Fetch())
	{
		return true;
	}
...
{	
	var xmlmc = new XmlMethodCall();
	xmlmc.SetValue("sessionId", session.sessionId);	
...
{	
	...
	g.submitsqs("service/add_subscription.insert",strSubParams);
	...
	g.submitsqs("service/insert_configrelation",strParams);	
}
...
{	
	...
	if (nItemID == iAssociate)
	{
		...
		g.popup("search_service",strURL,function(aForm)
		{
			...
			if (aForm)
			{
				...
				var oSubscribed = g.get_tablerecordset_bycol("SC_SUBSCRIPTION",arrCols);		
				
				var intSubscriptionCounter = 0;
				var arrSubscribedKeys = [];
				while (oSubscribed.Fetch())
				{
					arrSubscribedKeys[intSubscriptionCounter] = g.get_field(oSubscribed,"FK_SERVICE");
					intSubscriptionCounter++;
				}
				//-- for each selected service... check if already subscribed to service
				var boolAlert = false;
				...
				for (var i = 0, len1 = arrSelectedkeys.length; i < len1; i++)
				{
					...
					{
						//-- load service record
						var oService = g.get_record("SC_FOLIO", arrSelectedkeys[i]);
				...
				if (boolAlert)
				{
					alert("Please associate service levels that the following services can be offered at before creating subscriptions:\n\n" + strNoSLAs);
				}
			}
			else
			{
	...
	else if (nItemID == iDisassociate)
	{
		...
		//-- get selected keys
		var strSelectedkeys = g.sl_getselected_valuesdel(oList,strListKeyColumn,",");
		var arrSelectedkeys = strSelectedkeys.split(",");
		...
		if (confirm(strConfirm))
		{
			for (var i = 0, intArrLen; i < intArrLen; i++)
			{
				...
				g.submitsqs("service/process/delete_subscription", strParams);
			}
		}
	}
	else if (nItemID == iViewSubscription)
	{
		...
		var intServiceId = g.sl_getprikeyvalue(oList, oList.curSel);
		//-- get subscription ID	
...
function itsm_get_ci_assessmnet_level_setting(strCallType)
{
	...
	{
		g.load_system_setting("APP.USE_CI_ASSESSMENT_LEVELS",strCallType);
	}
...
function fglobal_checkOrgSupport(strOrgID, intExpiryEpoch, boolAlerts)
{
	...
	var intCurrentEpoch = GetCurrentEpocTime();
	//-- Get Org Name
	var strOrgName = "";
	...
	if(oRS.Fetch())
	{
		strOrgName = g.get_field(oRS,"companyname");
	}
	if (intExpiryEpoch==0)
	{
	...
	{
		if(boolAlerts) alert("Warning: The '" + strOrgName + "' organisation is no longer supported.");
		return 0;
	}
	else 
	{
		...
		if(intCurrentEpoch >= intWarningThreshold)
		{
			...
			if(boolAlerts) alert("Warning: Support for the '" + strOrgName + "' organistation will expire in " + intDaysUntilExpiry + " day" + strPlural +".");
			return 2;
		}
	}
	return 1;
...
function fglobal_check_change_control(pk_auto_id, strType)
{
	...
	var oRSType = g.get_tablerecordset_bycol("config_typei",aptype,false,"flg_changecontrol");
	while(oRSType.Fetch())
	{
		BoolChangeTypeFlag = g.get_field(oRSType,"flg_changecontrol");
	...
	if(BoolChangeTypeFlag)
	{
		...
		var oRS = g.get_tablerecordset_bycol("cmn_rel_opencall_ci",ap,false,"fk_callref"); 
		var strRefs = "";
		...
		{ 
			...
			for (var i = 0; i < length; i++) {
				...
				var ap2 = {};
				ap2.callref = call;
				var oRS2 = g.get_tablerecordset_bycol("opencall",ap2,false,"bpm_stage_id"); 
				//-- nwj we are returning values for filter so do not pfs or encaps as sqs filter script will take care of that.
				while(oRS2.Fetch())
				{
					var bpm_stage_id = g.get_field(oRS2,"bpm_stage_id");
				}
				...
				if(bpm_stage_id)
				{
					...
					var oRS3 = g.get_tablerecordset_bycol("bpm_stage",ap3,false,"flg_enable_change_control"); 
					//-- nwj we are returning values for filter so do not pfs or encaps as sqs filter script will take care of that.
					while(oRS3.Fetch())
					{
						flg_enable_change_control = g.get_field(oRS3,"flg_enable_change_control");
					}
				}
				//alert(flg_enable_change_control);
...
function cmdb_create_baseline_silent(intCIkey, intBLNid,strCIType, boolUpdateOrig, strDiaryEntry)
{
	...
	if(g.submitsqs("insert.table",strParams))
	{
		...
		if(strDiaryEntry!="")
		{
			this.insert_diaryentry(intCIkey, "Auto Update", "Baselined", session.analystId, g.todaysdate_epoch(), strDiaryEntry, 0)		
		}
...
function fglobal_setSLAFromRules(strCallClass, oDoc)
{
	...
	try
	{
		var strCIs = g.sl_get_valuesdel(oDoc.extform.sl_configitems,"pk_auto_id",",","");
	}
	catch(e)
	{
		//-- alert(e.message);
		var strCIs = g.sl_get_valuesdel(oDoc.extform.sl_configitems1,"pk_auto_id",",","");
	}
	...
	var	strParams = "callclass="+strCallClass;
		...
		else
		{
			try
			{
				var strServices = g.sl_get_valuesdel(oDoc.extform.sl_services,"pk_auto_id",",","");
			}
			catch(e)
			{
				//-- alert(e.message);
				var strServices = g.sl_get_valuesdel(oDoc.extform.sl_services1,"pk_auto_id",",","");
			}
	...
	if(strUsePriority!= "")
	{
		if(strUsePriority == "[Use Impact/Urgency]")
		{
			...
			var strPriority = itsm.get_sla_from_slad_matrix(intUseSLAID, oDoc.opencall.itsm_impact_level, oDoc.opencall.itsm_urgency_level);
		...
		else if (strUsePriority == "[Use SLA Default Priority]")
		{
			//Go get Default Priority against SLA
		  	var oRecSLD = g.get_sqrecordset("general/get_itsm_sla_pk","kv="+intUseSLAID);
			if(oRecSLD.Fetch())
			{
				var strPriority = g.get_field(oRecSLD, "fk_ssla");
				if(strPriority!="") oDoc.opencall.priority = strPriority;
			}
		}
		else
