<?php
	...
	$intDelWidget = trim($_POST["wid"]);
	if($intDelWidget=="")
	{
		echo generateCustomErrorString("-777","Please specify a widget to delete",true);
		exit(0);

	}

	$rsDeleted = delete_widget($intDelWidget);
	if($rsDeleted)
	{
		echo '{"status":"OK"}';
		exit(0);
	...
	{
		echo generateCustomErrorString("-777","Failed to delete the widget. Please contact your administrator",true);
		exit(0);
	}
?>
