...
Editor = function()
{
	...
	this.graph.getModel().addListener(mxEvent.CHANGE, mxUtils.bind(this, function(sender, evt)
	{
		this.modified = true;

		// save xml
		this.saveBpmGraphXml();
	}));
    ...
    // Enables rubberband selection - mouse selection
    new mxRubberband(this.graph);
	mxRubberband.prototype.defaultOpacity = 20;
};

//-- nwj - make rubberband element position absolute and start at mouse point
var mxrOrig = mxRubberband.prototype.start;
mxRubberband.prototype.start = function(x,y)
{
	 if(this.sharedDiv==null)this.createShape();
	 this.sharedDiv.style.position = "absolute";
	 this.sharedDiv.style.top = x;
	 this.sharedDiv.style.left = y;
	 mxrOrig.apply(this, arguments);
}
...
Editor.prototype.saveBpmGraphXml = function()
{
	//-- change the xml fromdb attribute to 1 and save xml i database
	var gXML =this.getGraphXml();
	var xmlString = mxUtils.getXml(gXML); //-- use utility to get xml string from node (browser compatible)

	//-- TK Issue in IE10 Does not Load the the XML correctly. Do not save undefined it will fail to load the workflow so instead throw an error and set to ''
	if(typeof xmlString === "undefined")
	{
		alert("Workflow Save Error. Please contact your Administrator.");
		xmlString="";
	}
	var time =  (new Date).getTime();
	var req = mxUtils.post("service/workflow/saveworkflow.php","graphxml="+encodeURIComponent(xmlString)+"&wid="+encodeURIComponent(ESP.workflowid)+"&sessid=" + encodeURIComponent(ESP.sessionid) + "&t=" + time,function(req)
	{
		if(req.getText()!="")
		{
			alert("Workflow Save Error\n\n"+req.getText());
		}
		else
		{
			this.modified = false;
...
Editor.prototype.initStencilRegistry = function()
{
	...
	mxStencilRegistry.loadStencilSet(STENCIL_PATH + '/flowchart.xml');
};
