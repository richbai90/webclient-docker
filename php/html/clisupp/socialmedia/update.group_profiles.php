<?php
	...
	$strSelectedGroup = $_REQUEST["groupid"];
	$strSelectedAnalysts = "";

	$strDelete = "delete from socmed_groups where fk_suppgroup = '". PrepareForSQL(strtolower(gv("groupid"))). "'";
	$test.= $strDelete."<BR>";
	$swDATA->Query($strDelete);

	$strInsert = "insert into socmed_groups (fk_suppgroup, dflt_send_act) values ('".PrepareForSQL(strtolower(gv("groupid")))."','".pfs(gv("defaultsendact"))."')";
	$test.= $strInsert."<BR>";
	...
	if(($rsComp!=false)&&(!$rsComp->eof))
	{
		while(!$rsComp->eof)
		{
				...
				if(isset($_POST[$keylookup]))
				{
					...
					$count=-1;
					$strSQL = "select count(*) as analystcount from socmed_analysts where fk_acc_id = '". PrepareForSQL(strtolower($_REQUEST["profileid"])). "' and fk_analyst_id = '". PrepareForSQL(strtolower($rsComp->f("analystid"))) ."'";
					...
					if($count==0)
					{
						$strInsert = "insert into socmed_analysts (fk_acc_id,fk_analyst_id,fk_analyst_name) values ('". PrepareForSQL($key_profile). "','". PrepareForSQL($key_analyst_id). "','".PrepareForSQL($_POST[$keylookup])."')";
						$test.= $strInsert."<BR>";
						$swDATA->Query($strInsert);
					}
				...
				{
					$strDelete = "delete from socmed_analysts where fk_acc_id = '". PrepareForSQL(strtolower($_REQUEST["profileid"])). "' and fk_analyst_id = '". PrepareForSQL(strtolower($rsComp->f("analystid"))) ."'";
					$test.= $strDelete."<BR>";
					$swDATA->Query($strDelete);
				}
	...
	*/
	...
	{
		//global $test;
		echo "<font style='color:green'>".$test."Your settings were successfully updated.</font>";
	}
?>
