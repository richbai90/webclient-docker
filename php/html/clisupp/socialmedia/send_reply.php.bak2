...
<head>
<title>Send Reply - Supportworks Social Media</title>
...
<?php 	
	...
	if($_POST["msgtxt"]!="")
	{
		...
		if(!$rsUserAccounts->eof)
		{
			...
			$oTweet->sendTweet($_POST["msgtxt"],$_POST['nReplyMsgID']);
			echo "Reply Sent";
		}
		else
		{
	...
	$tweet_id = $_GET['intTweetId'];
	if($tweet_id=="") 
		$tweet_id = gv('intTweetId');
	
	$msg_id= $_GET['intSwMsgID'];
	if($msg_id=="")
		$msg_id = gv('intSwMsgID');
	
	$msg_defaulttxt= $_GET['strDefaultText'];
	if($msg_defaulttxt=="")
...
?>
					<form name="frmSendMsg" method="POST" action="./twitter/reply_sent.php">
						...
						<input type="hidden" name="nTweetReplyMsgID" value="<?php echo $tweet_id;?>">
						<input type="hidden" name="nReplyMsgID" value="<?php echo $msg_id;?>">

						<table border=0>
							<tr><th style="text-align:right;">To:</th><td><?php echo $msg_defaulttxt;?></td></tr>
						...
						<tr><td></td><td><textarea id="msgtxt" name="msgtxt" rows="4" cols="70" onkeyup="Javascript:update_msg_character_count(document.getElementById('msgtxt').value, document.getElementById('msgcharcount'));">@<?php echo $msg_defaulttxt;?> </textarea></td></tr>
						<tr><td></td><td><div id="msgcharcount">140 Characters Remaining</div></td></tr>
						<tr><td colspan=2 style="text-align:right;"><input type="button" onclick="Javascript:window.close();" value="Cancel">&nbsp;<input type="submit" value="Send Reply"></td></tr>
						</table>
