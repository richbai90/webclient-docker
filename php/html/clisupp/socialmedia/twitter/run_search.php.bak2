<?php
	...
	if(gv('intPageId')>1)
	{
		$maxMsgSearchPage=gv('intPageId'); //-- Use to track last viewed msg search page so we know where to page from	
	}
	if(!gv('searchqry'))
	{
		$boolskip = true;
		$strErrorMsg = "Empty Search String";
	}
	if(!$boolskip)
	{
		$ret = oauth_request($keys, "https://api.twitter.com/1.1/search/tweets.json?count=100&q=".urlencode(gv('searchqry')));
		...
		$oSearchResults = $oResults->statuses;
	}
	if($maxMsgSearchPage==1)
	{
		if(gv('intPageId')!=-1)
		{
			$strOutput = "<div class='messagelisttitlebar'><div class='messagelisttitlebarname'>Search Results for '".gv('searchqry')."'</div><div class='messagelisttitlebaractions'><a href=\"Javascript:add_search('".gv('sessid')."','".gv('searchqry')."');\"><img src='./img/icons/zoom_in.png' alt='Save Query'/></a></div></div>";
	...
	if($oSearchResults)
	{
		...
		foreach($oSearchResults as $oCommunication)
		{
					...
					$strOutput .= "<br/><br/><div id='rtc".$oCommunication->id_str."' class='profilepicker'></div>";
							...
							$strOutput .= "<div class='messagetime'>".parseDateToTweetTime($oCommunication->created_at)."</div>";
						$strOutput .= "</div>";
					$strOutput .= "</div>";
			
				$strOutput .= "</div>";
			$messageid = $oCommunication->id;
		}
		if($scince_id>0)
		{
			$strOutput .= "<div class='messageloadmore' id='messageloadmore$maxMsgSearchPage'><a href=\"Javascript:load_younger(document.getElementById('searchquery').value,document.getElementById('messagelist'),'".gv('sessid')."',".$messageid.");\"><b>Load Younger Results</b></a>     <a href=\"Javascript:load_older(document.getElementById('searchquery').value,document.getElementById('messagelist'),'".gv('sessid')."',".$messageid.");\"><b>Load Older Results</b></a></div>";
		}else
		{
			$strOutput .= "<div class='messageloadmore' id='messageloadmore$maxMsgSearchPage'><a href=\"Javascript:load_older(document.getElementById('searchquery').value,document.getElementById('messagelist'),'".gv('sessid')."',".$messageid.");\"><b>Load Older Results</b></a></div>";
		}
	}
	else if($strErrorMsg!="")
	{
