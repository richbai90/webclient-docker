<?php
	...
	$ret = oauth_request($keys, "https://api.twitter.com/1.1/direct_messages.json?count=100&page=".$maxMsgSearchPage);
	...
	if($oSearchResults)
	{
		...
		foreach($oSearchResults as $oCommunication)
		{
			...
			$strOutput .= "<div class='messagewrap' id='messagewrap".$oCommunication->id."'>";
				...
				$strOutput .= "<div class='messageheader'><div class='messageauthor'><a href='Javascript:showUser(\"infopanel\",".$oCommunication->sender_id.",\"$user_name\",\"".gv('sessid')."\");'>@".$oCommunication->sender_screen_name."</a></div>";
				$strOutput .= "<div class='messagelinks'>";
				$strOutput .= "<a href='Javascript:send_dm(\"$oCommunication->sender_id\",\"$user_name\",\"".gv('sessid')."\");'><img src=\"./img/icons/user_comment.png\" alt='Direct Message'></a>";
				$strOutput.="</div>";
