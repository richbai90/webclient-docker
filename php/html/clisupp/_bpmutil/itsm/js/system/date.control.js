...
function show_date(anEvent, callbackFunction)
{
	...
	var srcEle = app.getEventSourceElement(anEvent);
	var showDate = new Date();

	var arrPos = app.findPos(srcEle); 

	posL = new Number(arrPos[0]);
	posT = new Number(arrPos[1]);

	//alert(srcEle.offsetHeight)
	var oDiv = srcEle.document.getElementById("date-picker");
	app.setPos(oDiv,posL,(posT + srcEle.offsetHeight));
	oDiv.style['display'] = "block";

	if(isIE)
	{
		var oIF = srcEle.document.getElementById("date-picker-shimer");
		oIF.style['display'] = "block";
		app.setPos(oIF,posL,(posT + srcEle.offsetHeight));
	}

	set_days_view(showDate, srcEle.document)
