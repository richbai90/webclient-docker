<?php
	...
	$FileNameDisplay = substr($FileName, ( (false === strpos($FileName,'/'))?$FileName:(strpos($FileName,'/') + 1) ) );
...
//	$path = preg_replace("/&{1}$/","",'../wizards/'.$FileName);
	$path = preg_replace("/&{1}$/","",$_SESSION['strDirectory'].$FileName);
	readfile($path);
	exit(0);
?>