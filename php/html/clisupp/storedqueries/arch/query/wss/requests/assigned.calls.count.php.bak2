...
switch ($strCallStatus) {
    case "notstarted":
        ...
        $where .= "cstm_rel_opencall_archgroups.status = 'Not Started'";
        break;
    case "inprogress":
        if(strtolower(trim($strTable)) == 'opencall' && !$boolAddTables){
            $sqlDatabase = "sw_systemdb";
        ...
        $where .= "cstm_rel_opencall_archgroups.status = 'In Progress'";
        break;
    case "completed":
        $sqlDatabase = "swdata";
        $where .= "cstm_rel_opencall_archgroups.status = 'Completed'";
        break;
    case "all":
        $sqlDatabase = "swdata";
...
$groups = "($gID)"; 
$where .= " AND " ;
$where .= " cstm_rel_opencall_archgroups.groupid in $groups";
$where .= " ".$strFilter;
