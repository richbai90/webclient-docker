<?php
  ...
  if((OPTION_CAN_VIEW_ORGCALLS & $intCustWebflag) == 0) {
	...
	if ($aRS->Fetch()) {
		...
		echo generateCustomErrorString("-303","Failed to retrieve Customer Organisation details. Please contact your Administrator.".$strSQL);
		exit(0);
	}
	...
	//--Set Where
	$where = " WHERE opencall.fk_company_id = '".PrepareForSQL($strOrgId)."'";
