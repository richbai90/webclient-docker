<?php
  ...
  $sqlDatabase = "swdata";
  $sqlCommand = "SELECT oc.*, ag.groupname, ag.groupid, ag.accepted_by, ag.id as archgroupid, udb.attrib1, udb.attrib2, udb.firstname, udb.surname FROM opencall oc left join cstm_rel_opencall_archgroups ag on oc.callref = ag.callref left join userdb udb on oc.cust_id = udb.keysearch WHERE oc.callref = ".$intCallref;
