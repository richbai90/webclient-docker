<?php
  ...
  $sqlDatabase = "swdata";
  $sqlCommand = "SELECT oc.*, rfc.*, ag.complete, ag.groupname, ag.groupid, ag.status as arch_status, ag.accepted_by, ag.accepted_by_name, ag.id as archgroupid, udb.attrib1, udb.attrib2, udb.firstname, udb.surname FROM opencall oc left join itsm_opencall_rfc rfc on oc.callref = rfc.callref left join cstm_rel_opencall_archgroups ag on oc.callref = ag.callref left join userdb udb on oc.cust_id = udb.keysearch WHERE oc.callref = ".$intCallref;
