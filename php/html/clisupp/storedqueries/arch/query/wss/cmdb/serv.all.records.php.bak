<?php
  ...
  $strAdditionalTables = $_POST['addtables'];
	...
	if ($serviceKeys == '') $serviceKeys = '-1';

	//Get cust details from userdb
	$strSQL  = "SELECT fk_company_id, department, site, subdepartment FROM userdb WHERE keysearch = '".$strWssCustId."'";
	$aRS = get_recordset($strSQL, 'swdata');
	if($aRS->result==false) {
		echo generateCustomErrorString("-303","Failed to retrieve Customer record. Please contact your Administrator.");
		exit(0);
	}
	$strOrgID = "";
	$strSiteID="";
	...
	$strSubDeptID="";
	if($aRS->Fetch()) {
		$strOrgID  = $aRS->GetValueAsString("fk_company_id");
		$strSiteID  = $aRS->GetValueAsString("site");
		$strDeptID  = $aRS->GetValueAsString("department");
		$strSubDeptID  = $aRS->GetValueAsString("subdepartment");
	}
	$relServiceKeys = get_cust_avail_rels($strOrgID, $strSiteID, $strDeptID, $strSubDeptID);
	if ($relServiceKeys == '') $relServiceKeys = '-1';

	//Build SQL to get CI details
	$strWhere = " WHERE	config_itemi.pk_auto_id IN (".$serviceKeys.")
								OR (config_itemi.ck_config_type like 'Service%' AND config_itemi.pk_auto_id in(".$relServiceKeys."))
