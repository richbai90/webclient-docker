...
if(!clasS_exists('itsmFunctions'))
{
	...
	function wssRequestAccess ($instanceid, $custid, $wsscustid, $callref, $webflag, $analystID = "") {
		...
		if($custid != $wsscustid) {
			if ($debug)
				error_log("WssCustId not equal to CustId");
			$strSQL = "SELECT value FROM websession_config WHERE instanceid = '".PrepareForSQL($instanceid)."' AND name ='ac_id'";
			$aRS = get_recordset($strSQL, 'sw_systemdb');
			if ($aRS->Fetch()) {
				$strAuthIDFld = get_field($aRS,"value");
			} else {
				$msg = "Failed to retrieve Web Session Configuration. Please contact your Administrator.";
				if ($debug)
				{
			...
			if($strAuthIDFld == 'keysearch') {
				$msg = "Requested Customer ID does not match Session Customer ID. Please contact your Administrator.";
				if ($debug)
					error_log($msg);
				return($msg);
			}
