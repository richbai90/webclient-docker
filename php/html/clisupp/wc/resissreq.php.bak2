<?php 
	...
	swdebug_print("RESISSREQ.PHP SessionID: " . $SwAnalystWebSession);

	if($con && $con < 33)
	{
		$source = $HTTP_POST_FILES['resissdata']['tmp_name'];

		if(swhd_sendcommand($con, "RESOLVE ISSUE " . $issueref))		// Start the transaction
		{
