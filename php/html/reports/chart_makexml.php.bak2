...
//-- get our request vars into local space
$lid  = gv("lid");
$sessid  = gv("sessid");
...
if (!$keys)
{
	$query = "SELECT ckeys,cvals,copts,xmlconf FROM tmp_graph_args WHERE id=".$lid;
	if (!$conf_conn->Query($query)) die("Query failed : " . htmlentities($conf_conn->error));
	$conf_conn->FetchRow();

	$keys = $conf_conn->row[0];
	$vals = $conf_conn->row[1];
	$opts = $conf_conn->row[2];
	$xml = $conf_conn->row[3];
...
$p = xml_parser_create();
xml_parse_into_struct($p, $xml, $attribs, $index);
xml_parser_free($p);
