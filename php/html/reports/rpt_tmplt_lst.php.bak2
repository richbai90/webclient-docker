...
/* 
	...
	RF		04/07/16	RickyF					The output was using  FormatValue() to generate the formatted callref on doing so a XMLMC query was initiated for each record (performance issue)
...
if (($orderby_columns[0]) || ($_POST['sortby']))
{
	...
	if ($_POST['sortby'])
	{
		if ($_POST['sortorder']=="DESC") $query .= $_POST['sortby']." DESC";
		else $query .= $_POST['sortby'];
	}
	else
	{
...
# Here the Label & Data strings for the chart engine are constructed from the values now held in the $stats array.
...
# If headings are turned off it builds a blank row.
...
# The results are finally looped through and displayed. This is also where charts are generated
# and just about everything else relating to the overall look is done.
for ($x = 0 ; $x < $rows ; $x++)
{
	# Place the values in the HTML table & keep tabs on any aggregate sums
	if (!$flag_hiderows)
	{
		if (($html_columnheads) && ($x == 0)) print '<tr>'.$html_columnheads.'</tr>';
		print '<tr><td><img src="images/space.gif" width="1" height="'.$lineheight.'"></td>';
	}
	for ($y = 0 ; $y < $cols ; $y++)
	{
		if (!$cols_to_extract[$y]['Vis']) continue;
		if (!$flag_hiderows)
		{
			...
			if ($cols_to_extract[$y]['Raw']) print '<td class="data" valign="top">'.$tree[$x][$y].'</td>';
			else
			{
				// RJC 17.01.08 Bug 60394 Added str to lower because oracle stores db names in upper case
	
				if ((strtolower($cols_to_extract[$y]['Col']) == "opencall.callref") && !is_scheduled())
				{
					$href_display = report_get_column_value($cols_to_extract,$tree[$x], "opencall.h_formattedcallref");
					//print '<td class="data"'.$cols_to_extract[$y]['WrDT'].' valign="top"><a href="../clisupp/details/'._APPCODE.'/'._CALLPHPFILE.'?sessid='.$sessid.'&callref='.filter_var($tree[$x][$y], FILTER_SANITIZE_NUMBER_INT).'"'.$callref_target.'>'.(str_replace("<", "&lt;", FormatValue($cols_to_extract[$y]['Col'], $href_display))).'</a></td>';
					// RickyF : Use the formatted callref for display value
					print '<td class="data"'.$cols_to_extract[$y]['WrDT'].' valign="top"><a href="../clisupp/details/'._APPCODE.'/'._CALLPHPFILE.'?sessid='.$sessid.'&callref='.filter_var($tree[$x][$y], FILTER_SANITIZE_NUMBER_INT).'"'.$callref_target.'>'.(str_replace("<", "&lt;", $href_display)).'</a></td>';
				}
				else if ((strtolower($cols_to_extract[$y]['Col']) == "opencall.h_formattedcallref") && !is_scheduled())
				{
					$href_link = report_get_column_value($cols_to_extract,$tree[$x], "opencall.callref");
					// print '<td class="data"'.$cols_to_extract[$y]['WrDT'].' valign="top"><a href="../clisupp/details/'._APPCODE.'/'._CALLPHPFILE.'?sessid='.$sessid.'&callref='.filter_var($href_link, FILTER_SANITIZE_NUMBER_INT).'"'.$callref_target.'>'.(str_replace("<", "&lt;", FormatValue($cols_to_extract[$y]['Col'], $tree[$x][$y]))).'</a></td>';
					// RickyF : Use the formatted callref for display value
					print '<td class="data"'.$cols_to_extract[$y]['WrDT'].' valign="top"><a href="../clisupp/details/'._APPCODE.'/'._CALLPHPFILE.'?sessid='.$sessid.'&callref='.filter_var($href_link, FILTER_SANITIZE_NUMBER_INT).'"'.$callref_target.'>'.(str_replace("<", "&lt;", $tree[$x][$y])).'</a></td>';
				}
				//DTE:- Moving the string replace to before the column overide so that we can force html tags
				//else print '<td class="data"'.$cols_to_extract[$y]['WrDT'].' valign="top">'.(str_replace("<", "&lt;", //FormatValue($cols_to_extract[$y]['Col'], $tree[$x][$y]))).'&nbsp </td>';
				else
				{
					print '<td class="data"'.$cols_to_extract[$y]['WrDT'].' valign="top">'.( FormatValue($cols_to_extract[$y]['Col'],(str_replace("<", "&lt;", $tree[$x][$y])))).'</td>';
				}
			}
		}
...
if ($flag_totals)
{
	...
	# Generate & add aggregate result rows
	...
	if ($flag_sum && $valid_sum) print '<tr><td align="right"><table border="0" cellspacing="0" cellpadding="0"><tr><td><img src="images/space.gif" width="1" height="'.$lineheight.'"></td><td class="datasum">Sum&nbsp;&nbsp;</td></tr></table></td>'.$html_sum.'</tr>';
	if ($flag_ave && $valid_ave) print '<tr><td align="right"><table border="0" cellspacing="0" cellpadding="0"><tr><td><img src="images/space.gif" width="1" height="'.$lineheight.'"></td><td class="datasum">Average&nbsp;&nbsp;</td></tr></table></td>'.$html_ave.'</tr>';
	if ($flag_min && $valid_min) print '<tr><td align="right"><table border="0" cellspacing="0" cellpadding="0"><tr><td><img src="images/space.gif" width="1" height="'.$lineheight.'"></td><td class="datasum">Minimum&nbsp;&nbsp;</td></tr></table></td>'.$html_min.'</tr>';
	if ($flag_max && $valid_max) print '<tr><td align="right"><table border="0" cellspacing="0" cellpadding="0"><tr><td><img src="images/space.gif" width="1" height="'.$lineheight.'"></td><td class="datasum">Maximum&nbsp;&nbsp;</td></tr></table></td>'.$html_max.'</tr>';
	if ($flag_count) print '<tr><td><img src="images/space.gif" width="1" height="'.$lineheight.'"></td><td align="center" colspan="'.$span.'" class="datasum"><b>Total Record Count : '.$rows.'</b></td></tr>';
}
?>
</table>
</form>
<button style="margin:1%" onclick="var value_h2 = $('.title').text();
		         ...
		         header_text += String.fromCharCode(13) + String.fromCharCode(10);

		         console.log('Cabecera');
		         console.log((header_text.match(/,/g) || []).length);
				 ...
				 for (var c = 0, m = arr.length; c < m; c++) {
		            	...
		            	if (flag==total_elements){
		            		//Remove the last semi colon from the row
