<?php
	...
	//-- given $fetchresourcepath and ids open files and echo out contents as json

	include("../../../php/session.php");
	include('../../../php/xml.helpers.php');
	
	//-- get application specific stuff
	$_applicationpath = $portal->fs_application_path;
	
	$arrReturn = new stdclass();
	$arrReturn->picklistforms = utf8_encode(@file_get_contents($_applicationpath . "xml/pickListForms/pick list forms.xml"));
	$arrReturn->treebrowserforms = utf8_encode(@file_get_contents($_applicationpath . "xml/sqltbforms/sql tree browser forms.xml"));
	$arrReturn->globalparameters = utf8_encode(@file_get_contents($_applicationpath .  "xml/globalparams/Global Parameters.xml"));
	$arrReturn->dbentityviews = utf8_encode(@file_get_contents($_applicationpath . "xml/dbevs/Database Entity Views.xml"));
	$arrReturn->swdataschema = utf8_encode(@file_get_contents($_applicationpath . "xml/db/swdata.json"));
	$arrReturn->systemschema = utf8_encode(@file_get_contents($portal->fs_root_path . "client/wcxml/system.dbschema.xml"));
	...
	$strToolbarFileName = $portal->fs_root_path . "client/forms/_system/toolbars.xml";
	$xmlfp = @file_get_contents($strToolbarFileName);
	if(!$xmlfp)
	{
		$arrReturn->systemtoolbars = "";
	}
	else
	{
		$strHTML ="";
		...
		$arrReturn->systemtoolbars = utf8_encode(trim($strHTML));
	}	
