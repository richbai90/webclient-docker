...
class Telnet {
	 ...
	 * @return boolean
	 */
	public function connect($overrideHost="",$overridePort="")
	{
		if($overrideHost!="")$this->host=$overrideHost;
		if($overridePort!="")$this->port=$overridePort;	
	...
	/** nwj - write big data to socket in chunks **/
	public function writeChunk($buffer) 
	{
		if (!$this->socket) {
		...
		if($bytes===false)	throw new Exception("Error writing to socket");
		else return $bytes;
