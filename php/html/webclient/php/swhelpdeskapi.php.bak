...
class CSwHDsession
{
	...
	function CSwHDsession($sessionid="",$server="", $webconnector="")
	{
	...
	function open_hd_connection($useport = 5005)
	{
		...
		if ($this->webconnector=="")
		{
			...
			return $this->open_sesshd_connection($useport);
		}
		else
		{
			//-- open helpdesk conn based on selfservice web connector
			return $this->open_wchd_connection($useport);
		}
	}
	...
	function open_sesshd_connection($useport = 5005)
	{
		$this->clearerror();

		$hd_connection = swhd_openasession($this->server,$this->sessid,$useport);
	...
	function open_wchd_connection($useport = 5005)
	{
		$this->clearerror();

		$hd_connection = swhd_wcopen($this->server, $this->webconnector,$useport);
		if($hd_connection < CS_HDCON_OK)
		{
			//-- returned a vaild hd connection
			return $hd_connection;
	...
	//-- 27.05.2004
	//--			- $callref - call being transfer
	function AcceptCall($callref, $strUpdate = "This call has been accetped")
	{
		//-- clear errors and create a helpdesk connection for the appropriate type of connection
		$this->clearerror();
		$con = $this->open_hd_connection();
	...
	function StartCallHold($callref,$description, $udsource="HSL Portal",$udcode="General Update",$udtype,$intHoldUntil)
	{
		...
		{	
			...
			$this->seterror("Helpdesk Command 'CLOSE CALL' was not accepted");
		}
		else
		{
			$this->seterror("Helpdesk Connection is not connected");
	...
	function StartCallUpdate($callref)
	{
		...
		{	
			...
			$this->seterror("Helpdesk Command 'UPDATE CALL ' was not accepted");
		}
		else
		{
			$this->seterror("Helpdesk Connection is not connected");
...
//-- helper
function _getHdResultAsNamedArray($strResult)
{
	$strResult = substr($strResult, 4);
	$resultarray = array();
	//-- Loop through our item=value strings and split them into a named array (map)
	$values = explode(";", $strResult);
	for($x = 0; $x < sizeof($values); $x++)
	{
		$item = $values[$x];
		$pair = explode("=", trim($item));
		if(@$pair[0])	$resultarray[$pair[0]] = trim(strtr($pair[1], "\"", " "));
	}
	return $resultarray;
}
