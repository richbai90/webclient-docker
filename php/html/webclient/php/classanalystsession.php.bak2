...
class classAnalystSession
{
	...
	function classAnalystSession($server_name)
	{
		...
		else
		{
			...
			$this->server_port = ($GLOBALS["HTTPS"]=="on")?443:$_SERVER['SERVER_PORT'];
	...
	function IsValidSession($sessionid)
	{
		...
		if($con)
		{
			...
			//-- nwj - 06.08.2009 - select bare minimum to validate session exists
			$result = odbc_exec($con, "SELECT s.analystid FROM swsessions s, swanalysts a WHERE s.SessionID = '" . $sessionid . "'" . " AND a.analystid=s.analystid");
			if($result)
			{
	...
	function LoadSessionConfig()
	{
		...
		if($con)
		{
			//	<FN dt=16-Oct-2006> $GLOBALS['tz'] is completely irrelevant when displaying timestamps from the past!
		  	//	To avoid its misuse, the column [timezoneoffset] was purposely commented in the SQL statement!!!!
			$result = odbc_exec($con, "SELECT * FROM swsessions s, swanalysts a WHERE s.SessionID = '" . $this->sessionid . "'" . " AND a.analystid=s.analystid");
			if($result)
			{
				while(odbc_fetch_row($result))
				{
					...
					while($i < $ColumnCount)
					{
						...
						$oAnalyst->$fieldName = odbc_result($result, $i);
					}
				...
				if ($result)
				{
					if (odbc_fetch_row($result))
					{
						$_SESSION["analyst"]->userdefaults = odbc_result($result,1);
					}
				}
