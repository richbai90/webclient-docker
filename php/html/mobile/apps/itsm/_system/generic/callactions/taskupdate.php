<?php 	$prefix = 'mpuptk_';
	...
	$strSQL = "select bpm_status_id from opencall where callref=".$_POST['_callref'];
	$rsData = new _swm_rs();
	...
	foreach($arrStatusValues as $val)
	{
		$strSelected = "";
		if($strCurrentValue == $val)
			$strSelected = "(Current Value)";

		$strHTML .= "<tr  style='cursor:pointer;'  status='"._html_encode($val)."' onclick=\"_set_bpm_status(this);\"><td width='50%'>"._html_encode($val)."</td><td width='50%'>"._html_encode($strSelected)."</td><td><img src='client/_system/images/icons/arrow.jpg'></td></tr><tr><td colspan='3'><div class='seperator'><div style='display:none;'></div></div></td></tr>";
	}
	if(haveappright("G",5))
	{
		$strHTML .= "<tr  style='cursor:pointer;' status='Skip Task (Not Required)' onclick=\"_set_bpm_status(this);\"><td width='50%'>Skip Task (Not Required)</td><td width='50%'>"._html_encode($strSelected)."</td><td><img src='client/_system/images/icons/arrow.jpg'></td></tr><tr><td colspan='3'><div class='seperator'><div style='display:none;'></div></div></td></tr>";
	}
	$strHTML .= "</table>";
	echo $strHTML;
?>
