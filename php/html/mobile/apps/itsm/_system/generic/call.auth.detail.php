<?php
	...
	{
		$strSQL = _swm_parse_string("select opencall.*,bpm_oc_auth.status as authstatus from opencall, bpm_oc_auth where bpm_oc_auth.fk_auth_id='[:_swm_sqlprep_aid]' and bpm_oc_auth.authortype='Analyst' and bpm_oc_auth.fk_callref=opencall.callref and opencall.callref="._swm_db_pfs($callref));
	...
	$strAuthStatus = $retRS->EmbedDataIntoString("rs","[:rs.authstatus.value]");
	...
	if(!$retRS->eof())
	{
		...
		if($strKeysearch!="")
		{
			...
			{
				$strCustHTML .= "<div class='grptitle'><span class='grptitlecontainer'><span class='grptitle'>Customer</span></span></div>";
				$strCustHTML .= "The customer '".$strKeysearch."' could not be loaded.";
			}
		}
	...
	$parseHTMLContent .= get_authorise_actions($strStatus,$strCallref,$strAuthStatus,$strCallclass);
	...
	$strOtherInputs .="<input type='hidden' name='_callaction' id='_callaction' value='Authorise'>";
