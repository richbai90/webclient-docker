<?php
	...
	$strSQL = _swm_parse_string("select count(*) as callcount, callclass from opencall where suppgroup='".lang_encode_from_utf(_swm_db_pfs($strGroupId))."' group by callclass");
	//echo $strSQL;
	$retRS = new _swm_rs();
	$retRS->query("swdata",$strSQL,true,null);

	$strChartXML="<chart caption='My Groups Open Calls' showPercentValues='1'>";
