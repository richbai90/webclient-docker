<?php
	...
	$strSQL = _swm_parse_string("select count(withinfix) as callcount, withinfix from opencall where owner='".lang_encode_from_utf(_swm_db_pfs($strAnalystId))."' and (status > 15 or status = 6) group by withinfix");
	//echo $strSQL;
	$retRS = new _swm_rs();
	$retRS->query("swdata",$strSQL,true,null);

	$strChartXML="<chart caption='My SLA Performance' showPercentValues='1'>";
