<?php
	...
	//-- methods that do complex data conversions
	include_once(_get_file_loc("[:_swm_client_path]/_helpers/sqlquery.complexconversions.php"));
	...
	class _swm_rs
	{
		...
		function query($strDB,$strSQL,$strFormat = false,$xmlComplexConversions = null, $intRowLimit = "")
		{
			$this->_columns = Array();
			...
			{			
				...
				if($strFormat=="true")
				{
					...
					if($xmlmcf->Invoke("data","sqlQuery"))
					{
	...
	class _swm_Column
	{
		...
		function formatwithinfixtotext()
		{
			if($this->value==0)return "SLA Breached";
			return "SLA Met"; 		
		}
	}
